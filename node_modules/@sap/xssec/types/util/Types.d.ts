...
export type JwksConfig = {
    /**
     * if true, shares the JWKS cache with the first instance of the same Service type that was created with this flag set to true, otherwise creates a new JWKS cache for each instance. Ignored when a custom impl is provided, since a custom cache is managed externally.
     */
    shared?: boolean;
    /**
...
export type Cache = {
     ...
     */
    set: (arg0: string, arg1: any) => (void | Promise<void>);
    /**
     * Retrieves the value of the given key from the cache.
     */
    get: (arg0: string) => (any | Promise<any>);
};
...
export type IdentityServiceJwtPayload = {
    ...
    app_tid?: string;
    /**
     * The app_tid of the SAP Cloud Identity Service application identified by the `azp` claim (authorized party). In app2app flows this is the sender's tenant, which can differ from `app_tid` (the receiver's tenant).
     */
    azp_app_tid?: string;
    /**
    ...
    ias_apis?: string[];
    /**
     * ID Type of principal
     */
    sap_id_type?: "app" | "user";
    /**
     * SCIM ID
     */
    scim_id?: string;
