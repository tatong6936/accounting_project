...
/**
 * @typedef {object} FunctionRendererContext
 * @property {Function} renderArgs - Renders a function argument node to SQL string.
 * @property {Function} error - Compiler error reporting function.
 * @property {string[]} path - Current CSN path for error reporting.
 * @property {{ embeddingDefaultModel: string, embeddingHanaDefaultRemoteSource: string }} vector - Vector embedding options.
 */

/**
 * @typedef {(this: FunctionRendererContext, signature: object) => string} DbFunction
 */
 ...
 */
/** @type {Record<string, Record<string, DbFunction>>} */
...
const hanaFunctions = {
  sqlite: {
    ...
    //
    // Note: Vector functions for SQLite (cosine_similarity, l2distance, l2normalize and vector_embedding) have the
    // same parameters as on HANA because we provide them via UDFs
    //
  },
  postgres: {
    nano100_between(signature) {
    ...
    //
    // Vector functions for postgres vector support based on the https://github.com/pgvector/pgvector
    //
    cosine_similarity(signature) {
      const args = getArgs(signature);
      checkArgs.call(this, 'cosine_similarity', args, 2);
      const x = this.renderArgs({ ...signature, args: [ args[0] ] });
      const y = this.renderArgs({ ...signature, args: [ args[1] ] });

      return `1 - cosine_distance(${ x },${ y })`;
    },
    l2distance(signature) {
      const args = getArgs(signature);
      checkArgs.call(this, 'l2distance', args, 2);
      const x = this.renderArgs({ ...signature, args: [ args[0] ] });
      const y = this.renderArgs({ ...signature, args: [ args[1] ] });
    ...
    l2normalize(signature) {
      const args = getArgs(signature);
      checkArgs.call(this, 'l2normalize', args, 1);
      const x = this.renderArgs({ ...signature, args: [ args[0] ] });
  ...
  h2: {
    ...
    //
    // Note: Vector functions for H2 (cosine_similarity, l2distance, l2normalize, and vector_embedding) have the
    // same parameters as on HANA because we provide them via UDFs
    //
  },
  common: {
    vector_embedding(signature) {
      /** @type {FunctionRendererContext} */ const ctx = /** @type {any} */ (this);
      return vectorEmbedding.call(ctx, signature);
    },
  },
  ...
  hana: {
    ...
    //
    // Vector functions - only identity check
    //
    cosine_similarity(signature) {
      const args = getArgs(signature);
      checkArgs.call(this, 'cosine_similarity', args, 2);
    ...
    vector_embedding(signature) {
      /** @type {FunctionRendererContext} */ const ctx = /** @type {any} */ (this);
      const sql = vectorEmbedding.call(ctx, signature);
      if (sql.match('SAP.'))
        return sql;
      // If the model does not start with SAP an additional fourth parameter
      // is necessary specifying the remote source behind which SAP AI Core lies on HANA
      return `${ sql.substring(0, sql.length - 1) }, ${ this.vector.embeddingHanaDefaultRemoteSource })`;
    },
  },
};
/**
 * Arguments:
 * 1. (mandatory) property for which to generate a vector
 * 2. (optional) DOCUMENT / QUERY kind for embedding function. If not provided we default to 'DOCUMENT'
 * 3. (optional) model. If not provided we default to a compiler setting or else the SAP NXY model
 * @this {FunctionRendererContext}
 * @param {object} signature
 * @returns {string}
 */
function vectorEmbedding( signature ) {
  const args = getArgs(signature);
  // Default to kind DOCUMENT if no kind is specified
  if (args.length === 1)
    args.push('DOCUMENT');
  // Default to the compiler option vectorEmbeddingDefaultModel if no model is specified
  if (args.length === 2)
    args.push(this.vector.embeddingDefaultModel);
  checkArgs.call(this, 'vector_embedding', args, 3);
  const field = this.renderArgs({ ...signature, args: [ args[0] ] });
  const kind = this.renderArgs({ ...signature, args: [ args[1] ] });
  const model = this.renderArgs({ ...signature, args: [ args[2] ] });
  return `vector_embedding(${ field }, ${ kind }, ${ model })`;
}
