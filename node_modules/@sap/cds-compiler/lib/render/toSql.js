...
const { transformExprOperators, isTopLevelPredicate } = require('./utils/operators');
...
function toSqlDdl( csn, options, messageFunctions ) {
  ...
  function renderViewColumn( col, elements, env ) {
    ...
    if (leaf && elements[leaf]?.virtual) {
      ...
      // HANA rejects bare predicates in the select list; wrap them in CASE … WHEN.
      result = env.indent + ((options.sqlDialect === 'hana' && isTopLevelPredicate(col))
        ? wrapPredicateInCase(renderExpr({ xpr: col.xpr }, env))
        : renderExpr(withoutCast(col), env));
      if (col.as)
...
/**
 * Wrap a rendered predicate in `CASE WHEN <p> THEN TRUE WHEN NOT (<p>) THEN FALSE
 * END`, as HANA requires for a predicate in the select list. The negated `WHEN`
 * (not an `ELSE`) keeps a NULL predicate result as NULL rather than FALSE.
 *
 * @param {string} predicate Rendered predicate SQL
 * @return {string}
 */
function wrapPredicateInCase( predicate ) {
  return `CASE WHEN (${ predicate }) THEN TRUE WHEN NOT (${ predicate }) THEN FALSE END`;
}
