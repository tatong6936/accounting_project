...
const { transformAnnotationExpression, implicitAs, isSimpleAnnoExprRef } = require('../../model/csnUtils');
 ...
 */
function adaptAnnotationsRefs(generatedForeignKeys, csnUtils, { error }, elementPath, options) {
  if (Array.isArray(generatedForeignKeys?.[0])) {
    ...
    adaptAnnotationsRefs(remapToArrayOfObjects(generatedForeignKeys), csnUtils, { error }, elementPath, options);
  }
  else {
          ...
          ref: (_parent, _prop, ref, path, _p, _ppn, ctx) => {
            ...
            if (csnUtils.isManagedAssociation(art)) {
              ...
              if (gfkForRef.length === 1) {
                ...
                if (ctx?.annoExpr?.['=']) {
                  // TODO: Apply the same fix for OData once the OData behavior is aligned;
                  //       when done, the `options` parameter can be removed from this function.
                  if (options?.transformation === 'effective')
                    ctx.annoExpr['='] = isSimpleAnnoExprRef(ctx.annoExpr) ? ctx.annoExpr.ref.join('.') : true;
                  else
                    ctx.annoExpr['='] = true;
                }
              }
