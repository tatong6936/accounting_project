...
const {
  dictAdd, dictAddArray, dictForEach,
...
function populate( model ) {
  ...
  function traverseElementEnvironments( art ) {
    ...
    else if (!isLeadingQuery( art )) { // already done for leading query elems
      while (art.items)
        art = art.items;
  ...
  function effectiveType( art ) {
    ...
    for (const a of chain) {
      ...
      if (a.kind !== 'select' && a.kind !== '$tableAlias')
  ...
  function populateArtifact( art, origEffective ) {
    ...
    if (art.kind === '$tableAlias') {
      ...
      proxyCopyMembers( art, 'elements', origEffective.elements, art.path?.location,
                        '$navElement' );
      return art;
    }
    if (art.query) {            // query entity/event/...
    ...
    if (art.expand) {
      ...
      if (!art.value) {
        ...
        if (origEffective?.target) // consider future feature `{ … } as x: AssocType
  ...
  function initFromColumns( query, columns, inlineHead = undefined ) {
    ...
    if (!inlineHead) {
      ...
      if (isLeadingQuery( query )) // never the case for 'expand'
  ...
  function assocIsToBeRedirected( assoc ) {
    ...
    return !query || isLeadingQuery( query );
  }
  ...
  function redirectImplicitlyDo( elem, assoc, target, service ) {
    ...
    if (assoc._main === target && elem._main?.kind === 'entity' &&
      ...
      // TODO: check with auto-exposed entities (which do not have _ancestors)
      return elem._main;
    }
  ...
  function isDirectProjection( proj, base ) {
    const { query } = proj;
    return query?.op?.val === 'SELECT' && proj.kind === 'entity' && // not event
      query.from?.path &&
      // TODO: what about navigation along assoc?
      base === resolvePath( query.from, 'from', query );
  }
  ...
  function isAutoExposed( target ) {
    // remark: effectiveType( target ) has been called → we can use links directly
    if (target.$autoexpose !== undefined)
      return target.$autoexpose;

    const origTarget = target;
    const chain = [];
    while (target.$autoexpose === undefined && setAutoExposed( target )) {
      // stop at first ancestor with annotation or at non-query entity
      let source = target._origin; // first table alias of leading query
      // no propagation along `Source:assoc` → cannot traverse source._origin
      // until we find main artifact
      while (source?._origin?._main) // sub query in from
        source = source._origin._origin; // its first table alias
      source = source?._artifact; // different to source._origin if `Source:assoc`
      if (!source || source._main) // ref error of `Source:assoc`
        break;
      chain.push( target );
      target = source;
    }
  ...
  function setAutoExposed( art ) {
    ...
    if (anno && anno.val !== null) { // XSN TODO: set val, but no location for anno short form
      ...
      return false;             // stop with @cds.autoexpose
    }
    // no @cds.autoexpose or @cds.autoexpose:null
    art.$autoexpose = model.$compositionTargets[art.name.id] ? true : null;
    return art.query; // still check for inherited @cds.autoexpose if query entity
  }
