...
class LinkedCSN {
  ...
  constructor (x,...etc) {
    ...
    function _link (d, name, parent, _kind) {
      ...
      else if (d.projection)                _set (d,'query', _query({SELECT:d.projection}))
      else if (d.returns)                   _link (d.returns)
      else if (d.items)                     _link (d.items)
      else if (d.value)                     _expr (d.value)
      else if (d.$calc && d.$calc !== true) _expr (d.$calc)
      else if (d.query)                     _query (d.query)
  ...
  *each (x, defs=this.definitions) {
  ...
  get entities() {
    // REVISIT: remove _compat_texts_entities with cds^11
    ...
    for (let d of this.definitions) if (filter(d)) entities[d.name] = d // adds all entities
...
/** Mark all {val} in an expression as non-params */
const _expr = x => {
  if (typeof x === 'string') return
  else if ('val' in x) Object.defineProperty(x,'param',{value:false})
  else if (x.xpr) x.xpr.forEach(_expr)
  else if (x.list) x.list.forEach(_expr)
  else if (x.func && x.args) (Array.isArray(x.args) ? x.args : Object.values(x.args)).forEach(_expr)
  else if (x.SELECT) _query(x)
}
/** Mark all {val} in a query's clauses as non-params */
const _query = q => {
  if (!q.SELECT) return //> a UNION or similar query without SELECT
  const { columns, where, having } = q.SELECT
  if (columns) columns.forEach(_expr)
  if (where) _expr (where)
  if (having) _expr (having)
  return q
}
