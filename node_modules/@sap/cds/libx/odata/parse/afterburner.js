...
const RELAXED_UUID_REGEX = /^[0-9a-z-]{1,36}$/i

const _isRelevantKey = k => k !== 'IsActiveEntity'
...
function _processWhere(where, entity) {
  for (let i = 0; i < where.length; i++) {
    ...
    if (element) {
      if (element.isAssociation && element.is2many) {
        const path = realRef.ref.map(r => r.id ?? r).join('/')
        cds.error({
          status: 400,
          message:
            `The collection-valued navigation property "${path}" cannot be used as an operand of a comparison in $filter. ` +
            `Use "${path}/any(...)" or "${path}/all(...)" instead.`
        })
      }
      const odataOp = { '>': 'gt', '<': 'lt', '>=': 'ge', '<=': 'le' }[operator]
      if (element.isAssociation && odataOp) {
        const path = realRef.ref.map(r => r.id ?? r).join('/')
        cds.error({
          status: 400,
          message:
            `The navigation property "${path}" cannot be compared with "${odataOp}" in $filter. ` +
            `The relational operators gt/lt/ge/le are defined only for primitive operands; for navigation properties only eq/ne are allowed.`
        })
      }
      i += 2
...
function _processSegments(from, model, namespace, cqn, protocol) {
  ...
  for (let i = 0; i < ref.length; i++) {
    ...
    if (incompleteKeys) {
      ...
      // REVISIT: 404 or 400?
      if (!current || current.kind === 'event') throw new cds.error(404, `Invalid resource path "${path}"`)
      ...
      if (current.params && current.kind === 'entity') {
        ...
        if (current._target._service !== target._service) {
          // not exposed target
          const _target = Object.create(target, { name: { value: target.name.replace(namespace + '.', '') } })
          _doesNotExistError(false, current.name, _target)
        }
...
const _checkAllKeysProvided = (params, entity) => {
  ...
  for (const keyInParam in params) {
    // REVISIT cds^11: entity.keys is used instead of keysOfEntity because it also covers keys excluded
    // from keysOf (e.g. @cds.api.ignore or up_ FK backlinks) that may still appear in request URLs.
    // Reconsider: it's questionable whether @cds.api.ignore-annotated keys should be accepted in URLs,
    // and current Fiori versions may no longer send requests with up_ FK keys.
    const allowed = isView
      ? keyInParam in (entity.params ?? {})
      : entity.keys?.[keyInParam] && !entity.keys[keyInParam].isAssociation
    if (!allowed) {
      // prettier-ignore
      const msg = `${isView ? 'Parameter' : 'Key'} "${keyInParam}" is not a ${isView ? 'parameter' : 'key'} of ${isView ? 'view' : 'entity'} "${entity.name}"`
      cds.error({ status: 400, message: msg })
    }
  }
}

const _doesNotExistError = (isExpand, refName, { name, kind, _return_of }) => {
  let msg
  if (_return_of) {
    const _return_of_name = _return_of.parent
      ? _return_of.name
      : _return_of.name.replace(_return_of._service.name + '.', '')
    msg = `The return type of ${_return_of.kind} "${_return_of_name}" does not have property "${refName}"`
  } else {
    msg = isExpand
      ? `Navigation property "${refName}" does not exist in "${name}"`
      : `Property "${refName}" does not exist in ${kind === 'type' ? 'type ' : ''}"${name}"`
  }
  cds.error({ status: 400, message: msg })
}

function _validateXpr(xpr, target, model, aliases = []) {
  if (!xpr) return []
  ...
  for (let i = 0; i < xpr.length; i++) {
    ...
    if (x.xpr) {
      _validateXpr(x.xpr, target, model, aliases)
      continue
    }
    ...
    if (x.ref) {
      ...
      if (x.ref[0].where) {
        ...
        if (!element) _doesNotExistError(true, refName, target)
        _validateXpr(x.ref[0].where, element._target ?? element.items, model)
      }

      if (ignoredColumns.includes(refName) || (!target?.elements?.[refName] && !aliases.includes(refName))) {
        _doesNotExistError(x.expand, refName, target)
      }
      ...
      if (x.ref.length > 1) {
        ...
        if (element.isAssociation) {
          // navigation
          _validateXpr([{ ref: x.ref.slice(1) }], element._target, model)
        } else if (element.kind === 'element') {
          // structured
          _validateXpr([{ ref: x.ref.slice(1) }], element, model)
        } else {
      ...
      if (x.expand) {
        ...
        if (!element._target || element._target._service !== target._service) {
          ...
          _doesNotExistError(true, refName, Object.create(target, { name: { value: targetName } }))
        }

        _validateXpr(x.expand, element._target, model)

        if (x.where) _validateXpr(x.where, element._target, model)
        if (x.orderBy) _validateXpr(x.orderBy, element._target, model)
      }
    }

    if (x.func) {
      _validateXpr(x.args, target, model, aliases)
      continue
    }

    if (x.SELECT) {
      const { target } = targetFromPath(x.SELECT.from, model)
      _validateQuery(x.SELECT, target, model)
    }
  }
...
function _validateQuery(SELECT, target, model) {
  const aliases = []

  if (SELECT.from.SELECT) {
    const { target } = targetFromPath(SELECT.from.SELECT.from, model)
    aliases.push(..._validateQuery(SELECT.from.SELECT, target, model))
  }

  aliases.push(..._validateXpr(SELECT.columns, target, model, aliases))

  _validateXpr(SELECT.orderBy, target, model, aliases)
  _validateXpr(SELECT.where, target, model, aliases)
  _validateXpr(SELECT.groupBy, target, model, aliases)
  _validateXpr(SELECT.having, target, model, aliases)
...
module.exports = (cqn, model, namespace, protocol) => {
  ...
  if (one && cqn.SELECT.columns?.[0]?.as === '$count')
    throw new cds.error({ status: 400, message: '$count cannot be applied on a single instance' })

  let returns_target
  if ({ action: 1, function: 1 }[current.kind]) {
    // validate query options against the return type
    returns_target = current.returns?.items ?? current.returns ?? {}
    _validateQuery(cqn.SELECT, Object.create(returns_target, { _return_of: { value: current } }), model)

    // remove everything but the subject if the operation name is not draftPrepare, et al.
    if (!current.name.match(/^draft[A-Z]/)) cqn.SELECT = { from: cqn.SELECT.from }
  }
  ...
  let _sel = cqn.SELECT
  while (_sel) {
    if (_sel.where) _processWhere(_sel.where, target)
    _sel = _sel.from?.SELECT
  }
  ...
  if (target && !returns_target) {
    ...
    _validateQuery(cqn.SELECT, target, model)
  }
