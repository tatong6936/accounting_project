    ...
    const spawn = _begin(async () => {
          ...
          try {
            ...
            const result = await _run({ ...task.context, tenant }, async () => {
              ...
              if (!opts.legacyLocking) {
                ...
                if (task.msg.queue?.task) _delete.where({ timestamp: { '<': _timestampPlusOneMS(task.msg.timestamp) } })
                await _delete
              }
              return result
            })
      ...
      for (const task of succeeded) {
        ...
        if (task.msg.queue?.every) {
          const _m = { ...task.msg }
          if (task.msg.queue.task) {
            // Read from database as the singleton task might have been modified/deleted in the meantime
