...
class RedisMessaging extends cds.MessagingService {
  async init() {
    ...
    // Pass GCP Memorystore server CA from binding into TLS options.
    // BTP GCP regions ship a private CA at credentials.tls.server_ca; without it,
    // node-redis fails with UNABLE_TO_VERIFY_LEAF_SIGNATURE.
    const ca = credentials && typeof credentials.tls === 'object' && credentials.tls.server_ca
    if (ca) config.socket = { ...config.socket, tls: true, ca }
