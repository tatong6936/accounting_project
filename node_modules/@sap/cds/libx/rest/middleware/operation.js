 ...
 */
exports.operation = async function rest_operation(req, res) {
  const op = req._operation
  const cdsReq = this.request4({ event: op.name, data: req._data, params: req._params, req, res })
  ...
  if (req._query) cdsReq.query = req._query
  else cdsReq.event = op.name.replace(`${this.service.namespace}.`, '')

  // Send the request to the service to be handled
  const result = await this.service.dispatch(cdsReq)
