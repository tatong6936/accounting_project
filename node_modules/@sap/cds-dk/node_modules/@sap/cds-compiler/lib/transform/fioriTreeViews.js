 ...
 */
function generateFioriTreeViewAnnotationsAndFields(def, defName, messageFunctions, csnUtils, transformers, options, csn) {
  ...
  function addHierarchyFields(def, defName) {
    addHierarchyFieldsTo(def, defName);

    // The localized convenience view is a stand-in for the annotated view at
    // runtime, so it must carry the same hierarchy columns. It is generated
    // before this transform runs, hence it is augmented here explicitly.
    const localizedView = csn?.definitions?.[`localized.${ defName }`];
    if (localizedView)
      addHierarchyFieldsTo(localizedView, `localized.${ defName }`);
  }

  function addHierarchyFieldsTo(def, defName) {
