...
function effectiveCsn( model, options, messageFunctions ) {
  ...
  // Rewrite association keys in unbound action/function params to use flattened names.
  // Not needed for bound actions: their params are reached via the parent entity and handled by flattenFKs in managedToUnmanaged.
  forEachDefinition(csn, (def) => {
    ...
    for (const param of Object.values(def.params)) {
      if (!param.elements)
        continue;
      for (const element of Object.values(param.elements)) {
        if (element.keys)
          element.keys.forEach((key) => {
            key.ref = [ key.ref.join('_') ];
          });
      }
    }
  });
