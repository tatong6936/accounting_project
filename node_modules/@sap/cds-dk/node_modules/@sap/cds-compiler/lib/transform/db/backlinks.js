...
function getBacklinkTransformer( csnUtils, messageFunctions, options, pathDelimiter, doA2J = true ) {
  ...
  function transformDollarSelfComparison( assocOp, assoc, assocName, elem, elemName, art, artifactName, path ) {
    ...
    // Skip when `$backlink` is present: the core compiler already validated the (possibly
    // redirected/included) target and provided the redirected keys/on
    ...
    if (!elem.$backlink && !art.query && !art.projection && assoc.target && assoc.target !== artifactName) {
    ...
    // Prefer the `$backlink` helper (renaming-aware, target redirected to this artifact)
    // over the backlink's own keys/on, so renamings are considered. Absent for
    // same-entity backlinks, where the plain keys/on below are already correct.
    const backlink = elem.$backlink;

    if (Array.isArray(backlink))
      return transformDollarSelfComparisonWithUnmanagedAssoc(assocOp, assoc, assocName, elemName, art, path, backlink);
    if (backlink?.keys?.length)
      return transformDollarSelfComparisonWithManagedAssoc(assocOp, assoc, assocName, elemName, art, path, backlink);
   ...
   * @param {CSN.Artifact} art
   * @param {CSN.Path} path
   * @param {object} [backlink] renaming-aware `elem.$backlink` (`{ keys }`) to use instead of `assoc.keys`
   * @returns {Array} New on-condition
   */
  function transformDollarSelfComparisonWithManagedAssoc( assocOp, assoc, originalAssocName, elemName, art, path, backlink ) {
    const conditions = [];
    ...
    const keys = backlink?.keys ?? assoc.keys;
   ...
   * @param {CSN.Artifact} art
   * @param {CSN.Path} path
   * @param {Array} [backlink] renaming-aware `elem.$backlink` ON-condition to use instead of `assoc.on`
   * @returns {Array} New on-condition
   */
  function transformDollarSelfComparisonWithUnmanagedAssoc( assocOp, assoc, originalAssocName, elemName, art, path, backlink ) {
    ...
    const newOnCond = cloneCsnNonDict(backlink ?? assoc.on, options);
