...
const { pathName, hasSpecialPathItemProp, isLeadingQuery } = require('../compiler/utils');
const { dictFirst } = require('../compiler/dictionaries');
...
function compactModel( model, options = model.options || {} ) {
  ...
  const firstSource = dictFirst( srcDict );
  if (firstSource) {
...
function enumerableQueryElements( select ) {
  return (universalCsn && !isLeadingQuery( select ));
}
...
function foreignKeys( dict, csn, node ) {
  if (!dict ||              // `Association to many Target` without specified keys
      ...
      gensrcFlavor && dict[$inferred])
    return;
  fkeys( dict, csn );
}

function fkeys( dict, csn = {} ) {
  csn.keys = [];
  for (const n in dict)
...
function addOrigin( csn, xsn, node ) {
  ...
  if (xsn.query) {
    let source = xsn._origin;   // table alias of leading query
    while (source?._main)       // source is still a query
      source = source._origin;
    if (!source)
      return;
    csn.$origin = (xsn.includes)
      ? includesOrigin( [ { _artifact: source }, ...xsn.includes ], xsn )
      : originRef( source );
  ...
  if (!origin) {
    if (parent && parentOrigin &&
        (parent.kind !== 'select' || isLeadingQuery( parent )) &&
...
function getParent( art ) {
  ...
  return (main && isLeadingQuery( parent )) ? main : parent;
}
...
// XSN `_origin` is not the same as _origin in Universal CSN...
...
function getOrigin( art ) {
  ...
  if (_origin) {                // also for query entities
    ...
    return (_origin.kind === 'builtin')
      ? undefined
      : (_origin.kind === '$tableAlias') ? _origin._origin : _origin;
  }
...
function originRef( art, user ) {
  ...
  if (user && parent._main && parent._main === user._main && !isLeadingQuery( parent ))
...
function annoValue( node ) {
  ...
  if (!csn || csn['#'] || csn.xpr && Object.keys( csn ).length === 1)
    return csn;                 // '#' and 'xpr' without sibling '='
  const asTrue = !csn?.ref || gensrcFlavor || universalCsn ||
    csn.cast || csn.param || csn.ref.some( id => typeof id !== 'string' );
  return Object.assign({ '=': !!asTrue || csn.ref.join( '.' ) }, csn );
}
...
function backlink( back, _csn, xsn ) {
  if (back._redirected?.length && !gensrcFlavor && xsn?._main?.kind === 'entity')
    return (back.on) ? condition( back.on ) : fkeys( back.foreignKeys );
  return undefined;
}
...
function exprInternal( node, xprParens ) {
  ...
  switch (val) {
    ...
    case 'and':
      for (const arg of node.args) {
        if (arg.$parens?.length)
          continue;
        if ((arg.op?.val === 'nary' || arg.op?.val === 'ixpr') &&
            arg.args[1]?.val === 'or' &&
            arg.args[1]?.literal === 'token')
          arg.$parens = [ arg.location ];
      }
      // FALLTHROUGH
  ...
  // An 1-item 'ixpr' or 'nary' can (in general) be replaced by that one item.
  // For lower *.sql diffs, not for 'xpr' in 'ixpr', produced before A2J recompilation.
  return rargs.length === 1 && !rargs[0].xpr
    ? rargs[0]
    : flattenInternalXpr( rargs, val );
}
...
function query( node, csn, xsn, _prop, expectedParens = 0 ) {
  if (node.op.val === 'SELECT') {
    ...
    if (elems && node._main && !isLeadingQuery( node ) && gensrcFlavor !== true) {
