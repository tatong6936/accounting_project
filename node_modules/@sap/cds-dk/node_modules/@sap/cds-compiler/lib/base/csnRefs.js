...
const referenceSemantics = {
  ...
  from_where: { lexical: justDollar, dynamic: 'ref-target' }, // ...using baseEnv
    ...
    if (semantics.lexical !== false) {
      ...
      if (head.charAt(0) === '$') {
        if (head !== '$self' && head !== '$projection')
          return { scope: '$magic' };
        // $self in filter of from -> TODO v8: disallow in core compiler?
        const fcache = (refCtx === 'from_where' && qcache) ? qcache.$next : qcache;
        const self = fcache && fcache.$queryNumber > 1 ? fcache._select : main;
...
function analyseCsnPath( csnPath, csn, resolve ) {
  ...
  for (; index < csnPath.length; index++) {
    ...
    if (isName || Array.isArray( obj ) || prop === 'returns') {
      ...
      refCtx = (baseCtx === 'from') ? 'from_where' : 'ref_where';
    }
