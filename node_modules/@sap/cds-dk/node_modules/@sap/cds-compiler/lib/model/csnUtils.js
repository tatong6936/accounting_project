      ...
      forAllQueries(queries, (query) => {
        if (query.SELECT?.from) {
          if (query.SELECT.from.args) {
            ...
            handleDependency(csn.definitions[query.SELECT.from], artifact, artifactName);
          }
          else if (query.SELECT.from.ref) {
            if (query.SELECT.from.ref[0].id)
              handleDependency(csn.definitions[query.SELECT.from.ref[0].id], artifact, artifactName);
            else
              handleDependency(csn.definitions[query.SELECT.from.ref[0]], artifact, artifactName);
          }
        }
  ...
  function handleArgs( artifact, artifactName, args ) {
    for (const arg of args) {
      if (arg.args)
        ...
        handleDependency(csn.definitions[arg.ref[0].id ? arg.ref[0].id : arg.ref[0]], artifact, artifactName);
    }
  }
