...
function tweakAssocs( model ) {
  ...
  function rewriteArtifact( art ) {
    ...
    if (art.query) {
      traverseQueryPost(art.query, null, (query) => {
        forEachGeneric( query, '$tableAliases', ( mixin ) => {
          if (mixin.$backlink?._redirected)
            pushLink( model, '_associations', mixin );
        } );
  ...
  function rewriteBacklink( elem ) {
    ...
    if (!backlink._redirected?.length)
      return;
    ...
    if (origin.on) {
      ...
      if (backlink.$errorReported)
        setLink( backlink, '_redirected', null ); // no further processing
    }
    else if (rewriteKeys( backlink, origin )) {
      setLink( backlink, '_redirected', null ); // no further processing
    }
  }
  ...
  function rewriteKeys( elem, assoc ) {
    ...
    if (elem.on)
      return false;             // foreign keys were transformed into ON-condition
    ...
    forEachInOrder( assoc, 'foreignKeys', ( orig, name ) => {
      ...
      if (elem._redirected)
        withIssue = rewriteKey( elem, fk ) || withIssue;
    } );
  ...
  function rewriteKey( elem, fk ) {
    ...
    for (const alias of elem._redirected) {
      if (alias.kind === '$tableAlias') {
        ...
        else if (elem._outer?.$backlink === elem) {
          const undef = targetElement.path[0];
          setArtifactLink( undef, null );
          setArtifactLink( targetElement, null );
          const outer = elem._outer;
          message( 'ref-missing-backlink-element',
                   [ (outer.value || outer.name).location, outer ],
                   { '#': (outer.value ? 'std' : 'def'), id: undef.id } );
          // TODO: more info if element was already missing in "intermediate" projection
          // TODO: collect all missing elements?
          return true;
        }
        else {
    ...
    // TODO: check message if this is reached
    return true;
  }
   ...
   * the ON-condition.
   */
  function removeArtifactLinks( elem ) {
  ...
  function addConditionFromAssocPublishing( elem, assoc, nav ) {
    ...
    // Empty filters don't add anything to the ON-condition.
    if (lastStep.where.args?.length) {
      elem.on = {
    ...
    else if (elem.on && !elem.on.$inferred) {
      elem.on.$inferred = 'copy';
    }
  ...
  function rewriteExpr( expr, assoc, tableAlias, navEnv = tableAlias ) {
    ...
    if (navEnv) { // from ON cond of element in source ref/d by table alias
      ...
      if (item.id === assoc.name.id &&
        ...
        const undef = expr.path.find( i => !i._artifact );
        const outer = assoc._outer;
        message( 'ref-missing-backlink-element',
                 [ (outer.value || outer.name).location, outer ],
                 { '#': (outer.value ? 'std' : 'def'), id: undef.id } );
        assoc.$errorReported = true; // for `_redirected: null` later
        // (do not set now, then we would see just the first missing element)
        // TODO: more info if element was already missing in "intermediate" projection
        // TODO: collect all missing elements?
      }
    }
      ...
      rewritePath( expr, item, assoc, elem, null ); // TODO: really no error?
    }
  }
   ...
   *
   * If `location` is falsy, do not issue an error.
   */
    ...
    for (const i of path) {
      if (!state) {
        ...
        if (state && state !== true)
          continue;
        if (!state && !location)
          return true;          // no rewrite possible
        break;
      }
    }
    if (state !== true)
...
function findRewriteTarget( expr, index, env, user ) {
  ...
  const items = (env.$queries?.[0] || env)._combined?.[item.id];
  ...
  const valueNav = user.value?.path[0]?._navigation;
  const tableAlias = (valueNav?.kind === '$tableAlias' &&
                      user._main?._origin?._origin === item._artifact?._main)
    ? valueNav
    : null;
