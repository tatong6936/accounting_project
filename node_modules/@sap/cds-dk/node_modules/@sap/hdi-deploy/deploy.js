...
if (fs.existsSync(defaultEnvFile)) {
  if (process.env.VCAP_SERVICES_FILE_PATH) {
    logger.log(`VCAP_SERVICES from '${defaultEnvFile}' will not take effect because of the VCAP_SERVICES_FILE_PATH environment variable.`);
  }
...
if (typeof process.env.HDI_DEPLOY_OPTIONS !== 'undefined') {
  ...
  for (const option in options) {
    if (option === 'auto_undeploy') {
      ...
      args.translateJSONEnvBooleanOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'async_make') {
      args.translateJSONEnvBooleanOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'async_make_timeout') {
      args.translateJSONEnvIntegerOptionToOption(logger, options, name, option, process.argv);
      ...
      args.translateJSONEnvBooleanOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'write_batch_size') {
      args.translateJSONEnvIntegerOptionToOption(logger, options, name, option, process.argv);
    } else {
...
for (let i = 2; i < process.argv.length; ++i) {
  ...
  if (arg === '-h' || arg === '--help') {
    const help = [
      ...
      '',
      "        --[no-]async-make     [don't] use MAKE_ASYNC instead of MAKE for the HDI make step",
      '                                  prevents connection timeouts for very long-running deployments;',
      '                                  by default, the synchronous MAKE is used',
      '        --async-make-timeout <ms>',
      '                                  number of milliseconds to wait for the async make job to finish when --async-make is active',
      `                                  by default, the timeout is ${opt.asyncMakeTimeout} ms`,
      '',
      "        --[no-]trace-vcap-services [don't] log VCAP services information",
      '                                  by default, the following VCAP services properties will be logged: name,',
      '                                  label, plan, tags, schema, database_id, user, hdi_user, and service_key_name (with --trace, the rest of the properties will also be logged)',
      '',
      ...
      '                                  number of files to write in one batch; must be a positive integer',
      `                                  by default, the batch size is ${logger.writeBatchSize}`,
      ''
    ];
  ...
  } else if (arg === '--async-make-timeout') {
    i = parseTimeoutOption('async-make-timeout', 'asyncMakeTimeout', i);
    asyncMakeTimeoutSet = true;
  } else if (arg === '--no-async-make') {
    opt.asyncMake = false;
  } else if (arg === '--async-make') {
    opt.asyncMake = true;
  } else if (arg === '--no-trace-vcap-services') {
    opt.traceVcapServices = false;
  } else if (arg === '--trace-vcap-services') {
    opt.traceVcapServices = true;
  } else if (arg === '--write-batch-size') {
    ++i;
    if (i >= process.argv.length) {
      logger.error(`Option write-batch-size requires an integer argument. Received: ${typeof process.argv[i]}`);
      process.exit(1);
    }
    logger.writeBatchSize = Number.parseInt(process.argv[i]);
    if (isNaN(logger.writeBatchSize) || logger.writeBatchSize <= 0) {
      logger.error(`Option write-batch-size requires a positive integer argument. Received: ${process.argv[i]}`);
      process.exit(1);
    }
  } else {
...
// This is called to resolve the final client and most importantly, to set `logger.client` which is used in many places
utils.getClient();
...
if (asyncMakeTimeoutSet && !opt.asyncMake) {
  logger.warn('--async-make-timeout has no effect without --async-make.');
}
...
function initializeServices (cb) {
  try {
    ...
    services = require('./lib/services.js')(opt.traceVcapServices);
    if (
      opt.detectServerVersion ||
      opt.detectHDIVersion ||
      opt.container_api_version ||
      opt.detectCloudVersion
    ) {
...
function showVersionAndOtherInformation (cb) {
  try {
    ...
    if (usedDefaultServicesFile) {
      if (process.env.VCAP_SERVICES_FILE_PATH) {
        logger.log(`The deployer detected the VCAP_SERVICES_FILE_PATH environment variable together with default-services.json.
Service definitions are loaded from the file specified by VCAP_SERVICES_FILE_PATH ("${process.env.VCAP_SERVICES_FILE_PATH}"); therefore, the contents of default-services.json will not take effect.`);
      } else {
    ...
    if (process.env.VCAP_SERVICES_FILE_PATH) {
      logger.log(`Using VCAP_SERVICES_FILE_PATH ("${process.env.VCAP_SERVICES_FILE_PATH}")`);
    }
