...
const { calculateHashFromStream, callbackTimeout, getUniqueAndDuplicatesByKey, getDeployOrUndeployFiles, writeBatchesWithTimeout } = require('./utils');
  ...
  async.each(deployedLocalFiles, (currentFilePath, cb) => {
    ...
    if (file && deployFileContent) {
      if (deployFileContent[1]) {
        ...
        } else if (typeof deployFileContent[1] === 'function') {
          stream = deployFileContent[1]();
        } else {
...
class DeployTask {
  ...
  connect () {
    ...
    if (this.options.live_messages && this.options.asyncMake) {
      this.logger.warn('--live-messages has no effect when --async-make is used.');
    } else if (this.options.live_messages) {
        ...
        try {
          this.deployContent = this.content.deployContent.map((fileOrFolder) => {
            if (isFile(fileOrFolder[0])) {
              /*
               * This is a function that returns a ReadStream. It is to avoid opening too many files at once.
               * look at content.js -- deployFileContent
               */
              if (typeof fileOrFolder[1] === 'function') {
                return fileOrFolder;
              }
  ...
  optimiseSynchronize () {
          ...
          updatedLocalFileSystem.forEach((file) => {
            this.content.deployContent.every((content) => {
              if (content.includes(file)) {
                if (isFile(content[0])) {
                  // Delay creation of ReadStream until the file is actually needed during batch writing.
                  const file = typeof content[1] === 'function' ? content : new FileWithContent(content[0], content[1]);
                  updatedDeployContent.push(file);
                } else {
    ...
    tasks.push((callback) => {
      writeBatchesWithTimeout(
        this.container,
        updatedDeployContent,
        this.logger.writeBatchSize,
        hdi_callback,
    ...
    tasks.push(function (callback) {
      async.waterfall([
        (innerCB) => {
          ...
          const make_task = new Make_Task(this.connection_id, this.container, this.hdiCreds, this.schema, this.options);
