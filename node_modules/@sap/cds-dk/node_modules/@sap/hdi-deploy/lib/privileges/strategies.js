...
class SQLGrantorStrategy {
   ...
   * @param {any} grantee Who to grant to.
   * @param {any} grantable Add WITH ADMIN OPTION.
   * @param {any} usingGroup Add WITH using group.
   *
   * @returns {undefined}
   * @memberOf SQLGrantorStrategy
   */
  handleGlobalRoles (roles, grantee, grantable, usingGroup) {
    this.tasks.push(this.client.grantRoles(roles, grantee, grantable, usingGroup));
  }
   ...
   * @memberOf SQLGrantorStrategy
   */
  handleSchemaRoles (schema, roles, grantee, grantable, usingGroup) {
    this.tasks.push(this.client.grantSchemaRoles(schema, roles, grantee, grantable, usingGroup));
  }
}
...
class HDIContainerGrantorStrategy {
   ...
   * @param {any} grantee Who to grant to.
   * @param {any} grantable Add WITH ADMIN OPTION.
   * @param {any} usingGroup Grant using role group (not supported, emits a warning).
   *
   * @returns {undefined}
   * @memberOf HDIContainerGrantorStrategy
   */
  handleSchemaRoles (schema, roles, grantee, grantable, usingGroup) {
    if (usingGroup) {
      logger.warn('using_group is not supported for HDI container grantor; grant will proceed without USING GROUP');
    }
...
class ProcedureGrantorStrategy {
  ...
  handleGlobalRoles (roles, grantee, grantable, usingGroup) {
    if (usingGroup) {
      logger.warn('using_group is not supported for procedure grantor; grant will proceed without USING GROUP');
    }
    roles.forEach((role) => {
      this.grant_privileges.push([
        'GLOBAL_ROLE',
...
class SQLRevokerStrategy {
   ...
   * @memberOf SQLRevokerStrategy
   */
  handleGlobalRoles (roles, revokee, grantable, usingGroup) {
    if (usingGroup) {
      return this.handleGlobalRolesUsingGroup(roles, revokee);
    }
    this.tasks.push(this.client.revokeRoles(roles, revokee));
  }

  /**
   * Revoke global roles from the revokee using group.
   * @param {any} roles Roles to revoke.
   * @param {any} revokee Whom to revoke from.
   *
   * @returns {undefined}
   * @memberOf SQLRevokerStrategy
   */
  handleGlobalRolesUsingGroup (roles, revokee) {
    this.tasks.push(this.client.revokeRolesUsingGroup(roles, revokee));
  }
  /**
   ...
   * @memberOf SQLRevokerStrategy
   */
  handleSchemaRoles (schema, roles, revokee, grantable, usingGroup) {
    if (usingGroup) {
      return this.handleSchemaRolesUsingGroup(schema, roles, revokee);
    }
    this.tasks.push(this.client.revokeSchemaRoles(schema, roles, revokee));
  }

  /**
   * Revoke schema roles from the revokee using group.
   *
   * @param {any} schema Schema the roles belong to.
   * @param {any} roles Roles to revoke.
   * @param {any} revokee Whom to revoke from.
   *
   * @returns {undefined}
   * @memberOf SQLRevokerStrategy
   */
  handleSchemaRolesUsingGroup (schema, roles, revokee) {
    this.tasks.push(this.client.revokeSchemaRolesUsingGroup(schema, roles, revokee));
  }
}
...
class HDIContainerRevokerStrategy {
  ...
  /**
   * Revoke schema roles from the revokee using group.
   * Emits a warning since using_group is not supported for HDI container revokers.
   *
   * @returns {undefined}
   * @memberOf HDIContainerRevokerStrategy
   */
  handleSchemaRolesUsingGroup () {
    logger.warn('using_group is not supported for HDI container revoker; revoke will proceed without USING GROUP');
  }
}
...
class ProcedureRevokerStrategy {
  ...
  /**
   * Revoke global roles from the revokee using group.
   * Falls back to plain revoke with a warning since using_group is not supported for procedure revokers.
   *
   * @param {any} roles Roles to revoke.
   * @param {any} revokee Who to revoke from.
   *
   ...
   */
  handleGlobalRolesUsingGroup (roles, revokee) {
    logger.warn('using_group is not supported for procedure revoker; revoke will proceed without USING GROUP');
    this.handleGlobalRoles(roles, revokee, false, false);
  }
  ...
  handleSchemaRolesUsingGroup (schema, roles, revokee) {
    logger.warn('using_group is not supported for procedure revoker; revoke will proceed without USING GROUP');
    this.handleSchemaRoles(schema, roles, revokee, false, false);
  }
}
