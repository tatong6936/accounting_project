...
const skipToken = (token, cqn) => {
  ...
  // REVISIT: can't we pass instead of fetching from cds.context?
  const query = cds.context?.http.req.query || {}

  // --- Handle simple numeric skiptoken
  if (!isNaN(token)) {
  ...
  // --- Handle pattern: $rows=..&$offset=..
  const params = new URLSearchParams(Buffer.from(token, 'base64').toString())
  if (params.has('$rows') && params.has('$offset')) {
  ...
  // --- Handle reliable pagination (base64 JSON)
  let decoded
  try {
