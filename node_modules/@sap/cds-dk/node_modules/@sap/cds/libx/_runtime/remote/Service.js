...
const { getCloudSdk, getCloudSdkResilience, shouldUseCloudSdk } = require('./utils/cloudSdkProvider')
...
class RemoteService extends cds.Service {
  init() {
    ...
    this.on('*', async function on_handler(req, next) {
      ...
      // clear keys from payload as some remote services don't like when they're there
      if (req.event === 'UPDATE' && req.target?.keys) for (const k of Object.keys(req.target.keys)) delete req.data[k]
      ...
      const requestConfig = extractRequestConfig(req, this)
  ...
  async handle(req) {
    if (this._requires_resolving(req)) {
      ...
      return this.dispatch(_req).then(result => postProcess(_req, result, this, true))
    }

    // we need to post process if alias was explicitly set in query
    const post_process = _isSelectWithAliasedColumns(req.query)
    return super.handle(req).then(result => (post_process ? postProcess(req, result, this, true) : result))
  }
}
