...
const ValidationError = Object.create (Error.prototype, {
...
class Validation {
  ...
  error (code, path, leaf, message, args) {
    const err = Object.create (ValidationError); (this.errors ??= []).push(err)
    err.code = code
    if (typeof message === 'string') err.message = message
    if (args) err.args = args
    if (path) {
      if (leaf) path = [ ...path, leaf ]
      if (this.options.path) path = [ this.options.path, ...path ] // e.g. used to prefix 'in/' for actions
      err.target = path.reduce((p,n) => this._targetFromPath(p,n),'')
      // err.path = path.map (p => p?.index || p)
    }
    return err
  }
  ...
  unknown (e,d,input, path) {
    ...
    cds.error (400, `Property "${e}" does not exist in ${target}`, {target}) 
  }
}
...
const $any = class any {
  ...
  check_asserts (val, path, /** @type {Validation} */ ctx) {
    ...
    const check_asserts = this.own('_check_asserts', () => {
      ...
      if (type_check) {
        asserts.push ((v,p, /** @type {Validation} */ ctx) => v == null || type_check(v) || ctx.error ('ASSERT_DATA_TYPE', p, this.name, null, [ v, this.toString() ]))
      }
      if (this._is_mandatory()) {
        asserts.push ((v,p, /** @type {Validation} */ ctx) => v != null && v.trim?.() !== '' || ctx.error ('ASSERT_MANDATORY', p, this.name, this['@mandatory.message'] || this['@mandatory'], [v]))
      }
      if (this['@assert.format']) {
        const format = new RegExp(this['@assert.format'],'u')
        asserts.push ((v,p, /** @type {Validation} */ ctx) => v == null || format.test(v) || ctx.error ('ASSERT_FORMAT', p, this.name, this['@assert.format.message'], [ v, this['@assert.format'] ]))
      }
      if (this['@assert.range'] && !this.enum) {
        ...
        asserts.push (
          min.val !== undefined && max.val !== undefined ? (v,p, /** @type {Validation} */ ctx) => v == null || min.val < v && v < max.val || ctx.error ('ASSERT_RANGE', p, this.name, this['@assert.range.message'], [ v, '>'+min.val, '<'+max.val ]) :
          min.val !== undefined ? (v,p, /** @type {Validation} */ ctx) => v == null || min.val < v && v <= max || ctx.error ('ASSERT_RANGE', p, this.name, this['@assert.range.message'], [ v, '>'+min.val, max ]) :
          max.val !== undefined ? (v,p, /** @type {Validation} */ ctx) => v == null || min <= v && v < max.val || ctx.error ('ASSERT_RANGE', p, this.name, this['@assert.range.message'], [ v, min, '<'+max.val ]) :
          (v,p, /** @type {Validation} */ ctx) => v == null || min <= v && v <= max || ctx.error ('ASSERT_RANGE', p, this.name, this['@assert.range.message'], [ v, min, max ])
        )
      }
      if (this['@assert.enum'] || this['@assert.range'] && this.enum) {
        ...
        asserts.push ((v,p, /** @type {Validation} */ ctx) => v == null || v in enums || vals.some(x => x == v) || ctx.error ('ASSERT_ENUM', p, this.name, this['@assert.enum.message'], [ typeof v === 'string' ? `"${v}"` : v, vals.join(', ') ]))
      }
...
class Composition extends entity {
  validate (data, path, ctx) { if (!data) return
    if (this.is2many && !Array.isArray(data)) return ctx.error ('ASSERT_DATA_TYPE', path, this.name, null, [ data, 'array of ' + this._target.name ])
