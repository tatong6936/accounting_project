const CoreService = require('./srv-dispatch')
 ...
 */
class ConsumptionAPI extends CoreService {
  ...
  schedule (...args) {
    const req = _req4 (...args), { ms4, cron } = cds.utils;
    (req.queue ??= {}).task = req.event
    return {
      after (t,u) { (req.queue ??= {}).after = ms4(t,u);            return this },
      every (t,u) { (req.queue ??= {}).every = cron(t) ?? ms4(t,u); return this },
      as    (t)   { (req.queue ??= {}).task  = t ?? req.event;      return this },
      then: (r,e) => cds.queued(this).send(req).then(r,e)
    }
  }
  unschedule (task) { return cds.queued(this)._unschedule(task) }
...
class ReflectionAPI extends ConsumptionAPI {
  ...
  get entities()   {
    const filter = cds.env.features.compat_texts_entities ? d => d.kind === 'entity' : d => d.kind === 'entity' && !d.name.endsWith('.texts')
    return super.entities = this.reflect('entities', filter)
  }
  get events()     {
    return super.events = this.reflect('events', d => d.kind === 'event')
  }
  get types()      {
    return super.types = this.reflect('types', d => !d.kind || d.kind === 'type')
  }
  get actions()    {
    return super.actions = this.reflect('actions', d => d.kind === 'action' || d.kind === 'function')
  }
  reflect (property, filter) {
    if (typeof property === 'function') filter = property
    return this.model?.childrenOf (this.namespace, filter) || []
  }
}

// REVISIT: remove opt-in for deprecated function API with cds^11
if (cds.env.features.compat_srv_getters) ReflectionAPI.prototype.reflect = function (property, filter) {
  const children = this.model?.childrenOf (this.namespace, filter) || []
  const fn = ns => !ns || ns === this.namespace ? children : this.model[property]?.(ns) || {}
  const fx = cds.utils.deprecated (fn, {
    kind: 'property',
    old: `srv.${property}()`,
    use: property === 'entities' ? `cds.entities()` : undefined
  })
  Object.setPrototypeOf (fx, children)
  Object.assign (fx, children)
  return fx
}
...
class Service extends ReflectionAPI {
  ...
  prepend (fn) { this.handlers.prepend(this,fn); return this }
  ...
  reject (e, path) { return this.handlers.register (this, 'before', e, path,
  ...
  get resolve() {
    return super.resolve = new Resolver(this)
  }
}
...
const Resolver = require('./srv-resolve')
