...
class Protocols {
  ...
  constructor (conf = cds.env.protocols||{}) {
    ...
    if (!('odata' in conf)) this['odata'] = this['odata-v4']
  ...
  /**
   * Constructs new adapters for a given service, and mounts them to the express app.
   */
  serve (srv, app, { before, after } = cds.middlewares) {
    ...
    if (app) {
      app.disable?.('etag') // we do our own etag handling in services, so disable express's default one, to avoid overhead
      app.disable?.('x-powered-by')
    }

    if (endpoints) for (let { kind, path } of endpoints) {

      /** Construct @type {import('./http')} adapter instances from resolved implementation  */
      ...
      /** Handle first one as default adapter */
      if (n++ === 0) {
      ...
      /** Note: we bind error handlers to adapter, so they can call {@link adapter.response4} */
      app.use (path, before, adapter.router, after.map(h => h.bind(adapter)))
    }
  }
