...
const _ensure_target = (srv,req) => {
  ...
  if (typeof req.query === 'object') {
    ...
    if (!q._srv) q.bind (srv) // ensure req.query is bound to the tx.ed service
    if (!req.target) {
      if (srv.namespace && q._subject.ref) _ensure_fqn (q._subject.ref,0, srv)
      req.target = cds.infer.target (q, srv.model)
    }
...
const _unhandled = (srv,req) => {
  ...
  cds.error (501, `Service "${srv.name}" has no handler for "${event}".`)
}
