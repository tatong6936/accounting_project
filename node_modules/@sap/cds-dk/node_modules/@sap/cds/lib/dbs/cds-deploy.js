...
deploy.data = async function (db, csn = db.model, o, srces, log=()=>{}) {
  ...
  function INSERT_from4 (db,m,o) {
    ...
    const use_upsert = cds.env.features.deploy_via_upserts
    const INSERT_into = schevo ? UPSERT.into : use_upsert ? entity => {
      const keys = m.definitions[entity.name||entity]?.keys
      if (!keys) return INSERT.into(entity)
      return {
        entries: records => {
          for (let k in keys) if (!(k in records[0]))
            return INSERT.into(entity).entries(records)
          return UPSERT.into(entity).entries(records)
        },
        columns: cols => ({ rows: rows => {
          for (let k in keys) if (!cols.includes(k))
            return INSERT.into(entity).columns(cols).rows(rows)
          return UPSERT.into(entity).columns(cols).rows(rows)
        }}),
        rows: rows => { for (let k in keys) if (!rows[0][k])
            return INSERT.into(entity).rows(rows)
          return UPSERT.into(entity).rows(rows)
        }
      }
    } : INSERT.into

    return (file) => ({
      '.yaml': { into (entity, yaml) {
        let records = cds.utils.yaml.parse(yaml); if (!records.length) return
...
deploy.prepare = async function (csn, srces) {
  ...
  for (const [file,e] of resEntries) {
    if (e === '*') {
      let init_js = await cds.utils.import (file)
