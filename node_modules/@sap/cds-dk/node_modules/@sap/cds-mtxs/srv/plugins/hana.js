...
if (db?.kind === 'hana' || db?.kind === 'hana-serverless') {
  const serverless = db.serverless || db.kind === 'hana-serverless' || (db.credentials?.url && db.credentials?.identity)
  if (!db.credentials?.sm_url && !db.credentials?.baseurl && !serverless) cds.error('No Service Manager credentials found. Make sure the application is bound to a BTP Service Manager instance.')

  const hana = require('./hana/hdi-ctnr-mgr').defaultInstance()
  ...
  cds.on ('serving:cds.xt.DeploymentService', ds => {
    ...
    ds.prepend(() => {
      ...
      ds.on ('tenantsByDb', async req => {
        ...
        const tenants = req.data.tenants ?? [...tenantsInT0]
        const hana = require('./hana/hdi-ctnr-mgr').defaultInstance() // TODO tms more generic handling -> new functions hana.toTenantId + hana.toDbId
...
function _hdiDeployParams(tenant, params = {}) {
  ...
  return _merge(defaults, paramsFromEnv, paramsFromTenantOptions, params?.hdi?.deploy)
}
...
async function resources4(out) {
  const cache = out?.endsWith('gen' + path.sep + 'base')
  if (cache && resources.has(out)) return resources.get(out)
  const _resources4 = async (out) => {
    const { 'cds.xt.ModelProviderService': mp } = cds.services
    try {
  ...
  if (cache) {
    const p = _resources4(out).catch(e => { resources.delete(out); throw e })
    resources.set(out, p)
    return p
  }
  return _resources4(out)
}
...
async function _deploy (req, _container, { skipExt = false, skipResources = false } = {}) {
  ...
  if (container?.credentials?.schema) req.data.schema = container.credentials.schema
  ...
  if (_csn) {
    ...
    if (csn) {
      if (csn.meta?.extended === false) {
        LOG.info('skipping HANA build for unextended model', { tenant })
      } else try {
  ...
  try {
    ...
    if (cacheBindings) {
      ...
      let driver
      try { require.resolve('hdb'); driver = 'hdb' } catch { /* continue */ }
      if (!driver) try { require.resolve('@sap/hana-client'); driver = '@sap/hana-client' } catch { /* continue */ }
      if (!driver) cds.error('No HANA client driver found. Add hdb or @sap/hana-client to your sidecar dependencies.')
