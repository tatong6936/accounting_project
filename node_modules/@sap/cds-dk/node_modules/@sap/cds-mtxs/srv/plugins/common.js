...
cds.on('serving:cds.xt.DeploymentService', ds => {
  ...
  const _getVersion = () => JSON.stringify({ version: require(cds.root + '/package.json').version })
  ...
  ds.after('subscribe', async (_, req) => {
    ...
    try {
      ...
      await t0_(INSERT.into(Tenants, { ID: tenant, metadata: JSON.stringify(metadata), ...(schema ? { schema } : {}), version }))
    } catch (e) {
  ...
  ds.prepend(() => {
    ds.on('getTenants', async req => {
      const { options } = req.data ?? {}
      const skipAfter = options?.skipAfter
      const query = skipAfter
        ? SELECT.from(Tenants, tenant => { tenant.ID }).where('modifiedAt <', skipAfter)
        : SELECT.from(Tenants, tenant => { tenant.ID })
  ...
  cds.once('served', () => {
    const cfInstanceIndex = process.env.CF_INSTANCE_INDEX
    if (cfInstanceIndex && cfInstanceIndex !== '0') return
