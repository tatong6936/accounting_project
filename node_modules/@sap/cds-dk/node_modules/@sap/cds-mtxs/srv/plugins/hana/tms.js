...
class TMSClient extends AbstractContainerManagerClient {
  constructor(credentials) {
    ...
    // clusterSize > 1 not supported yet for TMS
    const {
      clusterSize
    } = {
    ...
    if (clusterSize && clusterSize > 1) {
      LOG.warn(`Cluster size of ${clusterSize} is not supported for TMS. Defaulting to 1.`)
    }

    cds.utils.merge(cds.env.requires, { multitenancy: { jobs: { clusterSize: 1 } } })
    cds.utils.merge(cds.env.requires, { 'cds.xt.SaasProvisioningService': { jobs: { clusterSize: 1 } } })
    cds.utils.merge(cds.env.requires, { 'cds.xt.SmsProvisioningService': { jobs: { clusterSize: 1 } } })
  }
   ...
   */
  async findHanaTenantContainerAndCredentials(btpTenantName, { failures = 0, retryUntil } = {}) { // TODO expanding credentials does not work currently, change if available again
  ...
  get = async (tenant, options = {}) => {
    const { invalidCredentials, retryUntil, disableCache } = options
    if (!tenant) throw new Error('Tenant name is required')
    if (disableCache || invalidCredentials || !this.credentialCache[tenant]) {
      const previousFailures = this.failures.has(tenant) ? this.failures.get(tenant).count : 0
      const { container, credentials, hanatenant } = await this.findHanaTenantContainerAndCredentials(tenant, { failures: previousFailures, retryUntil })
      ...
      if (invalidCredentials && JSON.stringify(credentials) === JSON.stringify(invalidCredentials)) {
        LOG.info('previously rejected credentials provided by TMS → re-fetching')
  ...
  create = async (btpTenantName, parameters) => {
    ...
    try {
      ...
      if (err.status === 404) {
        // No container for tenant tenant found -> no hana tenant
        if (createOptions.presetHanaTenantId) {
          try {
            ;({ eTag, data: hanatenant } = await this.checkCreationStatus({ hanaTenantId: createOptions.presetHanaTenantId, expand: true }))
          } catch (error) {
            if (error.status === 404) {
              DEBUG?.('No hana tenant found with provided hana tenant id, creating new tenant')
            } else {
              throw error
            }
          }
  ...
  _createOptions = (btpTenantName, params) => {
    ...
    return { hanaTenantId, presetHanaTenantId: hana_tenant_id, labels, tmsOptions: cleanOptions, cleanupHanaTenants: cleanup_hana_tenants }
  }
