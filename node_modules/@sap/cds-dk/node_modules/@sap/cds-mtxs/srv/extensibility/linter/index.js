...
const ProtectedNamespaceChecker = require('./protected-namespace')
...
module.exports.lint = (extCsn, fullCsn, env = cds.env, cloneExt = false) => {
  const conf = env.requires[EXT_SERVICE_NAME] || env.requires.extensibility || env.mtx || {}
  ...
  const messages = [
    ...new ProtectedNamespaceChecker().check(reflectedCsn, reflectedFullCsn, compileBaseDir, linter_options),
...
module.exports.configCopyFrom = (env, target = {}) => {
  const source = env.requires?.[EXT_SERVICE_NAME] ?? env.requires?.extensibility
  ...
  Object.keys(source)
    ...
    .forEach(key => (config[key] = source[key]))
