...
module.exports = class SmsProvisioningService extends require('./abstract-provisioning-service') {
    ...
    _parseHeaders(headers) {
        ...
        const callbackUrl = this._resolveCallbackUrl(status_callback, subscription_manager_url)
        return {
            callbackUrl,
    ...
    _validateCertificate(req) {
        ...
        function parseDN(dn) {
            // Node's X509Certificate.issuer/subject uses newline as RDN separator.
            // Each line is a single "attribute=value" pair. Multiple lines with the
            // same attribute (e.g. multiple OU= entries) are all collected into an array.
            const fields = {}
            for (const line of dn.split('\n')) {
                ...
                if (fields[attr]) fields[attr].push(val)
                else fields[attr] = [val]
            }
            return fields
        }

        function isValid(expected, fromCert) {
            const certFields = parseDN(fromCert)
            return Object.entries(JSON.parse(expected)).every(([k, v]) => {
                if (v === '*') return true
                const certVals = certFields[k.toUpperCase()] ?? []
                // For array expected values every element must be present in the cert.
                // For a scalar expected value the cert must contain that exact value.
                if (Array.isArray(v)) return v.every(allowed => certVals.includes(allowed))
                else return certVals.includes(v)
            })
        }
