...
const { worker4 } = require('./worker/model-worker-pool')
...
const _errorOnMissingExtensionTable = () => {
  return (cds.env.requires['cds.xt.ExtensibilityService']?.['error-on-missing-extension-table'] === true
    || main.env.requires.extensibility?.['error-on-missing-extension-table'] === true
  )
}
...
module.exports = class ModelProviderService extends cds.ApplicationService {
  ...
  init() {
    ...
    // `service` and `flavor` are interpolated into a file system path in `_getPrebuiltBundle`.
    // Validate them here to prevent path traversal (e.g. reading arbitrary *.xml files).
    this.before('getEdmx', req => {
      const { service, flavor } = req.data
      const _valid = v => v == null || /^(?!.*\.\.)[\w-.]+$/.test(v)
      if (!_valid(service)) return req.reject(400, `Unsupported input service param ${service}`)
      if (!_valid(flavor)) return req.reject(400, `Unsupported input flavor param ${flavor}`)
    })
    ...
    async function _getCsn(req, checkExt) {
      ...
      let csn = await lru4(baseCache, JSON.stringify({ models, flavor, javaornode, skipMinify }), () =>
        worker4(path.join(__dirname, 'worker/model-worker.js'), { models, flavor, javaornode, skipMinify })
      )
      ...
      if (extensions) {
        ...
        csn = lru4(extensionCache, key, () => {
          try {
            ...
            if (!activated || e.code !== 'ERR_CDS_COMPILATION_FAILURE') throw e
            const message = _getCompilerError(e.messages)
            throw cds.error(`Extensions of tenant '${tenant}' are incompatible with the base model: ${message}`, { code: 'ERR_CDS_COMPILATION_FAILURE', status: 422, tenant, messages: e.messages })
          }
        })
      }
      ...
      if (csn) merge(csn, { meta: { extended: !!extensions } })
    ...
    async function _getExtensions4(tenant, appid = APPID, inactive = [], compat = false, skipExt = []) {
      ...
      try {
        ...
        if (!_errorOnMissingExtensionTable() && (error.cause?.status === 404 || error.status === 404 || dbErrorCodes?.getErrorCode(error) === dbErrorCodes.TABLE_NOT_FOUND
          || error.name === 'TimeoutError')) { // remove with cds 10
    ...
    async function getExtCsvs(req) {
      if (cds.env.requires['cds.xt.ExtensibilityService']?.activate?.['skip-csv'] ?? main.env.requires.extensibility?.activate?.['skip-csv']) return {}
