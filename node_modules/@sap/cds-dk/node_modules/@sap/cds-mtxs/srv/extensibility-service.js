...
const { token: authToken, authUrl } = require('../lib/utils')
...
const conf = () => ({ ...main.env.requires.extensibility, ...cds.env.requires['cds.xt.ExtensibilityService'] })
...
const _includeExt = () => {
  return conf()['check-existing-extensions'] !== false
}
class DbAdapter {
  ...
  static async put(ext) {
    ...
    if (ext.csvs && !ext.archive) {
      ...
      try {
        ...
        for (const csv in ext.csvs) {
          // defense-in-depth: 'csv' is user-supplied and must not escape 'root'
          const target = path.join(root, csv)
          if (path.relative(root, target).match(/^\.\.($|[/\\])/) || path.isAbsolute(path.relative(root, target))) {
            throw Error(`Invalid file name: "${csv}"`)
          }
          await write(target, ext.csvs[csv])
        }
    ...
    if (ext.csvs && Object.keys(ext.csvs).length > 0 && conf().activate?.['skip-csv']) throw Error('CSV deployment is disabled')
    ...
    await cds.tx({ tenant }, async tx => {
      ...
      // will break with missing appid
        await tx.run(INSERT.into('cds.xt.Extensions').entries({ ID, csn, i18n, sources: ext.archive ?? null, activated: this.activated(ext.status), tag, appid: ext.appid ?? APPID, timestamp: new Date().toISOString() }))
    })
    return ID
  }
...
module.exports = class ExtensibilityService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.after(['UPDATE', 'UPSERT'], 'Extensions', async (results, req) => { // TODO populate sources
      ...
      const res = await _fetchExtensions(req.data.ID, status)
    ...
    async function _fetchExtensions(ID, status) {
      try {
        return !ID ? await SELECT.from('cds.xt.Extensions') : await SELECT.one.from('cds.xt.Extensions').where(status != undefined ? { tag: ID, activated: DbAdapter.activated(status) } : { tag: ID })
      } catch (error) {
    ...
    cds.on('served', () => {
      cds.app?.post('/-/cds/login/grant', express.urlencoded({ extended: false }), async (req, res) => {
        try {
          ...
          // Endpoint accepts only passcode and refresh_token grant requests.
          const refresh_token = typeof query.refresh_token === 'string' ? query.refresh_token : undefined
          const passcode = typeof query.passcode === 'string' ? query.passcode : undefined
          if (!refresh_token && !passcode)
            throw cds.error('missing passcode or refresh_token', { status: 400 })

          const form = refresh_token ? { grant_type: 'refresh_token', refresh_token } :
                                       { grant_type: 'password', passcode }
      ...
      cds.app?.get('/-/cds/login/grant-metadata', authMeta)

      const redirectTo = to => (req, res) => res.redirect(308, to + req.url.slice(req.path.length))
      cds.app?.post('/-/cds/login/token', redirectTo('/-/cds/login/grant'))
      cds.app?.get('/-/cds/login/authorization-metadata', redirectTo('/-/cds/login/grant-metadata'))
    })
  ...
  async activateExtension(tenant, tag, status, options = {}) {
    ...
    try {
      ...
      await cds.tx({ tenant },async () => {
        ...
        if (status === ACTIVE && !(conf().activate?.['skip-db'] === true)) {
...
function _passcodeUrl(c, sub) {
  return authUrl(c.url, sub, '/passcode').toString()
}
