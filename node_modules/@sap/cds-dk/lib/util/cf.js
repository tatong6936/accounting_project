...
module.exports = new class CfUtil {
    ...
    async curl(urlPath, queryObj, bodyObj) {
        let url = this.#target.apiEndpoint + urlPath

        const headers = { 'Authorization': this.#target.token }
        ...
        if (!response.ok) {
            ...
            if (data?.errors?.[0]) {
                throw JSON.stringify(data.errors[0], null, 2)
            }
            throw `${response.status} ${response.statusText}: while accessing ${url}`
        }
        ...
        if (LOG._debug) {
            ...
            LOG.debug(`${method} ${url}\n${JSON.stringify({ headers: { ...headers, Authorization: 'bearer <token>' } }, null, 4)}\n${JSON.stringify(data, null, 4)}`)
        }
    ...
    async target() {
        if (!this.#target) {
            ...
            const orgObj = await this.org(org);
            if (!orgObj) {
                throw `CF org ${bold(org)} not found!`;
            }
            const orgGuid = orgObj.guid;

            const spaceObj = await this.space(orgGuid, space);
            if (!spaceObj) {
                throw `CF space ${bold(space)} not found in org ${bold(org)}!`;
            }

            const spaceGuid = spaceObj.guid;
    ...
    // CF V3 API treats commas in the 'names' filter as delimiters.
    // When a name contains a comma, omit it from the query and rely on client-side filtering.
    #safeNameQuery(query) {
        if (query.names?.includes(',')) {
            const rest = { ...query }
            delete rest.names
    ...
    async org(orgName) {
        const result = await this.curl('/v3/organizations', this.#safeNameQuery({ names: orgName }))
        return result?.resources?.find(r => r.name === orgName)
    }

    async space(orgGuid, spaceName) {
        const result = await this.curl('/v3/spaces', this.#safeNameQuery({ names: spaceName, organization_guids: orgGuid }))
        return result?.resources?.find(r => r.name === spaceName && r.relationships.organization.data.guid === orgGuid)
    }

    async instance(spaceGuid, instanceName) {
        const result = await this.curl('/v3/service_instances', this.#safeNameQuery({ names: instanceName, space_guids: spaceGuid }))
        return result?.resources?.find(r => r.name === instanceName && !!r.relationships.space.data.guid)
    }
