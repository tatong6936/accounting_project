...
function _captureAnnotationInCSN(targetString, annotationObject, csnDefsTargetObject) {
    annotationParser.captureAnnotationInCSN(targetString, annotationObject, csnDefsTargetObject);
}
...
function _parseAnnotationTerms(annotationTerm, annotationTermObject, csnDefsTargetObject) {
    // @Core needs special v4 handling for @Core.OptionalParameter (uses _updateElementObjectForDefaultValue)
    if (annotationTerm.startsWith('@Core.')) {
        _resolveCoreTerm(annotationTerm, annotationTermObject, csnDefsTargetObject);
    } else {
        annotationParser.parseAnnotationTerms(annotationTerm, annotationTermObject, csnDefsTargetObject);
    }
}

function _resolveAnnotationTerm(schemaDataAnnotation, parserContext) {
    return annotationParser.resolveAnnotationTerm(schemaDataAnnotation, parserContext);
}
...
function _parseAssociationProperty(foreignReference, associationNode, parserContext) {
    ...
    } else if (associationNode.$ReferentialConstraint) {
        associationProperty.on = []
        for (const constraint in associationNode.$ReferentialConstraint) {
            if (associationProperty.on.length) {
                associationProperty.on.push('and')
            }
            associationProperty.on.push(
                { ref: [foreignReference, associationNode.$ReferentialConstraint[constraint]] },
                '=',
                { ref: [constraint] }
            )
        }
...
function _parsePropertyContent(csnObj, elementObj, parserContext) {
    ...
    Object.keys(elementObj)
        .sort((ele1, ele2) => {
            if (elementObj[ele1]?.$Kind === elementObj[ele2]?.$Kind) {
                return 0
            } else if (elementObj[ele1]?.$Kind === 'NavigationProperty') {
                return 1
            } else if (elementObj[ele2]?.$Kind === 'NavigationProperty') {
                return -1
            }
            return 0
        })
...
function _generateCSN(edmxAsJson, ignorePersistenceSkip, parserContext, importerContext) {
    ...
    for (const [eName, entity] of Object.entries(csn.definitions)) {
        ...
        for (const comp of containingCompositions) {
            ...
            const entityKeys = Object.keys(entity.elements).filter(ele => entity.elements[ele].key)
            if (compTarget.elements[backLink]?.keys && !entityKeys.some(key => compTarget.elements[backLink + '_' + key])) {
                delete compTarget.elements[backLink].keys
            }

            // 3. View with parameters handling. In OData if the entity has a Composition Set with a backlink Parameters and every
            // other element is a key, its a View with Parameters. Import it as such to ensure proper outbound handling
            if (
                comp === 'Set' && backLink === 'Parameters' && 
                Object.keys(entity.elements).filter(e => entity.elements[e].key).length + 1 === Object.keys(entity.elements).length
            ) {
                const actualEntity = csn.definitions[entity.elements[comp].target];
                const actualEntityName = entity.elements[comp].target
                // Adjust entity so that the current elements are parameters, remove Set, remove key flags
                delete entity.elements[comp];
                entity.params = {}
                for (const ele in entity.elements) {
                    delete entity.elements[ele].key;
                    entity.params[ele] = entity.elements[ele];
                }
                delete entity.elements
                // Move compTarget out of service
                // Get rid of service prefix
                let nameWithoutService = eName.split(parserContext.primarySchema).pop();
                // Get rid of leading dot
                nameWithoutService = nameWithoutService.substring(1, nameWithoutService.length)
                csn.definitions[nameWithoutService] = actualEntity;
                delete csn.definitions[actualEntityName];
                delete csn.definitions[nameWithoutService].elements['Parameters'];
                // Adjust entity to * query from compTarget
                entity.query = {
                    SELECT: {
                        from: {
                            ref: [nameWithoutService]
                        },
                        columns: ["*"]
                    }
                }
