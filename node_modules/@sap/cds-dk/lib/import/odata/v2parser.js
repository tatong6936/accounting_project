...
function _getServiceEntityProperty(
	...
	if (length && length > 0) {
		...
		if (cdsDataType === undefined) {
			...
			// Additional fallback for Edm.Time with precision
			if (cdsDataType === undefined && dataType === 'Edm.Time') {
...
function _generateEDMXJSON2CSN(jsonObj, ignorePersistenceSkip, parserContext) {
	...
	// Parse edmx:Reference elements to build vocabNamespaceToAlias mapping
	const references = jsonObj["edmx:Edmx"]["edmx:Reference"];
	if (references) {
		const refArray = Array.isArray(references) ? references : [references];
		refArray.forEach(ref => {
			const include = ref["edmx:Include"];
			if (include) {
				const includeArray = Array.isArray(include) ? include : [include];
				includeArray.forEach(inc => {
					const namespace = inc._attributes?.Namespace;
					const alias = inc._attributes?.Alias;
					if (namespace && alias) {
						parserContext.vocabNamespaceToAlias[namespace] = alias;
					}
				});
			}
		});
	...
	try {
		...
		// Parse annotations after definitions are populated
		// Note: V2 annotations are opt-in (disabled by default for backward compatibility)
		// and require --odata-v2-annotations flag. V4 always imports annotations.
		const schemaAnnotations = jsonObj["edmx:Edmx"]["edmx:DataServices"].Schema.Annotations;
		if (schemaAnnotations && parserContext.annotations) {
			_parseV2SchemaAnnotations(schemaAnnotations, csn.definitions, parserContext);
		}
...
function _readAnnotationPrimitive(attributes) {
	if (attributes == null) return null;

	if (attributes.Bool !== undefined) return attributes.Bool === 'true';
	if (attributes.String !== undefined) return attributes.String;
	if (attributes.Int !== undefined) return Number(attributes.Int);
	if (attributes.Decimal !== undefined) return Number(attributes.Decimal);
	if (attributes.Path !== undefined) return { '=': attributes.Path };
	if (attributes.PropertyPath !== undefined) return { '=': attributes.PropertyPath };
	if (attributes.NavigationPropertyPath !== undefined) return { '=': attributes.NavigationPropertyPath };
	if (attributes.AnnotationPath !== undefined) return { '=': attributes.AnnotationPath };

	return null;
}

function _parseV2AnnotationNode(node) {
	if (node === null || node === undefined) return null;
	if (Array.isArray(node)) return node.map(item => _parseV2AnnotationNode(item));
	if (typeof node !== 'object') return node;

	if (node._attributes) {
		const primitiveValue = _readAnnotationPrimitive(node._attributes);
		const childKeys = Object.keys(node).filter(key => key !== '_attributes');
		if (primitiveValue !== null && childKeys.length === 0) {
			return primitiveValue;
		}
	}

	if (node.Collection) return _parseV2Collection(node.Collection);
	if (node.Record) return _parseV2Record(node.Record);

	const result = {};
	Object.keys(node).forEach(key => {
		if (key === '_attributes') return;
		if (key === 'PropertyValue') {
			_toArray(node[key]).forEach(propertyValue => {
				Object.assign(result, _parseV2PropertyValue(propertyValue));
			});
		} else if (key === 'Collection') {
			return _parseV2Collection(node[key]);
		} else if (key === 'Record') {
			Object.assign(result, _parseV2Record(node[key]));
		} else {
			const parsedValue = _parseV2AnnotationNode(node[key]);
	...
	return Object.keys(result).length ? result : _readAnnotationPrimitive(node._attributes);
}

function _parseV2PropertyValue(propertyValueNode) {
	const attributes = propertyValueNode._attributes || {};
	const propertyName = attributes.Property;
	const childKeys = Object.keys(propertyValueNode).filter(key => key !== '_attributes');
	let propertyValue = null;

	if (childKeys.length === 0) {
		propertyValue = _readAnnotationPrimitive(attributes);
	} else if (childKeys.length === 1) {
		const childKey = childKeys[0];
		if (childKey === 'Collection') {
			propertyValue = _parseV2Collection(propertyValueNode[childKey]);
		} else if (childKey === 'Record') {
			propertyValue = _parseV2Record(propertyValueNode[childKey]);
		} else {
			propertyValue = _parseV2AnnotationNode(propertyValueNode[childKey]);
		}
	} else {
		propertyValue = {};
		childKeys.forEach(childKey => {
			if (childKey === 'PropertyValue') {
				_toArray(propertyValueNode[childKey]).forEach(item => {
					Object.assign(propertyValue, _parseV2PropertyValue(item));
				});
			} else {
				propertyValue[childKey] = _parseV2AnnotationNode(propertyValueNode[childKey]);
			}
		});
	}
...
function _parseV2Record(recordNode) {
	if (recordNode === null || recordNode === undefined) return null;
	if (Array.isArray(recordNode)) return recordNode.map(item => _parseV2Record(item));

	const attributes = recordNode._attributes || {};
	const result = {};
	...
	Object.keys(recordNode).forEach(key => {
		if (key === '_attributes') return;
		if (key === 'PropertyValue') {
			_toArray(recordNode[key]).forEach(propertyValueNode => {
				Object.assign(result, _parseV2PropertyValue(propertyValueNode));
			});
		} else {
			result[key] = _parseV2AnnotationNode(recordNode[key]);
		}
	});
...
function _parseV2Collection(collectionNode) {
	if (collectionNode === null || collectionNode === undefined) return null;
	if (Array.isArray(collectionNode)) return collectionNode.map(item => _parseV2AnnotationNode(item));

	const values = [];
	Object.keys(collectionNode).forEach(key => {
		if (key === '_attributes') return;
		_toArray(collectionNode[key]).forEach(item => {
			values.push(_parseV2AnnotationNode(item));
		});
	});
...
function _parseV2AnnotationTerm(annotationNode) {
	const attributes = annotationNode._attributes || {};
	const annotationTerm = '@' + attributes.Term;
	let annotationTermObject = null;

	Object.keys(annotationNode).forEach(key => {
		if (key === '_attributes') return;
		annotationTermObject = _parseV2AnnotationNode(annotationNode[key]);
	});

	return [annotationTerm, annotationTermObject];
}

function _resolveV2AnnotationTarget(targetPath, csnDefs, parserContext) {
	const normalizedTargetPath = targetPath.replace(/\.$/, '');
	const targetSegments = normalizedTargetPath.split('/');
	const entityContainerPrefix = parserContext.primarySchema + '.EntityContainer';

	if (targetSegments.length > 1 && targetSegments[0] === entityContainerPrefix) {
		return csnDefs[parserContext.primarySchema + '.' + targetSegments[1]];
	}

	if (csnDefs[normalizedTargetPath]) {
		return csnDefs[normalizedTargetPath];
	}

	const directTarget = parserContext.primarySchema + '.' + targetSegments[targetSegments.length - 1];
	if (csnDefs[directTarget]) {
		return csnDefs[directTarget];
	}
}

function _parseV2SchemaAnnotations(schemaAnnotations, csnDefs, parserContext) {
	const parsedAnnotations = {};

	_toArray(schemaAnnotations).forEach(annotationsNode => {
		const targetPath = annotationsNode._attributes && annotationsNode._attributes.Target;
		if (!targetPath) return;

		if (!parsedAnnotations[targetPath]) {
			parsedAnnotations[targetPath] = {};
		}

		_toArray(annotationsNode.Annotation).forEach(annotationNode => {
			const [annotationTerm, annotationTermObject] = _parseV2AnnotationTerm(annotationNode);
			parsedAnnotations[targetPath][annotationTerm] = annotationTermObject;
		});
	});

	annotationParser.parseAnnotations(parsedAnnotations, csnDefs, parserContext, _resolveV2AnnotationTarget);
}
...
function getEdmxv2CSN(edmx2jsonModel, ignorePersistenceSkip, mockServerUc, context) {
	...
	// V2 annotations are opt-in (disabled by default for backward compatibility)
	// Controlled via --odata-v2-annotations CLI flag
	parserContext.annotations = !!context.odataV2Annotations;
	...
	if (isValid) {
		...
		parserContext.primarySchema = parserContext.serviceNamespace;
