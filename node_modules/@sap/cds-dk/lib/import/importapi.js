...
async function cds_import_from_edmx(filepath, options = { keepNamespace: false }) {
    ...
    let odataOptions = { include_namespaces: options.includeNamespaces, keep_namespace: options.keepNamespace, odataV2Annotations: options.odataV2Annotations};
    if (cds.env.import) {
 ...
 * cds11: remove once @cap-js/asyncapi is a required project dependency
 * @param {string} filepath
 * - a single valid filepath
 * @returns {object} CSN
 */
async function cds_import_from_asyncapi(filepath, options = {}) {
    filepath = await common.isValidInputFile(path.resolve(process.cwd(), filepath));
    const src = await common.getContentFromFile(filepath);
    let csn = require('@cap-js/asyncapi').import.asyncapi2csn(src);
