...
const KindToRequiresNameMap = {
    ...
    'hana-cloud': 'db',
    hana: 'db',
    'hana-mt': 'db', // cds10: remove, For compatibility only
    'sql-mt': 'db', // cds10: remove, For compatibility only
...
async function bind(options = {}) {
    ...
    for (const resolved of resolvedServices) {
        ...
        if (options.serviceArg) {
            ...
            if (!requiresName) throw `unknown CDS service name for service ${bold(instance)} of kind ${bold(kind)} – please specify as argument for ${bold('cds bind')}`
...
async function storeBindings(resolvedServices, options) {
    ...
    // just passing "--out" parses it to `true`, so we need to explicitly check for string type
    if (Object.hasOwn(options, 'out') && !(typeof options.out === 'string')) {
        throw '--out was specified without providing a path to an output file'
    }
