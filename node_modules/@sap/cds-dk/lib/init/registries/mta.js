...
const isJava = process.argv[process.argv.indexOf('--add') + 1].split(/[,\s+]/).includes('java') ||
               cds.env['project-nature'] === 'java' ||
               cds.cli.options?.add?.has('java') ||
               process.argv.includes('--java')
...
const ias = { in: 'resources', where: { 'parameters.service': 'identity' } }
...
const eventMesh = {
    in: 'resources',
    where: { type: 'org.cloudfoundry.managed-service', 'parameters.service': 'event-mesh-message-client' }
}
...
const requiredMtxSidecarApi4 = (srv, name) => ({
    in: [srv, 'requires'],
    where: { 'properties.CDS_MULTITENANCY_SIDECAR_URL': name }
})
...
const aicore = {
    in: 'resources',
    where: { type: 'org.cloudfoundry.managed-service', 'parameters.service': 'aicore' }
}
