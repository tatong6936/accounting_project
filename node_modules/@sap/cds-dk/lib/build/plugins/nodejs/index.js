...
module.exports = class NodejsBuildPlugin extends require('../edmx') {
    ...
    // Override to pass model for neighborhood resolution of plugin i18n files.
    // Cannot be done in InternalBuildPlugin because Java relies on the model-less behaviour.
    async collectMessageBundles(bundleDest, model) {
        ...
        for (const [locale, messages] of Object.entries(messagesData)) {
            if (Object.keys(messages).length > 0) {
                ...
                const content = Object.entries(messages).map(([k, v]) => `${k}=${v}`).join('\n') + '\n'
