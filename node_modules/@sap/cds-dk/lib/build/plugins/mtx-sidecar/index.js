...
const { BuildError, resolveRequiredSapModels, getI18nDefaultFolder } = require('../../util')
...
module.exports = class MtxSidecarBuildPlugin extends require('../nodejs') {
    ...
    async _buildSidecarApp(sidecarEnv) {
        ...
        // copy .npmrc from project root, if no .npmrc exists in the sidecar
        // (check src, not dest: copySrvContent runs in parallel here)
        if (!exists(path.join(src, '.npmrc')) && exists('.npmrc')) {
    ...
    async _buildMainApp(sidecarEnv) {
        ...
        // copy package.json and .cdsrc.json/.cdsrc.yaml from project root
        for (const file of ['package.json', '.cdsrc.json', '.cdsrc.yaml']) {
            if (exists(file)) {
                promises.push(this
                    .copy(path.join(cds.root, file))
                    .to(path.join(destMain, file)))
            }
        }
     ...
     */
     _compileSidecarSync(env) {
        ...
        const modelPaths = cds.resolve('*', { ...options, dry: true })
        const modelFilePaths = cds.resolve(modelPaths, options)
        ...
        DEBUG?.(`sidecar model: ${modelFilePaths.map(filePath => path.relative(cds.root, filePath)).join(", ")}`)
