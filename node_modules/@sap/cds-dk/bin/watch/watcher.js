...
module.exports = function watch(paths, { filter, ignore, hotReload } = {}) {
  ...
  if (useNativeRecursive) {
    // On Windows, ReadDirectoryChangesW fires FILE_NOTIFY_CHANGE events for mere
    // file opens (antivirus, search indexer, TypeScript loaders) without a write
    // having occurred, causing spurious server restarts.  Guard by confirming
    // mtime actually changed before emitting 'change'.  The cache is seeded on
    // the first event for each path so buffered pre-startup events are suppressed.
    // Mac uses kqueue/FSEvents which only fires on real writes — no guard needed.
    const _mtimes = process.platform === 'win32' ? new Map() : null
    const _watcherStart = _mtimes && Date.now()
    const _changedOnDisk = _mtimes
      ? async fullPath => {
        try {
          const { mtimeMs } = await fs.promises.stat(fullPath)
          const prev = _mtimes.get(fullPath)
          _mtimes.set(fullPath, mtimeMs)
          if (prev === undefined) return false          // first event: seed cache, suppress
          if (prev === mtimeMs) return false            // mtime unchanged: read-only open
          if (mtimeMs < _watcherStart) return false    // older than watcher: buffered OS event
          return true
        } catch (err) {
          _mtimes.delete(fullPath)
          if (err.code !== 'ENOENT' && err.code !== 'ENOTDIR') throw err
          return true  // stat failed (deleted file): let it through
        }
      }
      : () => true  // non-Windows: always emit
    ...
    const _watch = async root => {
      try {
        ...
        for await (const event of watcher) {
          ...
          } else if (_allow(fullPath) && _narrowJSONreloads(fullPath) && await _changedOnDisk(fullPath)) {
