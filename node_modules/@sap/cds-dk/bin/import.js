const { FILE, DIR } = require('./completion')
module.exports = Object.assign(_import, {
    _name: 'import',
    staticCompletions: () => ({
        '--from': ['asyncapi', 'edmx', 'openapi'],
        '--as':   ['cds', 'csn', 'json'],
        '--out':  DIR(),
        '-o':     DIR(),
        '_':      FILE('json,xml,edmx'),
    }),
    options:   [ '--services', '--for', '--out', '--as', '--name', '--include-namespaces', '--dependencies', '--from', '--config', '--destination', '--group', '--into-namespace' ],
    shortcuts: [ '-s',            '-4',    '-o',  '-as',     '-n',                  '-ns',             '-d',undefined,       '-c',       undefined, undefined,                '-2',        '-', '-f', '-b'],
    flags: [ '--dry', '--force', '--resolve-bindings', '--no-copy', '--no-save', '--keep-namespace', '--data-product', '--edmx', '--rfc', '--odata', '--openapi', '--asyncapi', '--odata-v2-annotations' ],
    help: `
# SYNOPSIS
...
# OPTIONS
    ...
    *--asyncapi*
    *--rfc*
        Imports the model from the one of the API formats OData EDMX, OpenAPI, AsyncAPI or RFC.
    ...
    *-2 | --into-namespace* <string>
        Specifies a custom namespace to be used for the imported service.
        This will override the namespace that was specified via the the
        @title annotation of the original service.
    ...
    *--odata-v2-annotations*
        Imports OData annotations into the CSN output. For OData V2, annotations
        are not imported by default and must be explicitly enabled with this flag.
        For OData V4, annotations are always imported.
...
# EXAMPLES
    ...
    cds import ~/Downloads/API_BUSINESS_PARTNER.edmx --dry
    ...
    cds import --openapi ~/Downloads/Petstore_OpenAPI.json
...
async function _import(args, options = {}) {
    ...
    if (options.from) {
        ...
        if (options.from && cds.import.options && cds.import.options[options.from]) {
            options = Object.assign(cds.import.options[options.from], options)
        }
