...
const mvn = require('../lib/init/mvn')
const term = require('../lib/util/term')
const { read, write: cdswrite, exists, mkdirp, local, path, fs } = cds.utils
const DEBUG = cds.debug('export')
...
class Export extends Minify {
  ...
  parseArgs (conf) {
    ...
    if (o.for === 'mvn') {
      if (!o.mvn)
        console.warn(term.warn(`\n Option --for mvn is deprecated. Use --mvn ${o.artifact || 'groupId:artifactId'} instead.\n`))
      const raw = o.artifact
      if (!raw || !raw.includes(':'))
        throw `\n Maven export requires a --mvn "groupId:artifactId" option.\n`
      const parts = raw.split(':')
      if (parts.length !== 2 || !parts[0] || !parts[1])
        throw `\n Invalid Maven artifact "${raw}". Expected format: "groupId:artifactId".\n`
      const [groupId, artifactId] = parts
      o.artifact = { groupId, artifactId }
    }
  ...
  async output (minified,o) {
    const isMvnExport = o.for == 'mvn'
    if (this.options.to === '-') return super.output (minified)
    if (this.options.to.endsWith('/*')) this.options.to = this.export_to()
    const rawModuleName = this.options.to
    if (isMvnExport) {
      const { groupId, artifactId } = o.artifact
      this.options.to = path.join(this.options.to, 'src', 'main', 'resources', 'cds', groupId, artifactId)
    }
    ...
    if (isMvnExport) {
      const pomPath = path.join(rawModuleName, 'pom.xml')
      if (exists(pomPath)) {
        DEBUG?.(`Skipping mvn.add: pom.xml already exists at ${local(pomPath)}`)
      } else {
        // Maven task modifies pom.xml and sets up the module — await separately
        const { groupId, artifactId } = o.artifact
        await mvn.add('cds_export', { 'to': rawModuleName, 'group-id': groupId, 'artifact-id': artifactId })
      }
    }
    console.log()
  }
  ...
  *export_texts() {
  ...
  async export_data (model) {
    const db = cds.db = await cds.deploy ('*',{silent:true}).to ('sqlite::memory:')
  ...
  /**
   * @param {Promise<string> | string | undefined} content
   */
  write (content) {
    return {
      to: async (/** @type {string}*/filename, force = true) => {
        if (!content) return
        const file = path.join (this.options.to, filename)
        if (!force && !this.options.force && exists(file)) return
        console.log (DIMMED,' >', local(file), RESET)
        return cdswrite(await content).to (file)
      }
    }
  }
}
