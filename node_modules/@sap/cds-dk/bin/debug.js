...
async function debug() {
  ...
  if (!appName) { // local app
    ...
    if (force) {
      ...
      const existingOpts = _execSyncSafe(`kubectl ${ns} get deploy ${appName} -o jsonpath="{.spec.template.spec.containers[0].env[?(@.name=='NODE_OPTIONS')].value}"`)
      const inspectFlag = `--inspect=0.0.0.0:${portLocal}`
      const stripped = existingOpts ? existingOpts.replace(/--inspect(-brk)?(=\S+)?\s*/g, '').trim() : ''
      const newOpts = stripped ? `${stripped} ${inspectFlag}` : inspectFlag
      _execSyncSafe(`kubectl ${ns} set env deployment/${appName} NODE_OPTIONS="${newOpts.replace(/"/g, '\\"')}"`, { stdio: 'inherit' })
    ...
    const hc = _execSyncSafe(`cf get-health-check ${appName}`)
    if (hc && /health check type is\s+http/i.test(hc)) {
      console.log()
      console.log(`${BOLD}Warning:${RESET} App '${appName}' uses an 'http' health check.`)
      console.log(`  While paused at a breakpoint, the event loop is frozen and the health endpoint`)
      console.log(`  cannot respond. Cloud Foundry will mark the container unhealthy and destroy it.`)
      console.log()
      console.log(`  To avoid this, switch the health check to 'process' for the debug session:`)
      console.log(`    ${BOLD}cf set-health-check ${appName} process${RESET}`)
      console.log(`    ${BOLD}cf restart ${appName}${RESET}`)
      console.log(`  Restore it afterwards with:`)
      console.log(`    ${BOLD}cf set-health-check ${appName} http${RESET}`)
      console.log()
      process.stdout.write(`  Press ${BOLD}Enter${RESET} to continue, or Ctrl+C to abort... `)
  ...
  async function openDevTools(port) {
    ...
    const clientHost = host === '0.0.0.0' || host === 'localhost' ? '127.0.0.1' : host
    try {
      switch (ide) {
        case 'chrome': {
          const res = await fetch(`http://${clientHost}:${port}/json/list`)
          const data = await res.json()
          if (!data?.length) throw 'No debugger found on port ' + port
          const wsPath = new URL(data[0].webSocketDebuggerUrl).pathname
          let command
          const url = `devtools://devtools/bundled/js_app.html?ws=${clientHost}:${port}${wsPath}`
  ...
  function _execSync(cmd, options={ encoding: 'utf-8', shell: true, maxBuffer: 10 * 1024 * 1024 }) {
    try {
