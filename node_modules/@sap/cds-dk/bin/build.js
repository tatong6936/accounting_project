const { DIR } = require('./completion');

module.exports = Object.assign(build, {
    staticCompletions: () => ({
        '--for':      ['hana', 'helm', 'java', 'mtx', 'mtx-extension', 'mtx-sidecar', 'nodejs'],
    ...
    shortcuts: ['-in', '-s', '-o', '-4'],
    flags: ['--no-clean', '--ws', '--ws-pack', '--no-ws-pack'],
    help: `
# SYNOPSIS
...
# OPTIONS
    ...
    *--no-ws-pack*
        ...
        Disable tarball based packaging of npm workspace dependencies.
...
async function build([project], options = {}) {
    ...
    if (options["no-ws-pack"] === "true" || options["no-ws-pack"] === true) {
        options["ws-pack"] = false
        delete options["no-ws-pack"]
    } else if (typeof options["ws-pack"] === 'undefined') {
        options["ws-pack"] = true
    }
    ...
    const { build, BuildError } = require('../lib/build')
    try {
...
function scanTaskOptionParams(optsParams) {
    ...
    quoteRegex.lastIndex = 0 // so we don't need to create a new regex every call since a constant would keep the match state
