...
const cli = {
  ...
  async exec (cmd = process.argv[2], ...argv) {
    ...
    try {
      ...
      if (task) {
        ...
        // For import: load plugins early to merge their default options
        // so if plugins need bindings it is handled by the _resolveBindings below
        if (cmd === 'import' && args[1]?.from) {
          await cds.plugins
          const pluginOpts = cds.import?.options?.[args[1].from]
          if (pluginOpts) for (const k in pluginOpts) {
            if (!(k in args[1])) args[1][k] = pluginOpts[k]
          }
          if (args[1].profile) {
            process.env.CDS_ENV = [...new Set([...process.env.CDS_ENV?.split(',') ?? [], ...args[1].profile.split(',')])]
          }
        }
  ...
  load (cmd,_default) {
    return /^(\.{1,2}\/|@|--)/.test(cmd) && _default && _default(true) // like `cds ./srv` or `cds @capire/bookshop` or `cds --flag`
    || /^-\d/.test(cmd) && _default && _default(true) // like `cds -2` (options with numbers)
    || /[\\/]/.test(cmd) && _default && _default(true) // like `cds b/c/d` or `cds c:\path` (paths with separators)
    || /\.[a-z]{1,5}$/i.test(cmd) && _default && _default(true) // like `cds model.cds` or `cds file.js`
...
async function _resolveBindings(options) {
  ...
  cds.env = cds.env.for('cds') // reload env (e.g. after profile activation)
  Object.assign(process.env, await bindingEnv({ ...options, env: cds.env }))
  cds.env = cds.env.for('cds') // reload env to pick up resolved bindings
...
function getCommands() {
  const excludeList = new Set(['cds.js', 'cds-ts.js', 'cds-tsx.js', 'fix-redirects.js']);
...
function _prepareTsIfNeeded(cds) {
  ...
  if (cds.cli?.command === 'serve' && fs.existsSync(tsconfig)) { // for `cds serve` started directly or through watch
    ...
    if (!process.env.CDS_TYPESCRIPT ) {
      const type = _moduleType(cds)
      const tsxLoader = type === 'module' ? 'tsx/esm' : 'tsx/cjs'
      try {
        cds._localOrGlobal(tsxLoader)
        DEBUG?.(`[cds] - Detected ${path.relative(cds.root, tsconfig)}. Running with ${tsxLoader}.\n`)
        process.env.CDS_TYPESCRIPT = 'tsx'
      }
      catch (e) {
...
function _moduleType(cds) {
  try {
    return require(path.join(cds.root??'.', 'package.json')).type
  } catch {
    return 'commonjs'
  }
}