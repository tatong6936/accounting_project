...
class WSDL {
    constructor(definition, uri, options) {
        ...
        this.valueKey = '$value';
        this.xmlKey = '$xml';
        this._orderIndexCache = new WeakMap();
        let fromFunc;
        this.uri = uri;
        this.callback = () => { };
        this._includesWsdl = [];
     ...
     * @param {String} type
     */
    objectToDocumentXML(name, params, nsPrefix, nsURI, type, options) {
    ...
    _getOrderIndexFor(schemaObject, schema) {
        ...
        const getLocalFromElement = (el) => {
            if (el.$name)
                return el.$name;
            if (el.$ref)
                return (0, utils_2.splitQName)(el.$ref).name;
            return null;
        };
        ...
        const resolveTypeNode = (qname, context = schema) => {
            if (!qname)
                return null;
            const q = (0, utils_2.splitQName)(qname);
            let ns;
            if (q.prefix === utils_2.TNS_PREFIX) {
        ...
        const walkBaseSequence = (node, seen = new Set()) => {
        ...
        const resolveGroupCompositor = (grp) => {
            if (!grp)
                return null;
            if (grp.$ref) {
                const q = (0, utils_2.splitQName)(grp.$ref);
                const ns = (schema && schema.xmlns && schema.xmlns[q.prefix]) ||
        ...
        const collectChoiceAlternatives = (choice) => {
            const out = [];
            const visit = (n) => {
                if (!n)
                    return;
    ...
    _orderElementKeys(obj, schemaObject, schema) {
        const keys = [];
        let i = 0;
        for (const k of Object.keys(obj)) {
            if (!Object.prototype.hasOwnProperty.call(obj, k))
                continue;
            if (k === this.options.attributesKey || k === this.options.xmlKey || k === this.options.valueKey)
                continue;
            const local = (0, utils_2.splitQName)(k).name;
        ...
        keys.sort((a, b) => {
            const ai = orderIndex.has(a.local) ? orderIndex.get(a.local) : Number.POSITIVE_INFINITY;
            const bi = orderIndex.has(b.local) ? orderIndex.get(b.local) : Number.POSITIVE_INFINITY;
            if (ai !== bi)
                return ai - bi;
            return a.idx - b.idx;
        });
