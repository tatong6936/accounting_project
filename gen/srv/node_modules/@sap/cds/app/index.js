...
const metadata = endpoint => odata(endpoint) ? `<span>/</span> <a href="${rel(endpoint.path)}/$metadata"><span>$metadata</span></a>` : ``
...
module.exports = { get html(){
  ...
  .map (({srv, endpoint}) => `
      <div id="${asHtmlId(srv.name)}-${endpoint.kind}">
        <h3 class="header">
          <a href="${rel(endpoint.path)}"><span>${endpoint.path}</span></a>${metadata(endpoint)} ${_moreLinks(srv, endpoint, undefined, false)}
        </h3>
        <ul>${_entities_in(srv).map (e => `
          <li id="${asHtmlId(srv.name)}-${endpoint.kind}-${asHtmlId(e)}">
            <div>
              <a href="${rel(endpoint.path)}/${e.replace(/\./g, '_')}"><span>${e}</span></a>
            </div>
        ...
        <ul>${_operations_in(srv).map (e => `
          <li id="${asHtmlId(srv.name)}-${endpoint.kind}-${asHtmlId(e.name)}" class="operation">
            <div>
              <a href="${rel(endpoint.path)}/${e.name}" title="${endpoint.path}/${e.name}"><span>${e.name} ${e.params}</span></a>
            </div>
          </li>`).join('')}
        </ul>
      </div>
