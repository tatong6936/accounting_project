...
const { getPreferReturnHeader } = require('../utils')
 ...
 * indicates whether this create handler is used for upsert, running after a prior update
 */
module.exports = (adapter, isUpsert) => {
  ...
  return function odata_create(req, res, next) {
    const {
      SELECT: { one, from }
    } = req._query
    ...
    return service
      .run(() => {
        return service.dispatch(cdsReq).then(result => {
          // If result is an Array with affected and the array does not only contain keys, its content was explicitely provided and shall be returned
          // If minimal requested and not etag, skip read after write
          const new_result = Array.isArray(result) && 'affected' in result
          let only_keys
          // REVISIT: remove cdsReq._.readAfterWrite with cds.features.legacy_srv_results in cds^11
          const read_after_write = cdsReq._.readAfterWrite || (new_result && (only_keys = _only_keys(target, result)))
          if (read_after_write && (target._etag || getPreferReturnHeader(req) !== 'minimal'))
            return _readAfterWrite(cdsReq)
          return new_result ? (!only_keys ? result[0] : cdsReq.data) : result
        })
      })
      .then(result => {
