  ...
  return async function $metadata(req, res, next) {
    ...
    if (cds.requires.extensibility || cds.requires.toggles) {
      ...
      try {
        ...
        if (!edmx) {
          // IMPORTANT
          // If MPS is served in process, only provide the extended model. If undefined, MPS has a shortcut to return the prebuilt edmx.
          // If MPS is a remote service, never provide the model. It potentially get's corrupted during serialization.
          const model = mps instanceof cds.ApplicationService ? cds.context.model : undefined
          edmx = await mps.getEdmx({ tenant, model, service: service.definition.name })
          metadataCache.edm = edmx
        }
        const extBundle = await mps.getI18n({ tenant, locale })
