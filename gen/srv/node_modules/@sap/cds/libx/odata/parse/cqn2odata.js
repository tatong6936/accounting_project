...
function _from(subject, kind, model) {
  let ref = getProp(subject, 'ref')
  ...
  for (const curRef of ref) {
    if (hasValidProps(curRef, 'where', 'id')) {
      // Format a request URL based on req.subject[n].ref

      const { where, id } = curRef
      queryTarget = model && _getQueryTarget(queryTarget, id, model)

      // Format URL keys from req.subject.ref[n].where
      let keys = ''

      if (Array.isArray(where) && where.length) {
        if (kind === 'rest') {
          if (where.length !== 1 && where[1] !== '=')
            cds.error(
              500,
              `Invalid subject: 'where' must specify a key via equality (key = value), got: ${JSON.stringify(where)}`
            )
          const val = where.length === 1 ? getProp(where[0], 'val') : getProp(where[2], 'val')
          keys = `/${val}`
        } else if (where.length === 3) {
          const [left, op, right] = where

          if (op === '=' && left.ref && 'val' in right) {
            keys = `(${encodeURIComponent(formatVal(right.val, left.ref.join('/'), queryTarget, kind))})`
            // REVISIT: keys in req.subject should already use the correct order
          } else if (op === '=' && right.ref && 'val' in left) {
            keys = `(${encodeURIComponent(formatVal(left.val, right.ref.join('/'), queryTarget, kind))})`
          } else {
            cds.error(
              500,
              `Invalid subject: 'where' must specify a key via equality (key = value), got: ${JSON.stringify(where)}`
          ...
          for (const cur of where) {
            if (hasValidProps(cur, 'ref')) {
              ...
              res.push(_format(cur, element, queryTarget, kind))
            } else if (cur === 'and') {
              res.push(',')
            } else {
              res.push(cur)
      ...
      path.push(`${id}${params}`)
      if (queryTarget?.params) path.push(`Set`)
...
function getOptions(cqnPart, url, kind, target, isCount) {
  ...
  for (const opt in cqnPart) {
    ...
    if (!hasValidProps(cqnPart, opt)) cds.error(500, `Feature not supported: SELECT statement with .${opt}`)
...
const _select = (cqn, subject, kind, model) => {
  const { url, queryTarget } = _from(subject, kind, model)
  ...
  return {
    method: 'GET',
    path: `${url}${isCount ? '/$count' : ''}${queryOptions ? `?${queryOptions}` : ''}`
  }
}

const _insert = (cqn, subject, kind, model) => {
  const INSERT = getProp(cqn, 'INSERT')
  const { url } = _from(subject, kind, model)
...
const _update = (cqn, subject, kind, model, method = 'PATCH') => {
  const { url } = _from(subject, kind, model)
  // TODO: support for .set as well
  const body = _copyData(getProp(cqn, 'UPDATE').data)
  return { method, path: url, body }
}

const _delete = (subject, kind, model) => {
  const { url } = _from(subject, kind, model)
  return { method: 'DELETE', path: url }
}

module.exports.cqn2odata = (cqn, { kind, model } = {}) => {
  let { query, subject, method } = cqn

  if (!subject) {
    query = cds.ql(cqn)
    subject = query._subject && { ref: query._subject.ref }
    method = undefined
  }

  if (query.SELECT) return _select(query, subject, kind, model)
  if (query.INSERT) return _insert(query, subject, kind, model)
  if (query.UPDATE) return _update(query, subject, kind, model, method)
  if (query.DELETE) return _delete(subject, kind, model)

  throw new Error('Unknown CQN object cannot be translated to URL: ' + JSON.stringify(query))
}
