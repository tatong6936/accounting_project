...
module.exports = cds.service.impl(function () {
  this.after(['INSERT', 'UPSERT', 'UPDATE'], async function (res, req) {
    ...
    if (cds.env.features.assert_touched_only !== false && IS_DRAFT_ENTITY && req.event === 'UPDATE')
      touched = Object.keys(req.data).filter(k => !(k in req.target.keys))
        ...
        for (const [entityName, serializedChanges] of Object.entries(changes)) {
          ...
          for (const row of results) {
            ...
            const failedAsserts = failedColumns.map(([element, message]) => {
              const error = { status: 400, message, target: element }

              // If error function was used in @assert expression -> use its output
              // Note: depending on DB, function result may be JavaScript object or JSON string
              if (typeof message === 'string') {
                if (message[0] !== '{' || message.startsWith('{i18n>')) return error //> it's a plain message string
                message = JSON.parse(message) //> it's an error function result
              }
              let { targets = [], ...details } = message
              if (targets.length) details.target = targets.shift()
              if (targets.length) details.additionalTargets = targets
              return Object.assign(error, details)
            })

            if (IS_DRAFT_ENTITY) {
              // FIXME: All of that shall move to lean draft!
              ...
              const newMessages = !touched
                ? failedAsserts
                : failedAsserts.filter(
                    a => touched.includes(a.target) || a.additionalTargets?.some(t => touched.includes(t))
                  )
