...
const _addToBeDeletedEntriesToResult = (results, entity, keys, newValues, oldValues, newBucketMap, metaCache) => {
  ...
  for (const oldEntry of oldValues) {
    // NOTE: diff_optimization should not be used!
...
const _skip = (entity, prop) => entity.elements[prop]._target['@cds.persistence.skip'] === true
...
const compareJson = (newValue, oldValue, entity, opts = {}) => {
  ...
  // NOTE: diff_optimization should not be used!
  if (cds.env.features.diff_optimization && oldValue && (!Array.isArray(oldValue) || oldValue.length)) {
