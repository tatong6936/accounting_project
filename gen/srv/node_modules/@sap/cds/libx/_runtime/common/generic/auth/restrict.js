...
async function enforce_auth(req) {
  ...
  const boundAction = req.target?.actions?.[req.event]
  if (
    cds.env.features.compat_static_auth ||
    req.event in { CREATE: 1, NEW: 1 } ||
    this.operations[req.event] ||
    // collection-bound actions have no instance to filter -> only a static where can apply
    (boundAction && isBoundToCollection(boundAction))
  ) {
