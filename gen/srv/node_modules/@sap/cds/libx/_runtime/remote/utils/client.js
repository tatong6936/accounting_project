...
const { getCloudSdk, getCloudSdkConnectivity, shouldUseCloudSdk } = require('./cloudSdkProvider')
const { executeHttpRequest: fetchExecuteHttpRequest } = require('./fetchClient')
...
const _executeHttpRequest = async ({ requestConfig, destination, destinationOptions, jwt }) => {
  ...
  if (shouldUseCloudSdk()) {
    ...
    // selectionStrategy is kept as a plain string in destinationOptions;
    // Cloud SDK needs the corresponding function — resolve it lazily here
    if (typeof destination.selectionStrategy === 'string') {
      const { DestinationSelectionStrategies } = getCloudSdkConnectivity()
      const strategy = DestinationSelectionStrategies[destination.selectionStrategy]
      if (typeof strategy !== 'function')
        cds.error`Unsupported destination selection strategy "${destination.selectionStrategy}".`
      destination = { ...destination, selectionStrategy: strategy }
    }
    if (LOG._debug) LOG.debug('Executing via @sap-cloud-sdk/http-client.')
    return executeHttpRequestWithOrigin(destination, sdkConfig, { fetchCsrfToken: false })
  }

  if (LOG._debug) LOG.debug('Executing via native fetch.')
  return fetchExecuteHttpRequest(destination, requestConfig)
}
...
module.exports.run = async (requestConfig, options) => {
  ...
  try {
    ...
    // for hcql, do not wrap error into a generic 502 error
    if (options.kind === 'hcql') {
      const { errors } = e.response.data
      if (errors) {
        const error =
          errors.length === 1 ? new cds.error({ ...errors[0], cause: errors[0] }) : new AggregateError(errors)
        error.message = error.message ? `Remote call failed with status code ${e.status}: ${error.message}` : e.message
        error.code = Number(error.code) || error.code || e.status
        throw error
      }
    }
  ...
  if (kind === 'hcql') {
    if (response.headers?.['content-type']?.includes('application/octet-stream')) return response.data
    let { data = [], affected, $count } = response.data
    if (affected !== undefined) data.affected = affected
    if ($count == null && response.headers['x-total-count']) $count = Number(response.headers['x-total-count'])
