...
class EMManagement {
  constructor({
    ...
    // NOTE: default to base path for SAP Event Mesh (enterprise-messaging)
    this.options.basePath ??= '/hub/rest/api/v1/management/messaging'
  ...
  async getQueue(queueName = this.queueName) {
    ...
    try {
      const res = await authorizedRequest({
        ...
        path: `${this.options.basePath}/queues/${encodeURIComponent(queueName)}`,
  ...
  async getQueues() {
    ...
    try {
      const res = await authorizedRequest({
        method: 'GET',
        uri: this.options.uri,
        path: `${this.options.basePath}/queues`,
        oa2: this.options.oa2,
        tokenStore: (this._tokenStore ??= {})
      })
      return res.body
  ...
  async getSubscriptions(queueName = this.queueName) {
    ...
    try {
      const res = await authorizedRequest({
        method: 'GET',
        uri: this.options.uri,
        path: `${this.options.basePath}/queues/${encodeURIComponent(queueName)}/subscriptions`,
        oa2: this.options.oa2,
  ...
  async createSubscription(topicPattern, queueName = this.queueName) {
    ...
    try {
      const res = await authorizedRequest({
        method: 'PUT',
        uri: this.options.uri,
        path: `${this.options.basePath}/queues/${encodeURIComponent(
          queueName
  ...
  async readinessCheck() {
    ...
    try {
      await authorizedRequest({
        method: 'GET',
        uri: this.options.uri,
        path: `${this.options.basePath}/readinessCheck`,
        oa2: this.options.oa2,
        tokenStore: (this._tokenStore ??= {})
      })
    } catch (e) {
