...
const prioritized = {
  ...
  '@cap-js/sqlite': true,   // to enable override by @cap-js/ai
...
exports.fetch = function (DEV = process.env.NODE_ENV !== 'production') {
  ...
  let plugins = JSON.parse(process.env.CDS_PLUGINS||'{}')
  ...
  for (const each in prioritized)
    if (each in plugins) plugins = Object.assign({ [each]: plugins[each] }, plugins)
...
exports.activate = async function (plugins = cds.env?.plugins || exports.fetch()) {
  ...
  async function _load (plugin, conf) {
    ...
    const module = await cds.utils._import (conf.impl)
