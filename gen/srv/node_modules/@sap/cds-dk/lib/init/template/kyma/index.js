...
const { exists, rimraf, copy, yaml, fs } = cds.utils
...
module.exports = class KymaTemplate extends require('../../plugin') {
  ...
  static hasInProduction() {
      if (exists('chart/Chart.yaml')) {
          const chartYaml = yaml.parse(fs.readFileSync(join(cds.root, 'chart', 'Chart.yaml'), 'utf8'))
  ...
  async combine(facet) {
    ...
    const {
      addHana, addHanaServerless, hasHanaServerless, addMultitenancy, addXsuaa, hasApprouter,
      addApprouter, addIas, addAms, hasXsuaa, hasMultitenancy, addWorkzoneStandard,
      addHtml5Repo, hasHtml5Repo, addAppFrontend
    } = readProject(facet)

    if (addHtml5Repo || addWorkzoneStandard || addApprouter && hasHtml5Repo) {
      if (addHtml5Repo) {
        await KymaTemplate.mergeDependency('service-instance', 'content-agent-service')
      ...
      if (hasApprouter && hasHtml5Repo) {
        await KymaTemplate.mergeDependency('service-instance', 'html5-apps-repo-runtime')
      }
    }
    if (addAppFrontend) {
      await KymaTemplate.mergeDependency('service-instance', 'content-agent-service')
      await KymaTemplate.mergeDependency('service-instance', 'app-front')
      await KymaTemplate.mergeDependency('content-deployment', 'html5-apps-deployer')
    }
    ...
    if (addHana || addHanaServerless || addMultitenancy) {
      if (hasHanaServerless) {
        await KymaTemplate.mergeDependency('service-instance', 'db')
      } else if (hasMultitenancy) {
  ...
  static async mergeDependency(name, alias) {
    ...
    await merge({
      dependencies: [{
        ...
        alias,
        version: '>0.0.0',
        ...(project.hasKymaUnifiedRuntime && { repository: 'https://int.repositories.cloud.sap/artifactory/virtual-unified-runtime-helm-dmz' })
      }]
