...
module.exports = class DataTemplate extends require('../../plugin') {
  ...
  options() {
    return {
      ...
      'composition-records': {
        type: 'number',
        help: 'The number of records to be created per to-many composition (default 2).'
      },
      'content-type': {
        type: 'string',
        short: 'c',
      ...
      'keys-only': {
        type: 'boolean',
        help: `Only fill key values into the generated CSV records, leaving all
  ...
  async run() {
    ...
    let compositionRecords = cds.cli.options['composition-records']
    if (typeof compositionRecords === 'string')  compositionRecords = parseInt(compositionRecords)
    ...
    await asJson(data, csn, records, {contentType, compositionRecords})
    ...
    if (contentType === 'csv') {
      ...
      const keysOnly = !!cds.cli.options['keys-only']
      if (keysOnly && headerOnly) console.log(`${dim('ignoring --keys-only, use --records to create entries')}`)
      await asCsv(data, csn, Object.assign({ headerOnly, keysOnly }, dataForParams))
    }
