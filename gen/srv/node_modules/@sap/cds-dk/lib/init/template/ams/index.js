...
module.exports = class AmsTemplate extends require('../../plugin') {
    ...
    async combine(facet) {
        ...
        const { addMta, addCfManifest, addKyma } = project
    ...
    async #combineWithMta(project) {
        const srvModule = srv4(project.srv.path)
        const amsPoliciesDeployer = {
            in: 'modules',
            where: { 'name': `${project.appName}-ams-policies-deployer` }
        }
        await merge(__dirname, 'files/mta.yaml.hbs').into('mta.yaml', { project, additions: [srvModule, amsPoliciesDeployer] })
    }
