...
module.exports = class HttpTemplate extends require('../../plugin') {
      ...
      for (const c of csn.childrenOf(s.name)) {
        if (c.name.match(filterStringAsRegex(filter))) {
          ...
          if (c.kind === 'entity') {
            ...
            const singleton = c['@odata.singleton'] || Object.keys(c).some(k => k.startsWith('@odata.singleton.')) // object form flattens to @odata.singleton.<sub>
            entities.push({
              name: c.name,
              readOnly: (c["@readonly"] || autoexposed || singleton) ?? false,
              insertOnly: c['@insertonly'] ?? false,
              noInsert: c['@Capabilities.InsertRestrictions.Insertable'] === false,
              noUpdate: c['@Capabilities.UpdateRestrictions.Updatable'] === false,
              noDelete: c['@Capabilities.DeleteRestrictions.Deletable'] === false,
          ...
          } else if (c.kind === 'action' || c.kind === 'function') {
            actions.push(c)
          }
        }
      }
  ...
  async #generateRequests(services, payload, csn) {
    ...
    for (const [serviceName, service] of services) {
      ...
      service.urlPaths.forEach((rawUrlPath) => {
        const urlPath = rawUrlPath.replace(/^\//, '') // avoid a double slash when the service path is '/'
        ...
        service.entities.forEach((e) => {
          ...
          const entitySet = entityNameSimple.replace(/\./g, '_') // OData flattens nested names, e.g. E.addr -> E_addr
          const nameSlug = uniqueName(slug(entitySet))
          const requestTitle = `\n### ${entityNameSimple}`

          // parameterized entities are addressed as E(p=…)/Set and are read-only
          if (e.params && Object.keys(e.params).length) {
            const args = Object.entries(e.params).map(([n, p]) => `${n}=${sampleParam(p)}`).join(',')
            const url = `{{server}}/${urlPath}${entitySet}(${args})/Set`
            requests.push(`${requestTitle}\n# @name ${nameSlug}_GET\nGET ${url}\n${headerAuth}\n`)
            return
          }

          const url = `{{server}}/${urlPath}${entitySet}`
          if (!e.insertOnly) requests.push(`${requestTitle}\n# @name ${nameSlug}_GET\nGET ${url}\n${headerAuth}\n`) // insert-only entities cannot be read
          if (e.readOnly) return

          const entityData = payload[`${e.name}`] ? payload[`${e.name}`][0] : null
          if (!entityData) return

          stripNonWritable(entityData, e.keys, e.elements)

          if (e.isDraft) {
            const keyNames = Object.keys(e.keys)
            const activeId = keyPredicate(e.keys, entityData, true)
            keyNames.forEach(k => delete entityData[k]) // draft keys are auto-generated, so drop them from the payload
            const postBody = JSON.stringify(entityData, null, 2)
            generateDraftRequests(requests, requestTitle, url, headers, headerAuth, postBody, keyNames, service)
            if (activeId) {
              const instance = `${url}${activeId.replace(/\)$/, ',IsActiveEntity=true)')}`
              generateBoundOps(requests, requestTitle, instance, headers, headerAuth, nameSlug, e.boundOps, serviceName)
            }
          }
          else {
            const postBody = JSON.stringify(entityData, null, 2)
            if (!e.noInsert) requests.push(`${requestTitle}\n# @name ${nameSlug}_POST\nPOST ${url}\n${headers}\n\n${postBody}\n`)
            const id = e.insertOnly ? undefined : keyPredicate(e.keys, entityData)
            if (id) {
              if (!e.noUpdate) requests.push(`${requestTitle}\n# @name ${nameSlug}_PATCH\nPATCH ${url}${id}\n${headers}\n\n${postBody}\n`)
              if (!e.noDelete) requests.push(`${requestTitle}\n# @name ${nameSlug}_DELETE\nDELETE ${url}${id}\n${headers}\n`)
              const parenId = keyPredicate(e.keys, entityData, true)
              generateBoundOps(requests, requestTitle, `${url}${parenId}`, headers, headerAuth, nameSlug, e.boundOps, serviceName)
            }
          }
        })
        ...
        service.actions.forEach((a) => {
          ...
          const actionSlug = uniqueName(slug(simpleNameAction))

          if (a.kind === 'function') {
            const args = Object.entries(a.params || {}).map(([n, p]) => `${n}=${literal(p.type, payload[a.name]?.[0]?.[n])}`).join(',')
            requests.push(`\n### ${simpleNameAction}\n# @name ${actionSlug}_GET\nGET ${url}(${args})\n${headerAuth}\n`)
            return
          }
    ...
    function generateDraftRequests(requests, title, url, headers, headerAuth, postBody, keys, service) {
      const nameDraftsCreate = slug(title+'_Draft_POST')
      const draftVar = k => keys.length > 1 ? `draftID_${k}` : 'draftID'
      const predicate = keys.map(k => `${k}={{${draftVar(k)}}}`).join(',') + ',IsActiveEntity=false'
      const bindings = keys.map(k => `@${draftVar(k)}={{${nameDraftsCreate}.response.body.$.${k}}}`).join('\n')
    ...
    // drop non-writable fields (readonly/computed/managed) from a request body, keeping keys for addressing
    function stripNonWritable(data, keys, elements) {
      const keyNames = new Set(Object.keys(keys || {}))
      for (const name of Object.keys(data)) {
        if (keyNames.has(name)) continue
        const el = elements?.[name]
        if (el && (el['@readonly'] || el['@Core.Computed'] || el['@cds.on.insert'] || el['@cds.on.update'])) delete data[name]
      }
    }

    // placeholder value for a parameterized-entity argument
    function sampleParam(p) {
      const t = p.type
      if (t === 'cds.String' || t === 'cds.UUID' || t === 'cds.Date' || t === 'cds.DateTime' || t === 'cds.Timestamp') return `''`
      if (t === 'cds.Boolean') return 'true'
      return '0'
    }

    // OData URL literal for a value of the given type
    function literal(type, value) {
      if (value === undefined || value === null) return 'null'
      return type === 'cds.String' ? `'${value}'` : `${value}`
    }

    // key predicate from scalar keys with a sample value; associations, structs, binary types and unseeded keys aren't addressable
    function keyPredicate(keys, data, paren) {
      const notAddressable = new Set(['cds.Association', 'cds.Composition', 'cds.Binary', 'cds.LargeBinary', 'cds.Vector'])
      const scalar = [...(keys || [])].filter(k => !k.is_struct && !notAddressable.has(k.type) && data[k.name] !== undefined)
      if (!scalar.length) return undefined
      const val = k => literal(k.type, data[k.name])
      if (paren || scalar.length > 1) return `(${scalar.map(k => `${k.name}=${val(k)}`).join(',')})`
      return `/${val(scalar[0])}`
    }

    // sample value for a bound-operation parameter
    function sampleValue(type) {
      if (type === 'cds.Boolean') return true
      if (type === 'cds.Integer' || type === 'cds.Int16' || type === 'cds.Int32' || type === 'cds.Int64' || type === 'cds.Decimal' || type === 'cds.Double') return 0
      return ''
    }

    // bound actions (POST body) and functions (GET with inline args) on a single instance
    function generateBoundOps(requests, title, instanceUrl, headers, headerAuth, nameSlug, boundOps, serviceName) {
      for (const [name, op] of Object.entries(boundOps || {})) {
        const params = op.params || {}
        if (op.kind === 'function') {
          const args = Object.entries(params).map(([n, p]) => `${n}=${literal(p.type, sampleValue(p.type))}`).join(',')
          requests.push(`${title} ${name}\n# @name ${nameSlug}_${slug(name)}_GET\nGET ${instanceUrl}/${serviceName}.${name}(${args})\n${headerAuth}\n`)
        } else {
          const body = {}
          for (const [n, p] of Object.entries(params)) body[n] = sampleValue(p.type)
          requests.push(`${title} ${name}\n# @name ${nameSlug}_${slug(name)}_POST\nPOST ${instanceUrl}/${serviceName}.${name}\n${headers}\n\n${JSON.stringify(body, null, 2)}\n`)
        }
      }
    }
  }
  ...
  async #getHostnameAndAuth(service, csn) {
    ...
    if (!this.approuterAuth) {
      ...
      const authUrl = new URL(`${certurl ?? url}/${suffix}`)
        ...
        const raw = await new Promise((resolve, reject) => {
          const req = https.request({
            ...
            res.on('end', () => {
              ...
              else reject(new Error(`Failed to fetch access token: ${res.statusCode}`))
            })
          })
          req.on('error', reject)
          req.end(data)
