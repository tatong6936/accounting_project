  ...
  deploy:
    ...
    steps:
      ...
      - uses: cap-js/cf-setup@v2
        with:
          api: {{#literal}}${{ vars.CF_API }}{{/literal}}
          org: {{#literal}}${{ vars.CF_ORG }}{{/literal}}
          space: {{#literal}}${{ vars.CF_SPACE }}{{/literal}}
          username: {{#literal}}${{ vars.CF_USERNAME }}{{/literal}}
          password: {{#literal}}${{ secrets.CF_PASSWORD }}{{/literal}}
      - run: npm install
      - run: npx cds up
