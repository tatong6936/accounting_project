...
const { fs: { promises: { cp } }, exists, mkdirp, path } = cds.utils
...
module.exports = class NodejsTemplate extends require('../../plugin') {
    ...
    async run() {
        ...
        if (cds.cli.command === 'init' || cds.cli.options.force) {
            ...
            await cp(path.join(__dirname, 'files/eslint.config.mjs'), 'eslint.config.mjs', { force: false })
            await cp(path.join(__dirname, 'files/.vscode'), '.vscode', { recursive: true, force: false })
        }
