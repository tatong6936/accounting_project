...
module.exports = class ApprouterTemplate extends require('../../plugin') {
    ...
    async combine(facet) {
        ...
        const {
            isNodejs, isJava,
            addIas, addXsuaa, hasXsuaa, hasIas, addMultitenancy, hasMTXRoute, hasMTXRouteJava,
            addMta, hasMta, hasMultitenancy,
            addKyma, hasKyma, srv, approuterPath
        } = project


        if (addMta || (addMultitenancy || addXsuaa || addIas) && hasMta) {
            const srvModule = srv4(srv.path)
            const apis = isJava && hasMultitenancy ? [requiredJavaApprouterApi4(srvModule), providedJavaApprouterApi] : []
            ...
            const modules = [srvModule, approuter]
            ...
            if (hasIas) {
                ...
                deletions.push({
                    item: { in: [approuter, 'requires'], where: { 'name': 'srv-api' } },
                    key: 'properties.name'
                })
            }
            await merge(__dirname, 'files/mta.yaml.hbs').into('mta.yaml', {
                with: project, additions, relationships, deletions,
            })
        }

        if(addKyma) {
          await KymaTemplate.mergeDependency('web-application', 'approuter')
        }
