...
module.exports = class IasTemplate extends require('../../plugin') {
    ...
    async combine(facet) {
        ...
        const { isJava, addMta, hasMta, addCfManifest, addKyma, addMultitenancy, hasApprouter, hasMultitenancy, srv } = project
        ...
        if (addMta || (hasApprouter || hasMultitenancy) && hasMta) {
            ...
            const srvIdentityBinding = { in: [srvModule, 'requires'], where: { 'parameters.config.app-identifier': 'srv' } }
            const srvCertUrlApi = { in: [srvModule, 'provides'], where: { 'name': 'srv-api' } }
            const additions = [srvModule, ias, srvIdentityBinding, srvCertUrlApi]
            const overwrites = []
