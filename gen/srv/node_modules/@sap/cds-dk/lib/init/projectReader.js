...
module.exports = new class ProjectReader {
     ...
     *
     * @param {string} [caller] The facet whose `combine()`/`run()` is requesting the project.
     *   Used to resolve `add<Facet>` accessors: a template asks "am I being added right now?".
     *   Omit when no facet context applies; `add<Facet>` then reflects only the CLI add list.
    ...
    readProject(caller) {
        ...
        if (this.isJava) env.build.target = '.'
        ...
        const UI_PACKAGES = ['@ui5/cli', 'react', 'vue', 'svelte', '@angular/core', 'vite']
        const isApp = name => {
            if (name.match(/^[._]/)) return false
            const dir = path.join(cds.root, cds.env.folders.app, name)
            if (exists(path.join(dir, 'webapp/manifest.json'))) return true
            if (exists(path.join(dir, 'index.html'))) return true
            const pJsonPath = path.join(dir, 'package.json')
            if (!exists(pJsonPath)) return false
            const { dependencies, devDependencies } = JSON.parse(fs.readFileSync(pJsonPath))
            const deps = { ...dependencies, ...devDependencies }
            return UI_PACKAGES.some(p => p in deps)
        }
        const reserved = {
            ...
            hasTyper: () => _hasTyper(),
            srv: () => {
                const path = join(env.build.target, env.folders.srv.replace(/\/$/, '')).replace(/\\/g, '/')
                return { path }
            },
            archiveName: () => {
            ...
            apps: () => {
                ...
                return apps.map((app, i) => {
                    ...
                    const appPkgPath = path.resolve(cds.root, cds.env.folders.app, app, 'package.json')
                    const appPkg = exists(appPkgPath) ? JSON.parse(fs.readFileSync(appPkgPath, 'utf-8')) : {}
                    const isStatic = !appPkg.scripts?.build
                    ...
                    return { app, strippedApp: app.replace(/-/g, ''), vizId, isNotLast: i < apps.length - 1, manifestAppId: manifest?.['sap.app']?.id ?? app, archiveName, isStatic }
                })
            },
            appUIPaths: () => {
                const appPath = path.resolve(cds.root, cds.env.folders.app)
            ...
            approuterPath: () => (_uiEmbedded() ? join(env.folders.app) : exists(join(env.folders.app, 'router')) ? join(env.folders.app, 'router') : join('.deploy', 'app-router')).replace(/\\/g, '/'),
            html5DeployerPath: () => (exists(join(env.folders.app, 'html5-deployer')) ? join(env.folders.app, 'html5-deployer') : join('.deploy', 'html5-deployer')).replace(/\\/g, '/'),
            appName: () => appName,
            ...
            hasMTXRouteJava: () => _inProd('kyma') && _isJava() && _inProd('multitenancy'),
            hasKymaUnifiedRuntime: () => {
                if (cds.cli.options['unified-runtime']) return true
                const chartPath = join(cds.root, 'chart', 'Chart.yaml')
                if (!exists(chartPath)) return false
                const chartYaml = cds.load.yaml(chartPath)
                return chartYaml?.dependencies?.some(d => d.repository?.includes('unified-runtime')) ?? false
            },
            ...
            const _get = property => {
                ...
                for (const prefix of ['has', 'is', 'add']) if (p.startsWith(prefix) && p.length > prefix.length) {
                    ...
                    if (prefix === 'add') {
                        ...
                        return inProd && (adding(facet) || (caller && adding(caller)))
                    }
                    return inProd
                }
            }
...
module.exports._readSpringBootConfig = async function (roots = [cds.env.folders.srv]) {
    ...
    for (let r of rootDirs) {
        ...
        if (file) {
            const docs = YAML.parseAllDocuments(fs.readFileSync(file, 'utf-8'))
            for (let yamlDoc of docs.map(d => d.toJS())) {
                let cds = yamlDoc?.cds;
                if (!cds) continue
 ...
 * @property {string} srv Service configuration.
 ...
 * @property {string} html5DeployerPath The path to the HTML5 application content deployer folder.
 ...
 * @property {boolean} hasKymaUnifiedRuntime Indicates if the Kyma deployment targets the SAP Unified Runtime.
