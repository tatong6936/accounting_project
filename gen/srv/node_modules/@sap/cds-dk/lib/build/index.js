...
const { BuildError, BuildMessage, resolveRequiredSapModels, normalizePath, getDefaultModelPaths, hasOptionValue } = require('./util')
...
async function executeCleanBuildTasks(plugins, runner) {
    ...
    const results = await Promise.allSettled(plugins.map((plugin) => {
        DEBUG?.(`cleaning, plugin [${plugin.constructor.name}], src [${path.relative(cds.root, plugin.task.src)}]`)
        return plugin.clean()
    }))
...
async function executePipeline(pipeline, options) {
    ...
    for (const group of pipeline) {
        const results = await Promise.allSettled(group.map((plugin) => {
            DEBUG?.(`building, plugin [${plugin.constructor.name}], src [${path.relative(cds.root, plugin.task.src)}]`)
