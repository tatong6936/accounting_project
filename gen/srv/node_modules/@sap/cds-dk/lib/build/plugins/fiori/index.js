...
module.exports = class FioriBuildPlugin extends require('../edmx') {
    ...
    async _ensureCachesInitialized() {
        ...
        try {
            for (let [appFolder, appModelGroup] of appModelGroups.entries()) {
                ...
                DEBUG?.(`model: ${modelPaths.map(modelPath => path.relative(cds.root, modelPath)).join(", ")}`)
    ...
    async _writeEdmxToWebapp(src, dest) {
        const subfolder = this._resolveAppFolder(src)
        const manifestPath = path.join(src, subfolder, 'manifest.json')
        let manifest

        try {
            manifest = require(manifestPath)
        } catch {
            console.log(`UI module does not contain a manifest.json [${path.relative(cds.root, manifestPath)}], skipping build`)
            return
        }
        ...
        if (!mainService) {
            ...
            DEBUG?.(`UI module does not have a datasource [mainService], [${path.relative(cds.root, manifestPath)}], skipping build`)
            return
        }
        ...
        if (!localUri || !uri) {
            DEBUG?.(`local uri setting missing for data source [mainService], [${path.relative(cds.root, manifestPath)}]`)
            return
        }
        ...
        const edmxPath = path.resolve(path.join(dest, subfolder), this._strippedUrlPath(localUri))
        return this.write(edmx).to(edmxPath)
    }

    /**
     * Folder holding manifest.json relative to the UI module root.
     * Defaults: `webapp` (UI5 v4 application), `src` (UI5 v5 component/library);
     * overridable via ui5.yaml path mapping, see https://ui5.github.io/cli/v4/pages/Configuration/#path-mapping
     */
    _resolveAppFolder(src) {
        for (const yamlFile of ['ui5.yaml', 'ui5-build.yaml']) {
            const yamlPath = path.join(src, yamlFile)
            if (!exists(yamlPath)) {
                continue
            }
            let paths
            try {
                paths = cds.load.yaml(yamlPath)?.resources?.configuration?.paths
            } catch (e) {
                if (e.name !== 'YAMLParseError') throw e // only tolerate malformed yaml
                DEBUG?.(`ignoring malformed [${path.relative(cds.root, yamlPath)}]: ${e.code}`)
                continue
            }
            const mapped = paths?.webapp ?? paths?.src
            if (mapped) {
                return mapped
            }
        }
        return exists(path.join(src, 'src', 'manifest.json')) ? 'src' : 'webapp'
    }
