...
const { hasOptionValue, BuildMessage, filterFeaturePaths } = require('../util')
...
module.exports = class Plugin {
        ...
        return {
            to: async (/** @type {string} */ dest) => {
                ...
                if (cds.utils.isdir(src)) {
                    ...
                    this.pushFile(dest)
                    return fs.promises.cp(src, dest, { recursive: true, dereference: true })
                }
            }
        }
    }
    ...
    async model() {
        ...
        DEBUG?.(`model: ${files.map(file => path.relative(cds.root, file)).join(", ")}`)
