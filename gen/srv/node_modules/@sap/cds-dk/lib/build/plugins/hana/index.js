...
const { BuildError, BuildMessage, hasOptionValue, getWorkspacePaths, normalizePath } = require('../../util')
const { CONTENT_PACKAGE_JSON, CONTENT_HDBTABLEDATA, CSV_FILE_DETECTION, CONTENT_ENV, CONTENT_DEFAULT_ENV_JSON, CONTENT_NODE_MODULES, CONTINUE_UNRESOLVED_SCHEMA_CHANGES, CSV_FILE_TARGET, OPTION_WS, SEVERITY_WARNING } = require('../../constants')
...
const FILE_EXT_HDBSYNONYMCONFIG = ".hdbsynonymconfig"
...
// Some HDI artifacts do not require explicit plugin specification in _.hdiconfig_.
// This list contains exceptions which will be ignored during the consistency check.
const PLUGINLESS_HDB_FILES = new Set([".hdbgrants", ".hdbrevokes"])

module.exports = class HanaBuildPlugin extends require('../internal') {

    static get taskDefaults() { return { src: normalizePath(cds.env.folders.db) } }

    static hasTask() {
        if (cds.env.extends) return false
        const src = path.join(cds.root, this.taskDefaults?.src ?? 'db')
        if (!exists(src)) return false
        const db = cds.env.requires.db
        return db?.kind === 'hana' || db?.dialect === 'hana'
    }
    ...
    async build() {
        ...
        if (!cds.cli.options.dry) {
            ...
            if (wsPaths.length || cds.cli.options?.[OPTION_WS] || hasOptionValue(this.task.options[OPTION_WS], true)) {
                ...
                const seen = new Set()
                const wsFiles = (await this._getWorkspaceFiles(wsPaths)).filter(src => {
                    if (seen.has(src.filePath)) return false
            ...
            // relocate .hdbsynonymconfig files containing *.configure properties to cfg/ folder
            // HDI's template mechanism for configuration files only works from the cfg/ folder
            // For inplace builds (Java), restrict the scan to compileDest only to avoid moving user's
            // hand-crafted src/ files; for staging builds scan all of src/ (everything there is generated).
            const synonymConfigScanDir = this.isStagingBuild() ? undefined : this.task.options.compileDest
            const relocatedDirs = await this._relocateSynonymConfigsToCfg(synonymConfigScanDir)
            await this._writeCfgHdiConfig(relocatedDirs)
        }
    }
     ...
     * <br>
     * Note: Content created in staging build will be deleted by default.
     */
    async clean() {
    ...
    async _copyResourcesExt(src, dest, model) {
        ...
        if (resources.length) {
            ...
            // preserve last-write-wins semantics for duplicate basenames while parallelizing distinct file copies
            const csvResourcesByName = new Map()
            for (const res of resources) {
                // do not duplicate resources that are already contained in db/src/**
                if (res && /\.csv$/.test(res) && !res.startsWith(dbSrc)) {
                    csvResourcesByName.set(path.basename(res), res)
                }
            }
            await Promise.all(Array.from(csvResourcesByName.entries()).map(([fileName, res]) => {
                return this.copy(res).to(path.join(this.task.options.compileDest, csvFolder, fileName))
            }))
        }
    ...
    async _copyResources(src, dest) {
        ...
        for (const csvDir of csvDirs) {
            allFiles.push(...await find(csvDir, { filter: entry => regexData.test(entry) && !isdir(entry) }))
        }
    ...
    async _compileToHdbtabledata(model) {
        ...
        const allCsvFiles = await find(destSrcDir, { filter: (entry) => {
            if (isdir(entry)) {
                return true
            }
    ...
    async _stripCsvComments(csvFiles) {
        ...
        await Promise.all(csvFiles.map(file => {
            if (this.isStagingBuild() || file.startsWith(this.task.options.compileDest)) {
                return cds.utils.csv?.stripComments(file, null, this.task.options.trimCsvWhitespaces)
            }
        }))
    }
    ...
    async _compileToHdbmigration(model, format, wsPaths) {
        ...
        try {
            ...
            if (afterImage) {
                if (!_toEqualIgnoreMeta(lastDev, afterImage)) {
                ...
                if (migrationTableFiles.length > 0 && !fs.existsSync(path.join(dbSrcDir, FILE_NAME_HDICONFIG))) {
                    const template = await _readTemplateAsJson('.hdiconfig-cloud')
    ...
    async _writePackageJson() {
        ...
        if (exists) {
            DEBUG?.(`skip create [${path.relative(cds.root, pkgJson)}], already existing`)
        }
        if (this.isStagingBuild() && !exists) {
            const content = await _readTemplateAsJson(FILE_NAME_PACKAGE_JSON)
    ...
    async _writeHdiConfig(compileHdiPlugins) {
        ...
        const destHdiPlugins = await this._readTypesFromCompileDest()
        const hdiPlugins = new Set([...DEFAULT_HDI_PLUGINS, ...undeployHdiPlugins, ...compileHdiPlugins, ...destHdiPlugins])
        let hdiConfigPath
        ...
        //     const defaultHdiConfig = await this._readTemplateAsJson('.hdiconfig-cloud')
        ...
        const template = await _readTemplateAsJson('.hdiconfig-all')
        let content = {
            'file_suffixes': {}
        }
        ...
        if (Object.keys(content['file_suffixes']).length < hdiPlugins.size) {
            const missingPlugins = Array.from(hdiPlugins)
                .filter(key => !content['file_suffixes'][key.slice(1)])
                .filter(key => !PLUGINLESS_HDB_FILES.has(key))
    ...
    async _writeHdiNamespace() {
        ...
        const content = await _readTemplateAsJson(FILE_NAME_HDINAMESPACE)
    ...
    async _writeUndeployJson() {
        if (this.isStagingBuild()) {
            ...
            const templateEntries = await _readTemplateAsJson(FILE_NAME_UNDEPLOY_JSON)
            let newEntries = []
    ...
    async _readTypesFromUndeployJson() {
        ...
        if (fs.existsSync(file)) {
            ...
            if (Array.isArray(undeployList)) {
                const hdiconfig = await _readTemplateAsJson('.hdiconfig-all')
                ...
                undeployList.forEach(entry => {
                    ...
                    if (extName && !hdiPlugins.has(extName)) {
                        if (keys.has(extName)) {
                            ...
                            this.pushMessage(`Ignoring invalid entry '${entry}' in undeploy.json file`, SEVERITY_WARNING)
                        }
                    }
                })
            }
    ...
    async _readTypesFromCompileDest() {
        const hdiPlugins = new Set()
        const dir = path.join(this.task.dest, 'src')
        if (!exists(dir)) return hdiPlugins

        const template = await _readTemplateAsJson('.hdiconfig-all')
        const knownSuffixes = new Set(Object.keys(template['file_suffixes']).map(key => '.' + key))
        const files = await find(dir, { filter: (entry) => !isdir(entry) })
        files.forEach(file => {
            const suffix = path.extname(file)
            if (knownSuffixes.has(suffix)) hdiPlugins.add(suffix)
        })
        return hdiPlugins
    }
    ...
    async _validateMigrationTableFiles() {
        const dbSrcDir = path.join(this.task.src, "src")
        const migrationTableFiles = await find(dbSrcDir, { filter: (res) => {
        ...
        if (migrationTableFiles.length > 0) {
            ...
            await Promise.all(migrationTableFiles.map(async file => {
                try {
                    ...
                    throw new Error(`${path.relative(cds.root, file)}: ${e.message}`, { cause: e })
                }
            }))
     ...
     * @returns {Promise<Array>} Array of workspace files, each entry contains the db src directory,
    ...
    async _getWorkspaceFiles(workspacePaths) {
        ...
        for (const src of dbPaths) {
            const files = await find(src, { filter: (entry) => {
                if (entry.match(/(\/|\\)gen(\/|\\)?$/)) {
            ...
            files.forEach(file => workspaceFiles.push({ dir: src, filePath: path.relative(src, file), fileName: path.basename(file) }))
        }
    ...
    /**
     * Relocates .hdbsynonymconfig files containing *.configure properties (e.g. schema.configure,
     * database.configure) from the dest src/ folder to the dest cfg/ folder.
     * HDI's template mechanism for configuration files only works when the config files reside
     * in the cfg/ folder. The HDI deployer scans cfg/ at startup, detects *.configure markers,
     * and applies runtime substitution from bound service credentials.
     */
    async _relocateSynonymConfigsToCfg(scanDir) {
        const destSrcDir = path.join(this.task.dest, 'src')
        const destCfgDir = path.join(this.task.dest, 'cfg')
        // scanDir defaults to destSrcDir (staging builds); for inplace builds it is compileDest
        // so that only generated files in src/gen are considered, not user's hand-crafted src/ files.
        const effectiveScanDir = scanDir ?? destSrcDir

        const synonymConfigFiles = await find(effectiveScanDir, { filter: (entry) => {
            if (isdir(entry)) {
                return true
            }
            return path.extname(entry) === FILE_EXT_HDBSYNONYMCONFIG
        }})

        // Track which directories under cfg/ received relocated files and their extensions
        const relocatedDirs = new Map() // dir -> Set of file extensions
        await Promise.all(synonymConfigFiles.map(async (file) => {
            if (await _hasConfigureProperties(file)) {
                const relPath = path.relative(destSrcDir, file)
                const cfgDest = path.join(destCfgDir, relPath)
                DEBUG?.(`relocating ${path.relative(this.task.dest, file)} to cfg/ folder (contains .configure properties)`)
                await fs.promises.mkdir(path.dirname(cfgDest), { recursive: true })
                await fs.promises.rename(file, cfgDest)
                this.pushFile(cfgDest)

                const cfgDestDir = path.dirname(cfgDest)
                if (!relocatedDirs.has(cfgDestDir)) {
                    relocatedDirs.set(cfgDestDir, new Set())
                }
                relocatedDirs.get(cfgDestDir).add(path.extname(file))
            }
        }))
        return relocatedDirs
    }

    /**
     * Create .hdiconfig files in cfg/ subdirectories that received relocated config files.
     * This ensures that the HDI deployer can recognize the file types in those directories.
     * Only creates .hdiconfig in directories where one does not already exist.
     * @param {Map<string, Set<string>>} relocatedDirs Map of directory paths to Sets of file extensions
     */
    async _writeCfgHdiConfig(relocatedDirs) {
        if (!relocatedDirs || relocatedDirs.size === 0) {
            return
        }
        // Collect all relocated file extensions across all directories
        const allExtensions = new Set()
        for (const extensions of relocatedDirs.values()) {
        ...
        const template = await _readTemplateAsJson('.hdiconfig-all')
        const content = { 'file_suffixes': {} }
        for (const key in template['file_suffixes']) {
            if (allExtensions.has('.' + key)) {
                content['file_suffixes'][key] = template['file_suffixes'][key]
            }
        }
        if (Object.keys(content['file_suffixes']).length === 0) {
            return
        }

        // Write .hdiconfig to each distinct directory under cfg/ that received relocated files,
        // but only if one does not already exist
        for (const dir of relocatedDirs.keys()) {
            const hdiConfigPath = path.join(dir, FILE_NAME_HDICONFIG)
            if (!fs.existsSync(hdiConfigPath)) {
                await this.write(content).to(hdiConfigPath)
            }
        }
    }
...
/**
 * Checks whether an .hdbsynonymconfig file contains any *.configure properties.
 * These are HDI template markers (e.g. schema.configure, database.configure) that
 * require the file to be placed in the cfg/ folder for HDI's runtime substitution.
 * @param {string} file - absolute path to the .hdbsynonymconfig file
 * @returns {Promise<boolean>} true if the file contains .configure properties
 */
async function _hasConfigureProperties(file) {
    try {
        const content = JSON.parse((await fs.promises.readFile(file, 'utf-8')).toString())
        return _objectHasConfigureProperty(content)
    // eslint-disable-next-line no-unused-vars
    } catch (e) {
        // if the file cannot be parsed, leave it in place
        return false
    }
}

function _objectHasConfigureProperty(obj) {
    if (obj === null || typeof obj !== 'object') {
        return false
    }
    for (const key of Object.keys(obj)) {
        if (key.endsWith('.configure')) {
            return true
        }
        if (typeof obj[key] === 'object' && _objectHasConfigureProperty(obj[key])) {
            return true
        }
    }
    return false
