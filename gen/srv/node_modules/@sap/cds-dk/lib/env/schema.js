function mergePluginSchema(pluginSchema) {
    ...
    return {
        into: async (targetSchema) => {
            ...
            if (pluginSchema.export) {
                const exp = targetSchema.$defs?.cdsRoot?.properties?.export?.properties
                if (exp) {
                    Object.assign(exp, pluginSchema.export)
                    targetSchema._cds_schema_overlays.push('export')
                }
            }

            if (pluginSchema.importFrom) {
                const oneOf = targetSchema.$defs?.importOptions?.from?.oneOf
                if (oneOf) {
                    oneOf.push(pluginSchema.importFrom)
                    targetSchema._cds_schema_overlays.push('importFrom')
                }
            }
        }
    }
