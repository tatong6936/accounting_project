...
const { localhostRegex } = require('./util/urls');
...
const DEFAULT_LOCALHOST_USER = 'yves';
const DEFAULT_LOCALHOST_PASSWORD = '';
...
module.exports = class TenantAction {
    ...
    async run(paramValues, { tenant, local } = {}) {
        ...
        if (local) {
            ...
            // use 'yves' as default username against localhost if no username is provided
            if (!params.has('username') && params.has('appUrl') && localhostRegex.test(params.get('appUrl'))) {
                params.set('username', DEFAULT_LOCALHOST_USER);
                params.set('password', DEFAULT_LOCALHOST_PASSWORD);
                params = await AuthManager.login(params.toEntries()); // set reqAuth param
            }
    ...
    async runClientOnly(params, tenant) {
        ...
        await mtxFetch('POST', url, {
            headers,
            body: JSON.stringify({ tenant, metadata: parsedBody, options: parsedBody })
        }, params);
    }
