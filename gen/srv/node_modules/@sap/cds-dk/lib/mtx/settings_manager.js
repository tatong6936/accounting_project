...
const OAUTH_PATH_LEGACY_TOKEN = '/-/cds/login/token';
const OAUTH_PATH = '/-/cds/login/grant';
const OAUTH_META_PATH = '/-/cds/login/grant-metadata';
const OAUTH_UAA_PATH = '/oauth/token';
...
async function getTokenUrl(params, renewUrl) {
    ...
    if (params.has('clientid')) {
        return deriveUaaTokenUrl(params);
    }
    return appUrlWith(OAUTH_PATH, params);
}

function uaaTokenUrl(passcodeUrl, { mtls } = {}) {
    const url = new URL(passcodeUrl);
    url.pathname = url.pathname.replace(/\/passcode\/*$/, '').replace(/\/+$/, '') + OAUTH_UAA_PATH;
    url.search = '';
    if (mtls) {
        url.hostname = url.hostname.replace(/\.authentication\./, '.authentication.cert.');
    }
    return url.toString();
}

async function deriveUaaTokenUrl(params) {
    const mtls = params.has('key');
    const metadata = await fetchAuthMetadata(params);
    const announced = mtls ? metadata?.cert_token_url : metadata?.token_url;
    const passcodeUrl = metadata?.passcode_url;
    const tokenUrl = announced ?? (passcodeUrl && uaaTokenUrl(passcodeUrl, { mtls }));
    if (!tokenUrl) {
        throw 'Cannot determine the authorization server for client-credentials login. ' +
            'Ensure MTX is running with extensibility enabled and up to date.';
    }
    params.set('directUaa', true);
    DEBUG?.(`Derived authorization-server token URL for ${mtls ? 'mTLS' : 'client-credentials'} login: ${tokenUrl}`);
    return tokenUrl;
}

async function fetchAuthMetadata(params) {
    let fetchUrl = appUrlWith(OAUTH_META_PATH, params);
    DEBUG?.(`Trying to get authorization metadata from GET`, fetchUrl);
    if (params.has('subdomain')) {
        const u = new URL(fetchUrl);
        u.searchParams.set('subdomain', params.get('subdomain'));
        fetchUrl = u.toString();
    }
    let responseData;
    try {
        const response = await fetch(fetchUrl /* no auth required */);
        if (!response.ok) {
            throw await buildResponseError('GET', fetchUrl, response);
        }
        ...
        if (error.status === 404) {
            ...
            DEBUG?.(getMessage(`Getting authorization metadata failed`, { error }));
        } else {
            const netError = buildNetworkError('GET', fetchUrl, error);
            DEBUG?.(getMessage(`Getting authorization metadata failed`, { error: netError }));
        }
        return undefined;
    }
...
class SettingsManager {
    ...
    static async saveSettings(params) {
        ...
        DEBUG?.(`Saving settings for project ${projectFolder}`);
        ...
        await this._saveToFile(projectFolder, paramsMap);
    ...
    static async loadAndMergeSettings(params, logout = false) {
        ...
        DEBUG?.(`Loaded project settings for ${projectFolder}: ${loadedSettings.format()}`);
    ...
    static async updateUrls(params, logout, loadedSettings) {
        ...
        async function updateTokenUrl() {
            ...
            const machineLogin = params.has('clientid');
            const renewUrl = !logout && !params.get('skipToken')
                && (machineLogin || !tokenUrl || tokenUrl.includes('//-/cds') || !tokenUrl.includes(OAUTH_PATH)
                    || params.get('appUrl') !== loadedSettings.get('appUrl')
                    || params.get('subdomain') !== loadedSettings.get('subdomain'));
            if (renewUrl) {
                ...
                if (params.has('passcodeUrl')) {
                    params.delete('passcodeUrl');
                    DEBUG?.('Cleared stale passcode URL due to token URL renewal');
                }
            }
        }
        await updateAppUrl();
    ...
    static async addAuth(params, logout) {
        ...
        async function addPasscode() {
            if (!params.has('passcodeUrl')) {
                const passcodeUrl = (await fetchAuthMetadata(params))?.passcode_url;
                if (passcodeUrl) {
        ...
        if (params.get('clientid') && !logout && !params.has('clientsecret') && !params.has('key')) {
    ...
    static async deleteSettingsWithoutToken() {
        const projectFolder = getProjectFolder();
