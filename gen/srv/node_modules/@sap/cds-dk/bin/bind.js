...
module.exports = Object.assign(cds_bind, {
    staticCompletions: () => ({
        '--on':          ['cf', 'k8s'],
        '--for':         ['development', 'production', 'hybrid'],
        '-4':            ['development', 'production', 'hybrid'],
        '--out':         FILE(),
        '--output-file': FILE(),
...
async function cds_bind(args, options) {
    ...
    if (app) {
        ...
        services.forEach(service => {
            ...
            if (type === 'identity' && Array.isArray(service.credentials?.['consumed-services']) && service.credentials['consumed-services'].length > 0) {
                console.log(`skipping ${term.bold(service.name)}: identity binding consuming another service, not the application's user authentication`)
                return
            }
