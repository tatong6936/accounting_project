...
// Extracted from watch/index.js to avoid cds.env loaded too eagerly
//
...
(async () => {
  ...
  require('./vite-watch')
  const bootstrap = require('../../bin/cds')(...args)

  cds.once('listening', async ({ server }) => {
    process.send({ code: 'listening', port: server.address().port })
    // if (process.env.CDS_WATCH_HOT_RELOAD) {
  ...
  try {
    await bootstrap
    if (cds.service.providers.length === 0) return _no_services_found()
  ...
  function _no_models_found(e) { console.log (`
    No models found in ${(e.files||cds.resolve(e.model||'*',false)).filter(f => !(f in {schema:1,services:2}))}.
