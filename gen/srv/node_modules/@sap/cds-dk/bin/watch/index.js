...
const ignoreDefaults = [
  /\.git/,
  new RegExp(`${escapedSep}\\.cds(${escapedSep}|$)`), // .cds folder — but not .cds model files or .cdsrc.json
  /node_modules/,
  /_out/,
  /@types/,
  /@cds-models/,
...
async function watch ([cwd], {
  ...
  if (!installed('@sap/cds', cwd)) {
    ...
    env.NODE_OPTIONS = [env.NODE_OPTIONS, `--import ${require.resolve('./esm-resolve-registration.js')}`].filter(Boolean).join(' ')
  }
  ...
  if (!/false|no/.test(options.livereload)) {
    ...
    liveReload = new (require ('./livereload-server'))
    env.CDS_LIVERELOAD_PATH = require.resolve ('./livereload-connect')
  ...
  const nodemon = require('./node-watch')
  const watcher = nodemon ({
    script: resolve(__dirname,'./watched'), cwd, env, ext, ignore, watched,
    delay,
    options
  }).on('restart', (files)=>{
    ...
    case 'listening':
      ...
      if (options.open) require('./open').openURL (options.open, env.PORT)
