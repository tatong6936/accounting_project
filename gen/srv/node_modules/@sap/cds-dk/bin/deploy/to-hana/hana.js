...
const { bold, info, warn } = require('../../../lib/util/term');

const { bind, storeBindings, env: bindingEnv } = require('../../../lib/bind/shared');
const { BUILD_TASK_HANA } = require('../../../lib/build/constants');

const { options, flags } = require('../options');
...
module.exports = new class HanaDeployer {
  ...
  _applyServiceKeyToVcap(base, cfServiceInstanceName, serviceKey) {
    const hanas = Array.isArray(base.hana) ? base.hana : [];
    const hanaTagged = hanas.filter(s => s?.name === cfServiceInstanceName);
    if (hanaTagged.length === 0) {
      const allHanaTagged = hanas.filter(s => Array.isArray(s?.tags) && s.tags.includes('hana'));
      const entry = allHanaTagged.length === 1
        ? allHanaTagged[0]
        : { name: cfServiceInstanceName, label: 'hana', tags: ['hana'], credentials: {} };
      if (!hanas.includes(entry)) {
        hanas.push(entry);
      }
      hanaTagged.push(entry);
    }
...
`
      ...
      if (!serviceName && hasVCAPEnv) {
        ...
        if (!vcapEnv.VCAP_SERVICES || serviceName) {
          ...
          const result = this._applyServiceKeyToVcap(base, cfServiceInstanceName, serviceKey);
          vcapEnv.VCAP_SERVICES = result;
          process.env.VCAP_SERVICES = JSON.stringify(result);
        }
    ...
    const { env4 } = require('../../../lib/init/projectReader')
