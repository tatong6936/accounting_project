...
async function up() {
  ...
  const { approuterPath, html5DeployerPath, apps, appName, hasApprouter, hasXsuaa, isMonorepoMicroservice, hasKyma, hasMta, k8s } = project
  ...
  if (toKyma) {
    ...
    if (project.hasHtml5Repo) {
      const html5DeployerResources = join(cds.root, html5DeployerPath, 'resources')

      if (apps.length > 0) {
        await mkdirp(html5DeployerResources)
      }
      ...
      for (const { app, archiveName } of apps) {
        ...
        if (needsBuild || process.env.SKIP_CHECK == 'true') {
          ...
          await copy(zipPath).to(html5DeployerResources, `${app}.zip`)
    ...
    if (project.hasAppFrontend) {
      ...
      for (const { app } of apps) {
        const appPath = path.join(cds.root, cds.env.folders.app, app)
        const appPkgJson = exists(path.join(appPath, 'package.json'))
          ? JSON.parse(fs.readFileSync(path.join(appPath, 'package.json'), 'utf-8'))
          : {}
        if (!appPkgJson.devDependencies?.vite) continue
        ...
        if (needsBuild || process.env.SKIP_CHECK == 'true') {
          ...
          console.log(info(`Skipping build for React app '${app}' - no changes detected (checksum: ${currentChecksum.substring(0, 8)}...)`))
        }
      }

      // for single UI app where Vite app is directly added into app/
      const appPath = path.join(cds.root, cds.env.folders.app)
      const rootPkgJson = exists(path.join(appPath, 'package.json'))
        ? JSON.parse(fs.readFileSync(path.join(appPath, 'package.json'), 'utf-8'))
        : {}
      if (rootPkgJson.devDependencies?.vite) {
        const rootZipPath = path.join(appPath, `dist/${appName}.zip`)
        if(isRootApp) {
          run(`npm run build --prefix ${appPath}`)
          await copy(rootZipPath).to(html5DeployerResources, `${appName}.zip`)
        }
      }
    }
  } else {
  ...
  if (toKyma) {
    ...
    let helmUpgrade = `helm upgrade --install ${appName} ./gen/chart ${nsHelm} --rollback-on-failure --wait --wait-for-jobs --timeout=20m`
