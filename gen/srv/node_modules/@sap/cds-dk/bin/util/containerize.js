...
function processBuildpack(bp, name, tag, clearCache = false, image, publish = false) {
  let { type, path, env, builder } = bp.buildpack
  const dependencyMirrors = Object.keys(process.env).filter(key => key.startsWith('BP_DEPENDENCY_MIRROR_') && process.env[key])
  ...
  const target = publish ? image : `${name}:${tag}`
  return [['pack',
    ...appendSubcommand(target, 'build'),
    ...
    ...dependencyMirrors.flatMap(key => ['--env', key]),
    ...(publish ? ['--publish'] : []),
    ...cacheArgs
  ]]
}
...
async function parseYAML(filename, repoOpt = '', options) {
  ...
  const commands = await Promise.all(modules.map(async (m) => {
    ...
    const publish = key === 'buildpack' && options.push
    return {
      name,
      tag,
      buildCmd: buildType(bp, name, tag, key === 'buildpack' && options.clearCache, repoImg, publish),
