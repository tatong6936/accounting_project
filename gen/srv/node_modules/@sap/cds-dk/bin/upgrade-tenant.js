module.exports = Object.assign(upgradeTenant, {
...
# SYNOPSIS
    ...
    *cds upgrade-tenant* <tenant>

    Upgrade a tenant subscribed to a multitenant SaaS app to its latest base
    model.

    Running *cds upgrade-tenant* with a URL is intended for non-productive use only,
...
async function upgradeTenant([tenant], options = {}) {
    const { getMessage } = require('../lib/mtx/util/logging');
    if (!tenant) {
        throw getMessage('Tenant not given.', { command: 'upgrade-tenant' });
    }
    ...
    if (options.local) {
        ...
        if (options.user) {
            throw getMessage('Username cannot be specified when running @sap/cds-mtxs locally.', { command: 'upgrade-tenant' });
        }
        if (options.at) {
            throw getMessage('URL cannot be specified when running @sap/cds-mtxs locally.', { command: 'upgrade-tenant' });
        }
    }
    ...
    await upgradeTenant.init('upgrade').run({
        ...options,
        username,
        password
    }, libOptions);
