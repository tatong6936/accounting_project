...
class Minify extends Command {
  ...
  async exec ( args, options ) {
    ...
    try {
      this.csn = await this.load (sources,o)
      this.min = await this.minify (this.csn,o)
      if (o.dry) return
      if (o.inspect) return this.inspect (this.min,o)
      else return this.output (this.min,o)
    } catch (e) {
   ...
   */
  async minify (csn,o) {
    let m = this.minifier ??= o.dry ? (new Dryifier).for(csn) : new Minifier
   ...
   */
  output (csn,o) { // eslint-disable-line no-unused-vars
...
class Dryifier extends Minifier {
  ...
  for (csn) { this.all = Object.keys (csn.definitions); return this }
  capture = {}
  level = 0

  keep (each,d) {
    if (each in this.kept) return
    if (each in this.defs) this.capture[each] = ++this.level
    super.keep (each, d)
    if (each in this.defs) this.level--
  }
  ...
  minify (csn,o) {
    ...
    for (let [ each, level ] of Object.entries(this.capture))
      console.log ('  '+GRAY,'•'.repeat(level)+GREEN, each, RESET)
