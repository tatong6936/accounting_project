...
module.exports = Object.assign ( compile, {
  staticCompletions: () => ({
    '--to':                  ['json', 'yml', 'edm', 'edmx', 'edmx-v2', 'edmx-v4', 'sql', 'hdbtable', 'hana', 'cdl', 'xsuaa', 'openapi', 'asyncapi'],
    '--for':                 ['odata', 'sql'],
    '--dialect':             ['sqlite', 'h2', 'postgres', 'hana'],
    '--sql-dialect':         ['sqlite', 'h2', 'postgres', 'hana'],
    '--log-level':           ['debug', 'info', 'warn', 'error'],
    '--odata-version':       ['4.0', '4.01'],
    '--flavor':              ['sources', 'files', 'parsed', 'xtended', 'inferred'],
    '--service':             ['all'],
    '--lang':                ['all'],
    '--dest':                DIR(),
    '--openapi:config-file': FILE('json'),
...
async function compile (models, options={}) {
  ...
  let chain = model.then?. (m => model=m) ?? Promise.resolve(model) // model could be a sync. value
  ...
  // add processor for sort
  if (options.sort) chain = chain.then (sort)
  ...
  function consoleLog(o) {
    ...
    if (process.stdout.isTTY) {
      const { enabled: colors } = require('../../lib').utils.colors
