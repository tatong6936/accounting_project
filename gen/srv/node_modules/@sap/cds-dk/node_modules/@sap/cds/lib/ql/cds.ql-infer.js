...
infer.elements = function (q) {
  ...
  const source = subject.SELECT ? cds.ql(subject) : infer.target(q)

  return _infer_elements4 (source, q.SELECT.columns, q.SELECT.excluding, q.SELECT.from.as)
  function _infer_elements4 (source, columns, excluding, alias) {

    // SELECT from Books; SELECT * from Books
    if (!columns || columns.length === 1 && columns[0] === '*')
      if (!excluding) return source?.elements

    const excludes = excluding?.reduce((p,n)=>(p[n]=1,p),{}) || {}
    const elements = {}
    columns.forEach (c => {

      // 1) SELECT *, ... from Books
      if (c === '*') {
        for (let e in source.elements)
          e in elements || e in excludes || (elements[e] = source.elements[e])
        return
      }

      const ref = c.ref?.map(_name4); if (ref?.length > 1 && alias === ref[0]) ref.shift()
      const as = c.as || ref?.join('_') || c.func || c.val || cds.error `Alias required for column expressions ${c}`
      let d = source, is2many

      // 2.1) SELECT ... : String from Books
      if (c.cast) {
        return elements[as] = builtin [c.cast.type]
      }
      // 2.2) SELECT cast (... as String) from Books
      if (c.xpr?.[0]?.cast) {
        return elements[as] = builtin [c.xpr[0].cast.type]
      }

      // 3) SELECT title, author.name, $user.id from Books
      if (c.ref && d?.elements) {
        if (ref[0] in $pseudos) { d = $pseudos[ref[0]]; ref.shift() }
      ...
      // 4) SELECT 1, 2+3, count(*) from Books; SELECT type, name from sqlite.schema
        ...
        return elements[as] = _typeof(c)
      }

      // 5) SELECT author.books { title } from Books
      if (c.expand) {
        if (d.items) { d = d.items; is2many = true }
        const nested = _infer_elements4 (d, c.expand, c.excluding)
        const struct = new cds.struct ({ elements: nested })
        return elements[as] = is2many ? new cds.array ({ items:struct }) : struct
      }

      // 6) SELECT author.books.{ title } from Books
      if (c.inline) {
        const nested = _infer_elements4 (d, c.inline, c.excluding)
      ...
      // else return elements[as] = as === d.name ? d : { __proto__:d, name:as }
    })
    return elements
  }
}

// REVISIT: should this prefer cds.context.model over q._srv?
...
const $pseudos = {
  $user: { __proto__: builtin.String,
    elements: {
      id: builtin.String,
      locale: builtin.String,
      tenant: builtin.String
    }
  },
  $now: builtin.Timestamp,
  $at: builtin.Timestamp,
  $from: builtin.Timestamp,
  $to: builtin.Timestamp,
  $locale: builtin.String,
  $tenant: builtin.String,
}
