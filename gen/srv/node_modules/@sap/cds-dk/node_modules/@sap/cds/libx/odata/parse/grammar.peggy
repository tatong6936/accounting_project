...
// ---------- JavaScript Helpers -------------

  {
    ...
    const _custom = (k, v) =>  {
      ...
      const params = SELECT.from._params ??= Object.create(null)
      let t = params
      let x = k.match(/^(\w+)\[(.*)\]$/)
      if (x) {
        // protect against proto pollution
        if (x.some(s => s === '__proto__' || s === 'constructor')) return
        while (x) {
...
// ---------- Entity Paths ---------------
  ...
  ODataRelativeURI // Note: case-sensitive!
    ...
    ( o"?" o ( '&'? o QueryOption ( o'&'+o QueryOption )*)? )? o {
      if (count) {
        if (!SELECT) throw Object.assign(new Error('$count cannot be applied on the root of the service'), { status: 400 })
  ...
  args
    = ref:ref o"="o val:keyVal more:( COMMA args )? {
      const args = [ ref, '=', val ]
      if (more) args.push ('and', ...more[1])
      return args
    }
    / val:keyVal {return [val]}

  guardedargs
    = ref:ref o"="o val:guardedKeyVal more:( COMMA guardedargs )? {
      const args = [ ref, '=', val ]
      if (more) args.push ('and', ...more[1])
      return args
    }
    / val:guardedKeyVal {return [val]}
...
// REVISIT: per OData spec $apply should be also supported inside of $expand
  expand
  // REVISIT: per OData spec, $expand=*($levels=N) is valid but currently not supported
  = '*' OPEN { throw Object.assign(new Error('Query options on "$expand=*" are not supported; use a named navigation property, e.g. "$expand=*,nav($expand=...)"'), { status: 400 }) }
  / (
      c:('*' / ref) {
        const col = c === '*' ? {} : c
        if (col.ref?.length > 1) throw Object.assign(new Error(`navigation "${col.ref.join('/')}" in "$expand" is not supported`), { status: 400 })
...
// ---------- Expressions ------------
  ...
  val
    ...
    guardedval
    = val:val {return val}
    / val:simpleword {throw Object.assign(new Error(`Invalid value: ${val}`), { status: 400 })};

  // Matches non-uuid-standard guid values like 'example', 'ABC-123', or 'abc-def-ghi'
  // A "relaxedGuid" must be followed immediately by ',' or ')', i.e. at a key-predicate boundary, never mid-expression.
  // Examples:
  //   Accepted in:   Foo(ID=example)  Foo(ID=abc-def,other=xyz)  Foo(example)
  //   Not accepted:  $filter=ID eq example  (space after value, not ',' or ')')
  //
  // Negative lookahead: !( [0-9]+ ("."[0-9]+)? ("e"[0-9]+)? ![a-zA-Z\-] )
  // > rejects standalone numbers (digits optionally followed by decimal/exponent, NOT followed by a letter or dash).
  // Examples:
  //   123      → [0-9]+ matches, no letter/dash after → inner succeeds → !(...) fails → rule fails → falls to number ✓
  //   123.45   → decimal, no letter after              → inner succeeds → !(...) fails → rule fails → falls to number ✓
  //   28dd1030 → [0-9]+ matches '28', then 'd' letter → ![a-zA-Z\-] fails → inner fails → !(...) succeeds → rule matches ✓
  //   example  → [0-9]+ fails (no leading digit)       → inner fails → !(...) succeeds → rule matches ✓
  //
  // Positive lookahead: &( [),] / !. )
  // > ensures the match ends at a key-predicate boundary (closing paren or comma),
  // > preventing bare identifiers from being consumed mid-expression.
  relaxedGuid "a relaxed guid"
    = @$[A-Za-z0-9\-]|1..36| &[,)]

  // Extended val for key predicates: accepts standard val types plus relaxedGuid for non-hex string keys.
  // Used in `args` (simplePath — hierarchy functions). Fails silently if input matches nothing.
  keyVal
    = val / val:relaxedGuid {return {val}}

  // Like keyVal but throws a 400 error when the input looks like a word yet matches nothing above.
  // Used in `guardedargs` (main path rule — standard OData key predicates like Foo(ID=value)).
  // The throw turns an unrecognized bare word (e.g. Foo(ID=???)) into an explicit client error
  // instead of letting the parser silently fall back to a segment match.
  guardedKeyVal
    = val:keyVal {return val}
    / val:simpleword {throw Object.assign(new Error(`Invalid value: ${val}`), { status: 400 })};
  ...
  function
    = func:functionName OPEN args:functionArgs CLOSE {
      if (strict && !(func.toLowerCase() in strict.functions)) {
        throw Object.assign(new Error(`Function "${func}" is not supported`), { status: 501 })
      }
    ...
    = (
      ...
      func:("topcount"i/"bottomcount"i/"topsum"i/"bottomsum"i/"toppercent"i/"bottompercent"i) args:commonFuncTrafo {
        ...
        if (!SUPPORTED_APPLY_TRANSFORMATIONS[func]) {
          throw Object.assign(new Error(`Transformation "${func}" in $apply is not yet supported.`), { status: 501 })
        }
  ...
  date "a date"
    = s:$( [0-9]+"-"[0-9][0-9]"-"[0-9][0-9] // date
      ...
      if (s.split('-')[0].length > 4)
        throw Object.assign(new Error(`The type Edm.DateTimeOffset is not compatible with "${s}"`), { status: 400 })
      return s
    }
