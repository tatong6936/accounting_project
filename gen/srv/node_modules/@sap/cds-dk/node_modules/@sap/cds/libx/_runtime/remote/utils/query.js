...
const _extractHcqlRequestConfig = cqn => {
  if (cqn.flat) cqn.flat()
  ...
  cds.error(400, 'Invalid CQN object can not be processed.', JSON.stringify(cqn), _extractHcqlRequestConfig)
}

const _extractODataRequestConfig = (req, srv) => {
  if (req.query.UPDATE?.where) cds.error(400, 'Filtering is not supported to specify the subject of UPDATE requests')
  if (req.query.DELETE?.where) cds.error(400, 'Filtering is not supported to specify the subject of DELETE requests')

  const queryObject = cds.odata.urlify(req, srv)
  ...
  const requestConfig = {
    ...
    url: queryObject.path.replace(/\( /g, '(').replace(/ \)/g, ')')
  }

  if (queryObject.method !== 'GET' && queryObject.method !== 'HEAD') {
    requestConfig.data = srv.kind === 'odata-v2' ? convertV2PayloadData(queryObject.body, req.target) : queryObject.body
  }

  return requestConfig
}

const _createODataRequestConfig = (query, data, target, srvKind) => {
  ...
  if (data && requestConfig.method !== 'GET' && requestConfig.method !== 'HEAD') {
    requestConfig.data = srvKind === 'odata-v2' ? Object.assign({}, convertV2PayloadData(data, target)) : data
  }

  return requestConfig
}

const _pathToRequestConfig = (method, path, data, target, srvName, srvKind) => {
  let url = path
...
const _extractRequestConfig = (req, srv) => {
  // REVISIT: Split off the other 'kind's here? odata / odata-v4, odata-v2, rest ...

  if (srv.kind === 'hcql' && typeof req.query === 'object') return _extractHcqlRequestConfig(req.query)
  if (typeof req.query === 'object') return _extractODataRequestConfig(req, srv)
  if (typeof req.query === 'string') return _createODataRequestConfig(req.query, req.data, req.target, srv.kind)

  // > no model, no service.definition
  return _pathToRequestConfig(
    req.method,
    req.path,
    req.data,
    req.target,
    srv.definition?.name || srv.namespace,
    srv.kind
  )
}
...
module.exports.extractRequestConfig = (req, service) => {
  const requestConfig = _extractRequestConfig(req, service)
