...
exports = module.exports = new class {
  ...
  get inspect() {
    ...
    return super.inspect = (v, o = { depth: 11, colors: this.colors.enabled }) => inspect(v,o)
  }
  get format() {
  ...
  get yaml() {
    return super.yaml = require('yaml')
  }
  get tar()  {
 ...
 * @returns { Promise<any> }
 */
exports.import = exports._import = id => {
  if (id.endsWith('.mjs')) return $import (id)
  if (id.endsWith('.cjs')) return require (id)
  else try { return require(id) } catch (err) {
    if (err.message === 'Cannot use import statement outside a module') return $import (id) // for jest
    if (err.code === 'ERR_REQUIRE_ESM')  return $import (id)
    else throw err
  }
}

// On Windows, dynamic import only support file URLs
const $import = process.platform === 'win32'
? id => import (new URL('file://'+id).href)
: id => import (id)

try { // If the project is ESM, always use dynamic import
  let {type} = require(join(cds.root,'package.json'))
  if (type === 'module') exports.import = exports._import = $import
} catch {/* ignore */}
...
const ms4 = exports.ms4 = (ts, unit, u=unit) => {
  ...
  const factor = ms4[u||unit||'ms'] || cds.error `Invalid time span unit: ${unit} in ${ts}`
  return ts * factor
}
...
/**
 * Returns the matched cron expression if `t` is a valid cron string, or `undefined` otherwise.
 * Accepts either 5-field (`m h dom mon dow`) or 6-field (`s m h dom mon dow`) cron strings.
 */
exports.cron = t => /^([0-9*][-0-9*/,]*\s+){4,5}[0-9*][-0-9*/,]*$/.exec(t)?.[0]
