...
const _newSubData = (val, key, transition, el, inverse, service, options = {}) => {
  if ((!Array.isArray(val) && typeof val === 'object') || (Array.isArray(val) && val.length !== 0)) {
    ...
    if (!mapped.transition) {
      const subTransition = getTransition(el._target, service, options.event)
...
const _newColumns = (columns = [], transition, service, withAlias = false, options) => {
  ...
  columns.forEach(column => {
    ...
    if (mapped && column.expand) {
      ...
      const subtransition = getTransition(expandTarget, service, options.event)
...
const _mappedValue = (col, alias, target, existingAliases) => {
  ...
  if (col.ref) {
    ...
    if (columnRef.length > 1 && target) {
      ...
      if (firstElement?.foreignKeys) {
        ...
        if (columnRef[1] in firstElement.foreignKeys) {
          const flattenedName = `${firstElement.name}_${columnRef[1]}`
          // Only flatten when the FK column is not already directly accessible in the service entity.
          if (!existingAliases?.has(flattenedName)) columnRef = [flattenedName]
        }
      }
    }
...
const _checkForForbiddenViews = (queryTarget, event, serviceKind) => {
  ...
  if (select) {
    ...
    if (select.where) {
      const LOG = LOGGERS[serviceKind] ?? LOGGERS.app
      LOG._debug &&
...
const _getTransitionData = (target, columns, service, event) => {
  _checkForForbiddenViews(target, event, service.kind)
 ...
 * @param event
 */
const getTransition = (queryTarget, service, event) => {
  ...
  const existingAliases = new Set(transitionColumns.map(c => c.as || c.ref?.[0]))
  const mappedColumns = transitionColumns.map(column => _mappedValue(column, alias, mappingTarget, existingAliases))
...
const _entityTransitionsForTarget = (from, model, service, options) => {
  ...
  if (typeof from === 'string') {
    return model.definitions[from] && [getTransition(model.definitions[from], service, options.event)]
  }
  ...
  return from.ref.map((f, i) => {
    ...
    if (i === 0 && !previousEntity) {
      ...
      if (entity) {
        ...
        return getTransition(entity, service, options.event)
      }
    }
 ...
 * @param service
 */
const resolveView = (query, service) => {
  LOGGERS[service.kind] ??= cds.log(service.kind || 'app')
