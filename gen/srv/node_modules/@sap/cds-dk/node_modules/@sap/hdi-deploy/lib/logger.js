...
function WriteToLogFile (message, severity, id) {
  if (logFile) {
    const msgId = id !== undefined ? id : ++logMessageId;
    switch (severity) {
    case 0 : message = JSON.stringify({id: msgId, origin: '@sap/hdi-deploy', severity: 'INFO', message: message, timestamp_utc: new Date().toUTCString()});
      break;
    case 1 : message = JSON.stringify({id: msgId, origin: '@sap/hdi-deploy', severity: 'WARNING', message: message, timestamp_utc: new Date().toUTCString()});
      break;
    case 2 : message = JSON.stringify({id: msgId, origin: '@sap/hdi-deploy', severity: 'ERROR', message: message, timestamp_utc: new Date().toUTCString()});
      break;
    default: message = JSON.stringify({id: msgId, origin: '@sap/hdi-deploy', severity: 'UNKNOWN', message: message, timestamp_utc: new Date().toUTCString()});
    }
...
function writeToStdOut () {
  ...
  const id = ++logMessageId;
  const severity = str.startsWith('WARNING: ') ? 'WARNING' : 'INFO';
  const logMessage = severity === 'WARNING' ? str.replace(/^WARNING: /, '') : str;
  WriteToLogFile(logMessage, severity === 'WARNING' ? 1 : 0, id);
...
function error () {
  writeToStdErr.apply(null, arguments);
}
...
exports.setClientValue = function (value) {
  client = value;
};
...
exports.logVerbose = function () {
  if (verbose) {
    liveness_ping.sent();
    const str = util.format.apply(null, arguments);
    // bypasses writeToStdOut — verbose output must not go to IPC or log file
    process.stdout.write(`${str}\n`);
  }
};
