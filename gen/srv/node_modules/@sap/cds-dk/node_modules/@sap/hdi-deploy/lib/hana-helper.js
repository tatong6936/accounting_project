 ...
 * @param {String} authenticationX509 x509 certificates (key + cert in correct order)
...
module.exports = function (
  ...
  if (authenticationX509) {
    credentials.authenticationX509 = authenticationX509;
    logger.trace('credentials.authenticationX509 set');
  }
  ...
  return {
     ...
     * @param {Boolean} usingGroup Using role in rolegroup
     * @returns {CallbackFunction} Function taking a standard callback as the only argument.
     */
    grantRoles: function (roles, grantee, withAdminOption, usingGroup) {
      let sql = withAdminOption ? `GRANT ${roles.map(identifier).join(',')} TO ${identifier(grantee)} WITH ADMIN OPTION` : `GRANT ${roles.map(identifier).join(',')} TO ${identifier(grantee)}`;
      sql = usingGroup ? `${sql} USING GROUP` : sql;
     ...
     * @returns {CallbackFunction} Function taking a standard callback as the only argument.
     */
    grantSchemaRoles: function (schema, roles, grantee, withAdminOption, usingGroup) {
      let sql = withAdminOption ? `GRANT ${roles.map((role) => `${identifier(schema)}.${identifier(role)}`).join(',')} TO ${identifier(grantee)} WITH ADMIN OPTION` : `GRANT ${roles.map((role) => `${identifier(schema)}.${identifier(role)}`).join(',')} TO ${identifier(grantee)}`;
      sql = usingGroup ? `${sql} USING GROUP` : sql;
      return execute(sql);
    },
    /**
    ...
    /**
     * Revoke global roles from the grantee that were originally granted using a role group.
     * Looks up the role group via SYS.ROLES and issues REVOKE ... GRANTED BY <rolegroup>.
     *
     * @param {Array} roles Role names to revoke.
     * @param {String} grantee Whom to revoke from.
     * @returns {CallbackFunction} Function expecting a callback for async chaining.
     */
    revokeRolesUsingGroup: function (roles, grantee) {
      return function (cb) {
        if (!roles || roles.length === 0) {
          return cb(null);
        }
        const uniqueRoles = [...new Set(roles)];
        const sql = `SELECT ROLE_NAME, ROLEGROUP_NAME FROM SYS.ROLES WHERE ROLE_NAME IN (${uniqueRoles.map(strLit).join(',')}) AND ROLE_SCHEMA_NAME IS NULL AND ROLEGROUP_NAME IS NOT NULL`;
        execute(sql)(function (err, result) {
          if (err) {
            return cb(err);
          }
          if (!result || result.length === 0) {
            logger.warn(`using_group: none of the roles [${uniqueRoles.join(', ')}] granted to ${grantee} have a role group in SYS.ROLES; no revoke performed`);
            return cb(null);
          }
          const groupToRoles = {};
          result.forEach((row) => {
            if (!groupToRoles[row.ROLEGROUP_NAME]) {
              groupToRoles[row.ROLEGROUP_NAME] = [];
            }
            groupToRoles[row.ROLEGROUP_NAME].push(row.ROLE_NAME);
          });
          const foundRoles = new Set(result.map((row) => row.ROLE_NAME));
          const missingRoles = uniqueRoles.filter((r) => !foundRoles.has(r));
          if (missingRoles.length > 0) {
            logger.warn(`using_group: no role group found for roles [${missingRoles.join(', ')}] granted to ${grantee}; these roles will not be revoked`);
          }
          const revokeTasks = Object.keys(groupToRoles).map((roleGroup) => {
            const groupRoles = groupToRoles[roleGroup].map(identifier).join(',');
            return execute(`REVOKE ${groupRoles} FROM ${identifier(grantee)} GRANTED BY ${identifier(roleGroup)}`);
          });
          async.series(revokeTasks, cb);
        });
      };
    },

    /**
     * Revoke schema-local roles from the grantee that were originally granted using a role group.
     * Looks up the role group via SYS.ROLES and issues REVOKE ... GRANTED BY <rolegroup>.
     *
     * @param {String} schema Schema the roles belong to.
     * @param {Array} roles Role names to revoke.
     * @param {String} grantee Whom to revoke from.
     * @returns {CallbackFunction} Function expecting a callback for async chaining.
     */
    revokeSchemaRolesUsingGroup: function (schema, roles, grantee) {
      return function (cb) {
        if (!roles || roles.length === 0) {
          return cb(null);
        }
        const uniqueRoles = [...new Set(roles)];
        const sql = `SELECT ROLE_NAME, ROLEGROUP_NAME FROM SYS.ROLES WHERE ROLE_NAME IN (${uniqueRoles.map(strLit).join(',')}) AND ROLE_SCHEMA_NAME = ${strLit(schema)} AND ROLEGROUP_NAME IS NOT NULL`;
        execute(sql)(function (err, result) {
          if (err) {
            return cb(err);
          }
          if (!result || result.length === 0) {
            logger.warn(`using_group: none of the schema roles [${uniqueRoles.join(', ')}] in schema ${schema} granted to ${grantee} have a role group in SYS.ROLES; no revoke performed`);
            return cb(null);
          }
          const groupToRoles = {};
          result.forEach((row) => {
          ...
          const missingRoles = uniqueRoles.filter((r) => !foundRoles.has(r));
          if (missingRoles.length > 0) {
            logger.warn(`using_group: no role group found for schema roles [${missingRoles.join(', ')}] in schema ${schema} granted to ${grantee}; these roles will not be revoked`);
          }
          const revokeTasks = Object.keys(groupToRoles).map((roleGroup) => {
            const groupRoles = groupToRoles[roleGroup].map((r) => `${identifier(schema)}.${identifier(r)}`).join(',');
            return execute(`REVOKE ${groupRoles} FROM ${identifier(grantee)} GRANTED BY ${identifier(roleGroup)}`);
          });
          async.series(revokeTasks, cb);
        });
