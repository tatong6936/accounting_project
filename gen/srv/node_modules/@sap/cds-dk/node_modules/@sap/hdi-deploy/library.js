...
function deploy (contentDir, deployerEnv, callback, io) {
  ...
  const response = {messages: [], deployerMessages: []};
  ...
  try {
    ...
    deployer.on('error', function (error) {
      const msg = `Deployer library child process error: ${error.message}`;
      console.error(msg);
    ...
    deployer.on('message', function (message) {
      if (message.hasOwnProperty('final_message_sent')) {
        if (deployer.connected) {
          ...
          console.debug('Deployer already disconnected, skipping final_message_received');
        }
      } else if (message.origin === 'server') {
        response.messages.push(message);
      } else if (message.origin === '@sap/hdi-deploy') {
        response.deployerMessages.push(message);
      } else {
        response.messages.push(message);
      }
    });
    ...
    deployer.on('close', function (code, signal) {
      ...
      response.deployerMessages = dedupeMessagesById(response.deployerMessages);
      response.deployerMessages.sort((a, b) => parseInt(a.id) - parseInt(b.id));
