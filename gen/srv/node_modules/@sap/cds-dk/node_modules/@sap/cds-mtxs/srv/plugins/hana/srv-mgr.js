...
class ServiceManagerClient extends AbstractContainerManagerClient {
  ...
  create = async (tenant, parameters) => {
    ...
    return cacheBindings ? this._bindingsCache[tenant] = binding : binding
  }
  ...
  get = async (tenant, options = {}) => {
    ...
    if (invalidCredentials && cacheBindings) delete this._bindingsCache[tenant]
    try {
  ...
  remove = async (tenant) => {
    ...
    if (cacheBindings) delete this._bindingsCache[tenant]
    this.failures.delete(tenant)
  ...
  async _bindings4(tenants, { disableCache = false, failures, retryUntil } = {}) {
    ...
    const uncached = useCache ? tenants.filter(t => !(t in this._bindingsCache)) : tenants
    DEBUG?.('retrieving', { tenants }, { uncached })
    if (uncached.length === 0) return tenants.map(t => this._bindingsCache[t])
    const planId = await this._planId()
    ...
    for (const tenantChunk of tenantChunks) {
      ...
      const tenantLabelQuery = tenants === '*' ? undefined : `tenant_id in (${tenantChunk.map(t => `'${t}'`).join(', ')})`

      const bindingFieldQuery = `ready eq 'true'`
      const chunkBindings = []
      let token
      do {
        const params = token
          ? tenantLabelQuery ? { token, labelQuery: tenantLabelQuery, fieldQuery: bindingFieldQuery } : { token, fieldQuery: bindingFieldQuery }
          : tenantLabelQuery ? { labelQuery: tenantLabelQuery, fieldQuery: bindingFieldQuery } : { fieldQuery: bindingFieldQuery }
        const { data } = await this.fetchApi('service_bindings', { params }, { failures, retryUntil })
        const { items, token: nextPageToken } = data
        chunkBindings.push(...items)
        token = nextPageToken
      } while (token)

      // Direct path: filter bindings that have the service_plan_id label
      const planIdStr = String(planId)
      const directMatches = chunkBindings.filter(b => b.labels?.service_plan_id?.includes(planIdStr))
      // Fallback: for bindings without the label, resolve via service instance plan
      const unresolved = chunkBindings.filter(b => !b.labels?.service_plan_id)
      if (unresolved.length > 0) {
        const matchingInstances = []
        const instanceFieldQuery = `ready eq 'true' and service_plan_id eq '${planId}'`
        token = undefined
        do {
          const params = token
            ? tenantLabelQuery ? { token, labelQuery: tenantLabelQuery, fieldQuery: instanceFieldQuery } : { token, fieldQuery: instanceFieldQuery }
            : tenantLabelQuery ? { labelQuery: tenantLabelQuery, fieldQuery: instanceFieldQuery } : { fieldQuery: instanceFieldQuery }
          const { data } = await this.fetchApi('service_instances', { params }, { failures, retryUntil })
          const { items, token: nextPageToken } = data
          matchingInstances.push(...items)
          token = nextPageToken
        } while (token)

        const matchingInstanceIds = new Set(matchingInstances.map(i => i.id))
        directMatches.push(...unresolved.filter(b => b.service_instance_id && matchingInstanceIds.has(b.service_instance_id)))
      }
      fetched.push(...directMatches)
    }
    if (useCache) {
      const cacheMisses = Object.fromEntries(fetched.filter(b => b.labels?.tenant_id).map(b => [b.labels.tenant_id[0], b]))
      Object.assign(this._bindingsCache, cacheMisses)
      return tenants.map(t => this._bindingsCache[t])
    }
    return fetched
  }

  async _planId() {
    if (this._planIdCached) return this._planIdCached
    ...
    return this._planIdCached = data.items[0].id
  }

  async _offeringId() {
    if (this._offeringIdCached) return this._offeringIdCached
    ...
    return this._offeringIdCached = data.items[0].id
  }
