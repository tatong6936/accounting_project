...
class AbstractContainerManagerClient {
  ...
  _token = async () => {
    ...
    const raw = await token(this, { form: { grant_type: 'client_credentials' } })
  ...
  fetchApi = async (url, conf, { failures = 0, retryUntil } = {}) => {
    ...
    return fetchResiliently(conf.baseURL + url, conf, { retriesLeft: retries ?? 10, maxRetryAfter: maxRetryAfter ?? 5000, retryUntil, failures })
  }
  ...
  // base implementation used by srv-mgr
  _succeeded = (response) => response.data?.state === 'succeeded'
  _failed = (response) => response.data?.state === 'failed'
  _pollError = (response) =>
    response.data?.errors?.[0] ?? response.data.errors
    ?? { error: 'Unknown Error', description: `Polling failed with state ${response.data?.state ?? 'unknown'}. Last operation ${response.data?.id}` }
  ...
  static _errorMessage = (e, action, tenant) => {
    ...
    let cause = e.description || e.cause ? require('os').EOL + `Root Cause: ${e.description ?? e.cause}` : ''
    if (e.broker_error) cause += ' broker error: ' + e.broker_error.StatusCode + ' ' + e.broker_error.ErrorMessage + ' ' + e.broker_error.Description
    return msg + cause
  }
