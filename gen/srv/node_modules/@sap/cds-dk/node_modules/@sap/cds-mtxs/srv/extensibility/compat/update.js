...
module.exports = (srv) => {
    ...
    srv.before(['UPDATE', 'UPSERT'], 'Extensions', req => { // must be synchronous to be handled first
        ...
        // reject file names with path components (path traversal, see DbAdapter.put)
        for (const files of [bundles, csvData]) {
            if (Array.isArray(files)) files.forEach(_assertPlainFileName)
        }
    ...
    // rejects a FILE name that is not a plain file name (prevents path traversal)
    const _assertPlainFileName = function (file) {
        const name = file?.name
        if (typeof name !== 'string' || !name.length) return
        if (path.isAbsolute(name) || name.includes('/') || name.includes('\\') || path.basename(name) !== name) {
            throw new cds.error({ message: `Invalid file name: "${name}"`, code: 400 })
        }
    }
