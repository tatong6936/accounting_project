...
const { SKIPPED_DB } = require('../jobs-service')
...
module.exports = class ProvisioningService extends cds.ApplicationService {
  ...
  _toMetadataJson(metadata, subscribedTenantId) {
    if (!metadata) return { subscribedTenantId };
    try {
  ...
  async _getTenants(options) {
    const ds = await cds.connect.to(DeploymentService)
    const all = await ds.getTenants(options)
  ...
  async _upgrade(context) {
    ...
    // Merge static upgrade config with dynamic options (dynamic takes precedence)
    const upgradeConfig = cds.env.requires[this.name]?.upgrade
    if (upgradeConfig) {
      options.upgrade = cds.utils.merge({ ...upgradeConfig }, options.upgrade)
    }

    // Determine skipAfter: from explicit option or derived from package.json mtime
    let skipAfter = options?.upgrade?.skipAfter
    if (!skipAfter && options?.upgrade?.skipAlreadyUpgraded) {
      const { stat } = require('fs/promises')
      const { mtime } = await stat(cds.root + '/package.json')
      skipAfter = mtime.toISOString()
    }

    const tenantsOptions = skipAfter ? { skipAfter } : undefined
    let tenants
    let skipped = []
    if (all) {
      tenants = await this._getTenants(tenantsOptions)
    } else if (skipAfter) {
      const upgradeable = await this._getTenants(tenantsOptions)
      tenants = tenantIds.filter(t => upgradeable.includes(t))
      // Requested tenants filtered out by skipAfter are considered already upgraded.
      skipped = tenantIds.filter(t => !upgradeable.includes(t))
    } else {
      tenants = tenantIds
    }

    const { isSync } = this._parseHeaders(cds.context.http?.req.headers)
    if (!tenants.length && !skipped.length && isSync) return
    ...
    const { 'cds.xt.DeploymentService': ds } = cds.services
    const js = await cds.connect.to(JobsService)
    const { clusterSize = 3 } = js._config()
    const dbToTenants = (tenants.length && clusterSize > 1 && ds) ? await ds.tenantsByDb(tenants) : [new Set(tenants)]
    // Skipped tenants go into a separate pseudo-db cluster so JobsService records them (and their
    // tasks) as FINISHED without upgrading.
    let clusters = dbToTenants
    if (skipped.length) {
      // Normalize array-form to a db-id-keyed object so skipped tenants get their own cluster.
      clusters = { ...dbToTenants }
      // Drop skipped tenants from regular clusters (some tenantsByDb impls ignore the tenant list).
      const skippedSet = new Set(skipped)
      for (const db of Object.keys(clusters)) {
        clusters[db] = new Set([...clusters[db]].filter(t => tenants.includes(t) && !skippedSet.has(t)))
      }
      clusters[SKIPPED_DB] = skippedSet
    }
    LOG.info('upgrading', { tenants, skipped })


    return await new Promise((resolve, reject) => {
      const tx = js.tx({ tenant: cds.context.tenant, user: new cds.User.Privileged() })
      const job = tx.enqueue(DeploymentService, 'upgrade', clusters, { options }, async error => {
        if (error) {
  ...
  async _token() {
    ...
    const raw = await token(cred, { query: { response_type: 'token' }, form: { grant_type: 'client_credentials' } })
  ...
  _resolveCallbackUrl(callbackPath, baseUrl) {
    if (!callbackPath || !baseUrl) return undefined
    if (typeof callbackPath !== 'string' || !/^\/(?![/\\])/.test(callbackPath) || callbackPath.includes('\\')) {
      throw cds.error(`Invalid callback path: ${callbackPath}`, { status: 400 })
    }
    const base = new URL(baseUrl)
    const resolved = new URL(callbackPath, base)
