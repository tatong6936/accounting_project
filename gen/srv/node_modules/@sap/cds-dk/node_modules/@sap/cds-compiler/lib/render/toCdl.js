...
class CsnToCdl {
  ...
  renderAnnotationValue( annoValue, env, isTopLevel = false ) {
    ...
    // Ellipsis: `... up to <val>`
    if (annoValue['...'] !== undefined && Object.keys( annoValue ).length === 1) {
   ...
   * @returns {string}
   */
  // TODO: not quite sure why we split at '.' when quoting is needed
  quoteAnnotationPathIfRequired( anno, env ) {
    if (anno === '...')
      return this.quoteNonIdentifier(anno, env);
