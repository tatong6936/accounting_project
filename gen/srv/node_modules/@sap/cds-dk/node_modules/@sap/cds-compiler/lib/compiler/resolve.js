...
function resolve( model ) {
  ...
  /**
   * Warn if a type contains an unmanaged association, because the ON-condition
   * can't be rewritten if the type is used as element type.
   */
  function checkUnmanagedAssocInType( art ) {
    if (art._main?.kind !== 'type')
      return;
    if (!art.target || !getInheritedProp( art, 'on' ))
      return;

    warning( 'def-unsupported-assoc', [ art.name.location, art ], {},
             'With an unmanaged association, a type can only be used as include' );
  }
  ...
  function resolveQuery( query ) {
    ...
    if (!isLeadingQuery( query )) // will be done later
      forEachGeneric( query, 'elements', resolveRefs );
    if (query.from)
      resolveJoinOn( query.from );
    resolveWhereOrHaving( query, 'where' );
    if (query.groupBy)
      resolveBy( query.groupBy, 'groupBy', 'groupBy' );
    resolveWhereOrHaving( query, 'having' );
  ...
  function resolveWhereOrHaving( query, prop ) {
    const expr = query[prop];
    if (!expr)
      return;
    const merged = expr.op?.val === 'and' &&
      expr.args.some( a => a.$extended === prop );
    if (!merged) {
      const user = expr._block ? expr : query;
      resolveExpr( expr, prop, user );
      return;
    }
    for (const arg of expr.args) {
      const user = arg._block ? arg : query;
      resolveExpr( arg, prop, user );
    }
  }
  ...
  function checkBareDollarSelf( assoc ) {
    ...
    function registerBacklink( item, right, itemIsPath ) {
      ...
      if (assoc.$backlink) {
        ...
        if (last?._artifact !== assoc.$backlink._origin)
          setLink( assoc.$backlink, '_origin', null );
        return;
      }
  ...
  function checkBacklinkPartner( assoc ) {
    ...
    const origin = assoc.$backlink?._origin;
    const target = origin?.target?._artifact;
    if (!target)
      return;
    ...
    if (target._ancestors || target.$inferred === 'autoexposed') {
      ...
      if (redirectionChain( null, main, ancestor, true )) {
        const msg = (assoc.on.$inferred ? 'inferred' : 'std');
        message( 'ref-deprecated-backlink', [ assoc.$backlink.location, assoc ],
                 // eslint-disable-next-line @stylistic/object-curly-newline
                 { '#': msg, target, id: '$self', name: origin.name.id } );
        return;
      }
    }
    if (main.kind !== 'aspect') {
  ...
  function resolveExprNode( expr, expected, user, type ) {
    ...
    if (expr.path) {
      ...
      if (query._main?.kind === 'type' || query._origin === undefined) {
  ...
  function resolveExprPath( expr, expected, user ) {
    ...
    if (last.args || last.where || last.cardinality) {
      const unexpectedFilter = (expected !== 'annotation' && expected !== 'column' &&
        ...
        expected === 'annotation' && last.where?.args?.length === 0 && 'last-empty-filter';
