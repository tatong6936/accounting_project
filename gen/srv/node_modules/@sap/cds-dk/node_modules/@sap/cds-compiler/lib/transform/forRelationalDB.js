...
const { createReferentialConstraints, createDraftAdminDataConstraints, assertConstraintIdentifierUniqueness } = require('./db/constraints');
...
function transformForRelationalDBWithCsn(csn, options, messageFunctions) {
  ...
  // the DraftAdministrativeData_DraftUUID foreign key still gets a cascade constraint;
  // must run after generateDrafts, as the shadow tables don't exist before
  if (options.sqlDialect !== 'plain' && options.sqlDialect !== 'h2')
    createDraftAdminDataConstraints(csn, options);
  ...
  forEachDefinition(csn, [ (artifact, artifactName) => {
    ...
    generateFioriTreeViews(artifact, artifactName, messageFunctions, csnUtils, { setAnnotation, addElement, createScalarElement }, options, csn);
  }, transformViews ]);
