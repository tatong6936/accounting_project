 ...
 * e.g. `elements`.  TODO: remove `prop` and `name`, rename to `setParentAndMain`.
*/
...
function extensionRefContext( ext ) {
  ...
  else if (ext.artifacts || ext.$syntax === 'empty') {
    // extend … with definitions (or friends) or bare { extend: "ref" } in CSN
    // TODO v8: not for `extend context` and `extend service` → !ext.expectedKind ?
...
function isLeadingQuery( query ) {
  return query === query._main?.$queries?.[0];
}
