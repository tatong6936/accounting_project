...
function applyTransformationsInternal( parent, prop, customTransformers, artifactTransformers, _options, path = [] ) {
  ...
  function standard( _parent, _prop, node ) {
    if (_prop === '$backlink')  // above line in transformers doesn't seem to help...
      return;                   // ...in the OData backend
...
/**
 * Return true if the annotation expression ref node is a simple reference:
 * all ref steps are plain strings, and the node has neither `param` nor `cast`.
 *
 * @param {object} annotation An annotation expression node (has `ref`, optionally `param`, `cast`)
 * @returns {boolean}
 */
function isSimpleAnnoExprRef(annotation) {
  return annotation.ref && annotation.ref.every(s => typeof s === 'string') && !annotation.param && !annotation.cast;
}
