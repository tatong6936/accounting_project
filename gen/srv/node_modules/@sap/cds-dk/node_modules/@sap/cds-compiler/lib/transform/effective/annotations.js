...
const { expressionToCdlString } = require('../../render/toCdl');
...
/**
 * Replace all annotation expression nodes with their CDL-like string representation.
 *
 * @param {CSN.Model} _csn
 * @param {CSN.Options} options
 * @param {object} messageFunctions
 * @returns {object} Transformer object for applyTransformations
 */
function stringifyAnnotationExpressions( _csn, options, messageFunctions ) {
  // exclude '#' so enum-symbol annotation values are kept as is
  const expressionTransformers = {
    ref: stringifyExpression,
    xpr: stringifyExpression,
    list: stringifyExpression,
    val: stringifyExpression,
    func: stringifyExpression,
    SELECT: stringifyExpression,
    SET: stringifyExpression,
  };

  return {
    '@': (parent, prop, _anno, path) => transformAnnotationExpression(parent, prop, expressionTransformers, path),
  };

  function stringifyExpression( expressionNode, _prop, _value, _path, parent, parentProp ) {
    // Keep unchecked refs that already have textual representation.
    if (expressionNode.ref && typeof expressionNode['='] === 'string') {
      delete expressionNode.ref;
      delete expressionNode.param;
      delete expressionNode.cast;
      return;
    }

    parent[parentProp] = expressionToCdlString(expressionNode, options, messageFunctions);
  }
}
...
module.exports = {
  ...
  sealAnnoMagic,
  stringifyAnnotationExpressions,
};
