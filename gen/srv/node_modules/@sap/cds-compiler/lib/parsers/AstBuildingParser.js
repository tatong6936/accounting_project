...
class AstBuildingParser extends BaseParser {
  ...
  checkStructProps( dict ) {
    // see also isAnnotationExpression()
    if (!dict['=']) {
      const prop = dict['#'] && '#' ||
        dict.xpr && Object.keys( dict ).length === 1 && 'xpr' ||
        dict['...'] && Object.keys( dict ).length === 1 && '...';
      if (!prop)
        return;
    ...
    if (prop) {
      this.error( 'syntax-invalid-anno-struct', dict[prop].name.location,
                  { '#': 'std', prop, siblingprop: '=' } );
    }
    else if (dict['='].literal !== 'string' || !dict['='].val) {
