...
const { exprProperties, isAnnotationExpression } = require('../base/builtins');
...
function expr( e, spec ) {
  if (Array.isArray( e )) {
    if (e.length > 1)           // parse CSN produced with option 'structXpr'
      return condition( e, spec );
    if (e.length === 1) {       // CSN v.0.1.0 way for parentheses
      const loc = location();
...
function condition( cond, spec ) {
  ...
  return (cond.length < 3)      // nothing to structurize with 1 or 2 nodes
...
function calculateKind( def, spec, xsn ) {
  ...
  if (spec.prop === 'extensions') {
    ...
    if (def.extend && Object.keys( def ).length === 1)
      xsn.$syntax = 'empty';    // for Warning instead Error with undefined ref
