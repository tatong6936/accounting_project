...
const { transformExpression, isSimpleAnnoExprRef } = require('./applyTransformations');
      ...
      if (context?.$annotation) {
        ...
        adaptRefs.push((...args) => {
          ...
          if (refChanged && annotation['='])
            // For simple refs (all-string steps, no param/cast), = contains the string representation of the ref.
            annotation['='] = isSimpleAnnoExprRef(annotation) ? annotation.ref.join('.') : true;
        });
      }
      else {
        // adapt queries later
  ...
  function createForeignKeyElements() {
    ...
    function createFks( parent, prop, dict, path ) {
      ...
      Object.entries(dict).forEach(([ elementName, element ]) => {
        ...
        if (options.transformation === 'effective') {
          adaptAnnotationsRefs(fks, csnUtils, messageFunctions, eltPath, options);
