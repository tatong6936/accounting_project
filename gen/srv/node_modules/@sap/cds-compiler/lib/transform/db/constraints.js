...
/**
 * Add an `ON DELETE CASCADE` referential constraint on the `DraftAdministrativeData_DraftUUID`
 * foreign key of each draft shadow table, so deleting a draft's `DRAFT.DraftAdministrativeData`
 * row removes all its shadow rows.
 *
 * Runs after the draft shadow entities exist (they don't during `createReferentialConstraints`).
 * Their `DraftAdministrativeData` association is unmanaged and points at the service view, so the
 * regular pass skips it; this pass references the `DRAFT.DraftAdministrativeData` table directly.
 *
 * Opt-in via the `draftAdminDataConstraints` option and independent of the global
 * `assertIntegrity` / `assertIntegrityType` switches.
 *
 * @param {CSN.Model} csn
 * @param {CSN.Options} options
 */
function createDraftAdminDataConstraints( csn, options ) {
  if (options.draftAdminDataConstraints !== true)
    return;

  const draftAdminData = 'DRAFT.DraftAdministrativeData';
  if (!csn.definitions[draftAdminData])
    return;

  const delimiter = options.sqlMapping === 'hdbcds' ? '.' : '_';
  const foreignKeyName = `DraftAdministrativeData${ delimiter }DraftUUID`;

  const validated = !options.integrityNotValidated;
  const enforced = !options.integrityNotEnforced;

  forEach(csn.definitions, ( artifactName, artifact ) => {
    if (artifact.kind !== 'entity' || artifact.query || artifact.projection)
      return;
    const foreignKey = artifact.elements?.[foreignKeyName];
    const assoc = artifact.elements?.DraftAdministrativeData;
    // only draft shadow tables have both the association and its flattened foreign key
    if (!foreignKey || !assoc || assoc.type !== ASSOCIATION)
      return;

    const identifierPrefix = options.pre2134ReferentialConstraintNames ? '' : 'c__';
    const identifier = `${ identifierPrefix }${ getResultingName(csn, options.sqlMapping, artifactName) }_DraftAdministrativeData`;
    const mapKey = `${ getResultingName(csn, 'quoted', artifactName) }_DraftAdministrativeData`;

    if (!('$tableConstraints' in artifact))
      artifact.$tableConstraints = Object.create(null);
    if (!artifact.$tableConstraints.referential)
      artifact.$tableConstraints.referential = Object.create(null);

    artifact.$tableConstraints.referential[mapKey] = {
      identifier,
      foreignKey: [ foreignKeyName ],
      parentKey: [ 'DraftUUID' ],
      dependentTable: artifactName,
      parentTable: draftAdminData,
      onDelete: 'CASCADE',
      onDeleteRemark: options.testMode ? 'Draft shadow entries are deleted when their DraftAdministrativeData entry is deleted' : null,
      validated,
      enforced,
    };
  });
...
module.exports = { createReferentialConstraints, createDraftAdminDataConstraints, assertConstraintIdentifierUniqueness };
