...
function translateAssocsToJoinsCSN(csn, options) {
  ...
  // DO NOT DELETE the `require('fs').writeFileSync(...)` lines in this function!
  // Uncomment them to see the CSN input and output for A2J
  // require('fs').writeFileSync('./a2j_in.csn', JSON.stringify(csn,null,2))
  ...
  // require('fs').writeFileSync('./a2j_out.csn', JSON.stringify(newCsn,null,2))
  return newCsn;
}
...
function translateAssocsToJoins(model, inputOptions = {}) {
    ...
    function createOnCondition(assoc, srcAlias, tgtAlias, compareTenants) {
      ...
      */
        ...
        for (let i = 0; i < assoc.$flatSrcFKs.length; i++) {
          args.push({
            ...
            args: [ constructPathNode( [ srcAlias, prefixFK(assoc.$elementPrefix, assoc.$flatSrcFKs[i]) ] ),
              constructPathNode( [ tgtAlias, resolveTgtFKToAlias(assoc.$flatTgtFKs[i], tgtAlias) ] ) ],
          });
        }
        return parenthesise((args.length > 1 ? { op: { val: 'and' }, args } : args[0] ));
      }
      else if (assoc.on) {
      ...
      function cloneOnCondExprStream(expr) {
        const { args } = expr;
        const result = { op: { val: expr.op.val }, args: [ ], $parens: expr.$parens };
          ...
          // ON cond of the forward assoc with swapped src/tgt aliases
            ...
            if (fwdAssoc) {
              ...
              if (env.assocStack.length === 2) {
                ...
                result.args.push(createOnCondition(backlinkOf(assoc, fwdAssoc), ...swapTableAliasesForFwdAssoc(fwdAssoc, srcAlias, tgtAlias)));
              }
      ...
      function cloneOnCondExprTree(expr) {
        ...
        if (fwdAssoc) {
          ...
          return createOnCondition(backlinkOf(assoc, fwdAssoc), ...swapTableAliasesForFwdAssoc(fwdAssoc, srcAlias, tgtAlias));
        }
      ...
      function swapTableAliasesForFwdAssoc(fwdAssoc, srcAlias, tgtAlias) {
        ...
        if (assoc.kind === 'mixin' && (i > 0 || fwdTarget !== assocSource) && fwdOrigin.kind !== 'mixin') {
          ...
          // The backlink association's target may be reached through a view
          // over it, whose alias is not the target artifact itself; fall back
          // to the alias whose `_origin` chain reaches the target.
          let j = tas.findIndex(ta => ta._artifact === fwdOrigin.target._artifact);
          if (j < 0) {
            j = tas.findIndex((ta) => {
              for (let a = ta._artifact; a; a = a._origin) {
                if (a === fwdOrigin.target._artifact)
                  return true;
              }
              return false;
            });
      ...
      // Build the backlink ON from the renaming-aware `$backlink` rather than the
      // forward assoc's base-entity keys/on, if it is available.
      function backlinkOf( backlinkAssoc, fwdAssoc ) {
        const backlink = backlinkAssoc.$backlink;
        return backlink && (backlink.foreignKeys || backlink.on) ? backlink : fwdAssoc;
      }

      // Resolve the target foreign key to the alias of the target table.
      // If the target foreign key is not found in the scope of the target alias,
      // try to find it in the origin chain of the foreign key artifact.
      function resolveTgtFKToAlias( fk, tgtAlias ) {
        const env = tgtAlias && (tgtAlias._effectiveType || tgtAlias._artifact);
        const scope = env && env.elements;
        if (!scope || !fk || scope[fk.id] || !fk._artifact || !fk._artifact._origin)
          return fk;
        for (let o = fk._artifact._origin; o; o = o._origin) {
          if (o.name && scope[o.name.id])
            return { id: o.name.id, _artifact: fk._artifact, acc: fk.acc };
        }
        return fk;
      }
    } // createOnCondition
  ...
  */
  ...
  // to-csn now auto-wraps AND args that are OR-terms (see json/to-csn.js `case 'and'`),
  // so A2J no longer needs to pre-wrap sub-expressions to preserve SQL precedence.
