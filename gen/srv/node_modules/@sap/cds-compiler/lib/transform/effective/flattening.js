...
function flattenRefs(csn, options, csnUtils, messageFunctions) {
  ...
  forEachDefinition(csn, (def, defName) => {
    // Unbound actions/functions/events: only flatten annotation expressions, not params.
    // Their params may carry $self on-conditions that cannot be resolved without a query context.
    if (def.kind === 'action' || def.kind === 'function' || def.kind === 'event') {
      forEachMemberRecursively(def, (member, memberName, prop, path) => {
        Object.keys(member).filter(pn => pn.startsWith('@') && member[pn]).forEach((pn) => {
          transformAnnotationExpression(member, pn, [ refFlattener ], path);
  ...
  cleanup.forEach((obj) => {
    // strip leading $self from ref array and (if it is a string) from '='
    if (obj.ref && obj.ref[0] === '$self') {
      obj.ref.shift();
      if (typeof obj['='] === 'string' && obj['='].startsWith('$self.'))
        obj['='] = obj['='].slice('$self.'.length);
    }
  });
}
