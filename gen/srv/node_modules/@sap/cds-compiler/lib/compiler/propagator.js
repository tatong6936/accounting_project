...
function propagate( model ) {
  ...
  function onlyViaArtifact( prop, target, source ) {
    if (source._main)           // don't propagate from member
      return;
    // TODO v8: also do not propagate into member in general, now just not into returns
    if (target._parent?.returns === target)
      return;
    // Additionally, do not propagate for `select from Source:assoc`:
    if (target.query) {
      let alias = target._origin;
      while (alias?._origin?._main) // sub query in from
        alias = alias._origin._origin; // its first table alias
      if (alias?._artifact?._main) // Source:assoc` (BTW, `source` is its target)
        return;
    }
    annotation( prop, target, source );
  }
...
function getOrigin( art ) {
  ...
  while (origin?.kind === '$tableAlias' || origin?.kind === 'select')
