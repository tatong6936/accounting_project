...
function fns( model ) {
  ...
  const referenceSemantics = {
    ...
    annotate: {
      ...
      messageMap: { 'ref-undefined-def': 'ext-undefined-def' },
   ...
   */
  function extendableArtifact( art, user, ref, semantics ) {
    if (art.kind !== 'namespace')
      return art;
    const { location } = ref.path[ref.path.length - 1];
    if (user.artifacts)
      return art; // allow `extend with definitions` on "namespace" without message

    // for `annotate`, handle "namespaces" just like unknown artifacts: only emit a warning
    signalNotFound( 'ref-undefined-def', [ location, user ], null, { art }, semantics );
    return false;
  }
  ...
  // for message text variant and text argument:
  ...
  function getQueryOperatorName( query ) {
    ...
    for (let set = query._main.query; set.op; set = set.args[0]) {
      ...
      if (right.name && query.name.id >= (right.name.id ?? 0))
