...
async function main() {
    ...
    const steps = [
        {
            name: 'Drop misnamed mixed-case view (created by previous migration with double quotes)',
            sql: `DROP VIEW IF EXISTS "AccountingService_Batches"`
        },
        {
            name: 'Refresh lowercase AccountingService_Batches view (the one CAP actually queries)',
